...
# HF endpoint, pinned explicitly rather than read from the ambient HF_ENDPOINT /
# HF_HUB_ENDPOINT env vars, which localm never exposes as a setting.
...
# exists - see GGUFEmbedder.__init__).
#
# _EMBED_CTX_CEILING caps that auto-sized window, so batch and scratch buffers
# are not sized to a model's full native window on every call. An explicit n_ctx
# argument still wins outright over the auto-sized value.
...
def resolve_embedding_model_path(*, allow_download: Optional[bool] = None) -> Optional[str]:
    ...
    #    on a UNC path hands the string to the Windows SMB redirector, which
    #    blocks on an unroutable host and auto-authenticates against a reachable
    #    one, so this cannot run after the three lookups below. Returns None,
    #    per the not-resolvable contract, and records and logs the reason.
    bad = _nonlocal_spec_reason(spec)
    if bad:
        _record_resolve_failure(
            spec,
            # NOT {spec!r}: repr() doubles the separators in a Windows path, and
            # pathscrub's literal-prefix match and its regex backstop then fail
            # to recognise the escaped form. Quoting without repr() lets the
            # scrub see the real separators.
    ...
    #    is a different outcome from not being registered at all: the spec named
    #    something that exists, it is just not a GGUF this embedder can load. A
    ...
    try:
        ...
        if info:
            ...
            # stat, never after. A UNC hit is treated as nothing usable here and
            # falls through, exactly as an absent entry does.
            from localm.pathsafe import is_unc_or_device_path
            if path and not is_unc_or_device_path(str(path)):
                ...
                try:
                    ...
                    # permission error, a race with a delete): still a real
                    # registry hit, of unknown shape, so it is reported
                    # generically rather than letting the exception escape.
                    # embed_texts() does not wrap get_embedder() and
                    # /v1/embeddings catches only RuntimeError, so an uncaught
                    # OSError would surface as a 500 instead of a 422.
    ...
    if not known:
        # Neither branch uses {spec!r} - see the comment on the same choice at
        # this function's step-0 refusal above.
...
class GGUFEmbedder:
    ...
    def __init__(self, model_path: str, *, n_gpu_layers: int = 99,
        ...
        # up to IsolatedEmbedder through the runner's load meta, so the PARENT
        # can warn a user whose model is being mis-pooled.
        ...
        # same as the chat backend. The returned buffer must stay alive through
        # llama_load_model_from_file below; it is read once at load time, not
        # held as a live pointer. VRAM preflight lives in the PARENT
        ...
        # One contiguous scope over both native calls below, never re-entered
        # per call: a model load is one-shot, so dedup_native_stderr's
        # background-thread cost is paid once. It groups and repeat-collapses
        # llama_load_model_from_file's per-tensor lines and
        # llama_init_from_model's reserve lines, which otherwise go straight to
        # this child's inherited fd 2.
        #
        # This is not a console.print: dedup_native_stderr writes to a plain
        # file object opened by _stable_console_stream() with encoding utf-8 and
        # errors backslashreplace, so an unencodable byte is escaped rather than
        # raising. It touches neither stdout nor stdin, so it cannot corrupt this
        # child's multiprocessing-Queue IPC.
    ...
    def embed(self, texts: List[str]) -> List[List[float]]:
        ...
        with self._lock:
            ...
            # short-circuited to a zero vector and kept out of the packing math
            # below, having no tokens to pack.
...
class IsolatedEmbedder(VramSizingMixin):
    ...
    def __init__(self, model_path: str, *, n_gpu_layers: int = 99,
        ...
        # determine, since only it loads the model. The parent's own VRAM
        # preflight (_check_vram, see _preflight_vram below) needs a concrete
        # number BEFORE any child spawns, so self.n_ctx starts at the ceiling,
        # which the child's auto-sized value can never exceed, and is overwritten
        # with the real figure the child reports at load (_reload). The ORIGINAL
        # request (None for auto, or an explicit override) is kept separately so
        # _reload forwards it rather than this placeholder.
        ...
        # Seeding it here makes _reload() pass cpu_only to the child from the
        # first spawn. Exposed via gpu_fallback_reason() for the GUI.
        ...
        # overlapping RPCs are two threads blocked in the same resp_q.get() and
        # each takes whichever response arrives first, which can hand a caller
        # vectors belonging to a DIFFERENT text. Costs no throughput: the single
        # child drains req_q FIFO.
        ...
        # decremented lock-free in embed(), the same best-effort precision as
        # Engine's own _pin/_unpin.
    ...
    def _preflight_vram(self) -> None:
        ...
        # the vulkan build list_gpus() is blind to the real split devices, so the
        # DETECTED count collapses a live 2-way split to < 2 and would take the
        # single-GPU _check_vram() branch on an active split. On vulkan the
        # gpu_split_shortfall() call below self-skips the per-device check, logs
        # the skip at INFO, and the subprocess-isolated loader backstops an
        # oversized load.
    ...
    def _reload(self) -> None:
        ...
        # gpu_split_ratios is unset) is resolved HERE and carried into the child,
        # which must not probe for it. Skipped when this embedder is CPU-bound
        # anyway (cpu_only, or zero GPU layers): nothing is offloaded, so there
        ...
        if not cpu_only and self.n_gpu_layers != 0:
            # wait_for_inflight: off-loop here, since loads run in an executor or
            # CLI thread, so a heartbeat-probe collision joins instead of
            # declining auto into the equal fallback.
        ...
        # The child is the only place that loads the model, so it is the only
        # place that can resolve auto against the model's real native window.
        ...
        try:
            with self._rpc_lock:
                ...
                try:
                    ...
                    # child: EmbedderRunner has no __del__ and GC never
                    # terminates an mp.Process, so it would sit blocked on
                    # req_q.get() holding the model in VRAM while the next call
                    # spawned a SECOND worker beside it, out of reach of
                    # close(), reset_embedder() and release_for_exit().
                    ...
                    # A crash while GPU-offloaded is systemic rather than
                    # transient: an incomplete ROCm runtime (rocBLAS/hipBLASLt
                    # without their Tensile kernel data) aborts the native
                    # process on every GPU embed call, so retrying with the SAME
                    # config would crash again indefinitely. Falls back to CPU
                    # ONCE per embedder instance and retries THIS call inline,
                    # and warns, so the fallback is discoverable. In-memory only:
                    # the user's n_gpu_layers setting still governs the chat
                    # model.
...
# model load, _embedder_runner.LOAD_TIMEOUT_DEFAULT). So the cheap-looking
# readers below - loaded_dim, loaded_path, last_error, gpu_fallback_reason - are
# cheap in WORK and unbounded in WAITING, and each one's does-not-trigger-a-load
# line means only the first of those. NONE OF THEM MAY BE CALLED FROM AN
# `async def` ROUTE HANDLER: on the event loop that wait blocks every client and
# every route. scripts/check_event_loop_offload.py fails on a new instance.
...
#          timeout means not-already-warm, the same answer the blocking call
#          would have given.
...
# last failed, or None when nothing is latched. Not a bare bool, so a changed
# spec can be told apart from the same broken one: see get_embedder for how a
# changed spec clears the stale latch, and _set_resolve_outcome for the matching
# resolve-side guard.
    ...
    *on_progress*, if given, receives coarse human-readable stage announcements
    ...
    with _LOCK:
        ...
        if _LOAD_FAILED_SPEC is not None:
            ...
            # caller that changed embedding_model since the failure was recorded
            # is recognised as asking about something new. Reused below, not
            # re-derived, for the second latch test.
            cur_spec = _current_spec()
            if _LOAD_FAILED_SPEC == cur_spec:
                return None
            # The config has moved on since this was latched, so it no longer
            # describes what is about to be tried and must stop blocking it.
        ...
        # choice, else full offload. The FINAL placement is chosen after the swap
        # runs, against post-eviction free VRAM, so a successful eviction still
        # yields a GPU load and only a card that stays full demotes the embedder
        # to CPU (see _choose_embedder_gpu_layers).
    ...
    # here for a running chat load therefore does not hold _LOCK, so status
    # probes stay responsive.
    ...
    with _LOAD_LOCK:
        with _LOCK:
            ...
            if _LOAD_FAILED_SPEC is not None:
                ...
                _LAST_ERROR = None
            try:
                # The stated bound is the deadline this stage actually runs
                # under: _embedder_runner.LOAD_TIMEOUT_DEFAULT (300s), which
                # applies because _reload's spawn_and_load() passes no override.
                ...
                # getattr, not attribute access: this status line runs inside the
                # try, so anything it raised would be caught as a LOAD failure
                # and drop embeddings to lexical-only for the rest of the
                # process. A line that only describes the load must not fail it.
                ...
                # whatever embedding_model names right now, and a later call
                # compares against that identity to tell this exact broken model
...
# A daemon worker already dies with the parent, but it is torn down explicitly
# at interpreter exit so its queue feeder threads never delay shutdown. The
# embedder outlives any single request and, unlike GgufBackend/ModelRunner, is
# not torn down by an owning caller on every unload.
