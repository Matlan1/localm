...
class VramSizingMixin:
    ...
    # badge all reason about "does it fit" with the same number; a class attribute
    # rather than a module read so tests can monkeypatch it.
    _VRAM_OVERHEAD_BYTES = VRAM_OVERHEAD_BYTES

    # Latched once a torch import has hit the DLL entry-point conflict in this
    # process, so it is never retried. A BACKSTOP: _free_total_vram_bytes() skips
    # the torch attempt entirely whenever _loader.native_lib_loaded() is True, so
    # this only covers a path that reaches `import torch` some other way. A class
    # attribute, shared across every instance in this process, so tests can reset
    # it like the other per-process caches.
    _torch_rocm_init_broken: bool = False

    # Latched once a torch VRAM read has BLOWN ITS DEADLINE in this process.
    # Separate from _torch_rocm_init_broken above, which names a DLL entry-point
    # conflict that RAISES; this one is a wait that never returns. Both produce
    # the same response, stop asking torch. Latched rather than retried: each
    # retry costs another full deadline and another abandoned thread.
    ...
    def _free_total_vram_bytes() -> "tuple[Optional[int], Optional[int]]":
        ...
        if VramSizingMixin._torch_vram_read_wedged:
            return None, None
        # Once llama.cpp's own native runtime is already loaded IN THIS PROCESS
        # (true inside GgufWorker and the embedder's isolated worker, which load
        # it to run their model), a torch import hits the DLL-identity conflict
        # every time, and each worker is a fresh process so the
        # _torch_rocm_init_broken cache above never carries over. The attempt is
        # skipped; this method's caller falls back to gpu_memory_isolated(),
        # which answers without touching torch.
        from localm.inference.backends.llamacpp import _loader
        if _loader.native_lib_loaded():
            return None, None
        # Gated on torch not already being resident: a torch in sys.modules has
        # finished importing here, so the reads below are ordinary calls and the
        # child probe's verdict says nothing about them. See the docstring.
        if "torch" not in sys.modules:
            ...
            if discover.isolated_torch_unavailable():
                ...
                return None, None
        # Bounded, because neither the import nor a torch.cuda call has a timeout
        # of its own and the caller here is a model load. Same abandon-the-thread
        # shape as discover._list_gpus_with_status; see the docstring for why
        # (None, None) is a safe release value.
        result: dict = {}
        ...
        try:
            ...
            # held by an abandoned import). Degrade to unmeasurable rather than
            # run the read unbounded on the caller's own thread.
        ...
        if not done.wait(deadline):
            ...
            # guarantees that): GPU sizing now runs on the isolated native probe,
            # or on the configured n_gpu_layers if that cannot answer either.
            _dbg.warning(
    ...
    def _torch_free_total_uncapped() -> "tuple[Optional[int], Optional[int]]":
        ...
        try:
            ...
            # isolated-probe fallback. Logged at debug rather than a bare pass so
    ...
    def _free_vram_bytes(cls) -> Optional[int]:
        ...
        through here."""
        # One debug line per read, stating raw/source/corrected: every value here
        # feeds a sizing decision.
        from localm.debuglog import logger as _dbg
    ...
    def _kv_bytes_per_token(self) -> int:
        ...
        if cached is None:
            ...
            try:
                ...
                # Not fatal (step 3 still answers), but logged so a regression in
                # the probe is discoverable instead of invisible.
                from localm.debuglog import logger as _dbg
    ...
    def _check_vram(self) -> None:
        ...
        # single main-GPU reading under-budgets a model that fits only combined.
        # See _split_free_total_bytes for the full contract and fallbacks.
        ...
        # n_cpu_moe load pins its routed-expert weights to system RAM, where they
        # never draw on this budget at all.
        ...
        # (0 < g < 99, auto-sized or an explicit -g) puts only some layers on the
        # GPU. A full or "all" load (>= 99) charges the entire weight.
        ...
        if total is not None and need > total:
            # On a split box the ceiling being exceeded is the split's combined
            # one, so the message says that rather than "this GPU".
            ceiling = (
        ...
        where = (f" across the {split_devices} GPUs in the configured split"
                 if split_devices >= 2 else "")
        # This fires only when even a possibly-inflated free already reads as
        # insufficient, but the quoted GB figure itself can still be the raw,
        # cross-process-blind reading when the device-global correction declined,
        # so the note below says so rather than stating it as fact.
    ...
    def _check_context_fit(self, n_ctx: int, current_ctx: int = 0) -> Optional[bool]:
        ...
        # (None, None, 0) without probing (its native_lib_loaded guard), and the
        # single-device fallback below stays honest on a split box because the
        # isolated native probe declines at 2+ GPU devices
        ...
        # VRAM (offload_kqv); CPU layers' KV lives in system RAM. Mirrors
        # _check_vram's weight fraction so a partial auto load is charged only its
        # GPU share. Uses architecture-accurate per-token KV where it can be had
        # (the loaded model's accessors, else this file's GGUF header), and only
        # then the size-class heuristic.
        ...
        #  - current KV in VRAM (normal): freeing it returns that KV to the VRAM
        #    pool, and `free` was measured with the old KV still resident, so the
        #    NET growth is the charge (delta <= free  <=>  full target <= budget).
        #  - current KV already in SYSTEM RAM (a prior grow chose
        #    offload_kqv=False): the GPU holds NO KV, `free` already reads the
        #    whole KV budget, and freeing the old context reclaims RAM, not VRAM.
        #    The FULL target is charged, so the KV returns to VRAM only when the
        #    whole of it genuinely fits and otherwise stays in RAM.
        ...
        # State the decision: this runs in the worker, where nothing else
        # surfaces what the grow was judged against.
        from localm.debuglog import logger as _dbg
        ...
        # Does not fit VRAM: keep the FULL window, put the KV cache in system RAM
        # and let generation run slower. The hint is emitted once per loaded-model
        # session, since every subsequent grow overflows the same way.
        if not self._ram_kv_hint_shown:
            self._ram_kv_hint_shown = True
            from localm.debuglog import logger as _dbg
            # Same caveat as _check_vram's warning: this fires only when even a
            # possibly-inflated free already reads as insufficient, but the quoted
            # GB figure can still be the raw, cross-process-blind reading when the
            # device-global correction declined.
    ...
    def _auto_ctx_max(self, capped: bool = True) -> int:
        ...
        # _effective_model_bytes_for_vram(), not the raw _model_bytes(), as in
        # _check_vram: an n_cpu_moe load's pinned expert weights never draw on
        # this budget.
    ...
    # cached yet). Conservative: llama.cpp offloads min(n, real_count), so
    # overshooting is clamped in llama.py and undershooting only underuses VRAM.
    # The true count is cached after the first load (model_meta) and used from
    # then on.
    _ASSUMED_LAYERS = 32
    ...
    def _auto_gpu_layers(self) -> Optional[int]:
        ...
        # _effective_model_bytes_for_vram(), not the raw _model_bytes() (only the
        # EXISTENCE check above needs the raw file size), as in _check_vram: an
        # n_cpu_moe load's pinned expert weights never draw on this budget.
        model = self._effective_model_bytes_for_vram()
    ...
    def _effective_gpu_layers(self) -> int:
        ...
        if auto is None:
            ...
            # can answer - no GPU present, or the probe daemon is unreachable)
            # still needs a working default: offload via the display driver and
            # let the model try to fit as configured. A debug line rather than a
            # per-load console notice; if a full offload then does not fit, the
            # native load fails loudly with a VRAM hint (load()'s except handler).
            from localm.debuglog import logger as _dbg
