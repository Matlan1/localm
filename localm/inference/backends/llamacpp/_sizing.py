...
"""VRAM measurement and load-sizing logic shared by ``GgufBackend`` (the parent-facing proxy) and ``GgufWorker`` (the child process that owns the real native model)."""
...
def embedder_ctx_reservation_bytes() -> int:
    """VRAM to hold back from the auto-sized context budget for the CONFIGURED embedding model, so the chat model's window does not claim the room the embedder will need when it loads later (first memory/RAG use)."""
    try:
...
class VramSizingMixin:
    ...
    def _free_total_vram_bytes() -> "tuple[Optional[int], Optional[int]]":
        """(free, total) bytes on the configured main GPU device (device 0 when unset - see main_gpu_index / discover.resolve_main_gpu_index), or (None, None) when not measurable."""
        import sys
        import threading
    ...
    def _torch_free_total_uncapped() -> "tuple[Optional[int], Optional[int]]":
        """The actual torch read, with NO bound - call :meth:`_free_total_vram_bytes`, not this."""
        try:
            import torch
        except Exception as e:
            from localm.debuglog import logger as _dbg
    ...
    def _free_vram_bytes(cls) -> Optional[int]:
        """Free VRAM in bytes on the GPU the model runs on, or None when not measurable."""
    ...
    def _device_global_free_bytes(total: Optional[int]) -> Optional[int]:
        """``total`` minus ALL-process VRAM usage on the configured main GPU, or None when no device-global correction applies - the raw reading is not known-blind on this platform, or the correction source cannot map/answer."""
        if total is None:
            return None
        try:
    ...
    def _free_reading_may_be_blind() -> bool:
        """Whether the free-VRAM figure a caller is about to PRINT could still be the raw, cross-process-blind reading rather than _device_global_free_bytes's correction of it."""
    ...
    def _total_vram_bytes(cls) -> Optional[int]:
        """Total VRAM in bytes on the configured main GPU device, or None when not measurable."""
        return cls._free_total_vram_bytes()[1]

    @classmethod
    def _split_free_total_bytes(cls) -> "tuple[Optional[int], Optional[int], int]":
        """``(free, total, devices)`` summed across the 2+ devices this load will actually spread over, or ``(None, None, 0)`` when no combined budget applies and the caller must fall back to the single-device readings above."""
        from localm.inference.backends.llamacpp import _loader
        if _loader.native_lib_loaded():
            return None, None, 0
        try:
    ...
    def _split_overhead_bytes(self, devices: int) -> int:
        """``_VRAM_OVERHEAD_BYTES`` scaled by how many devices the load spreads over - the flat constant when it spreads over one (or the count is not known), so the single-GPU path is arithmetically unchanged."""
    ...
    def _effective_model_bytes_for_vram(self) -> int:
        """VRAM-resident weight bytes for THIS load: ``_model_bytes()``, minus whatever ``n_cpu_moe`` pins to SYSTEM RAM instead (see llama.py's ``_apply_cpu_moe`` - the routed-expert tensors of the first ``n_cpu_moe`` layers never touch VRAM at all)."""
    ...
    def _vram_holder_hint(self) -> str:
        """Best-effort: name a concrete live sibling localm instance holding VRAM on this same GPU device (port, model, how long ago it last updated), via the cross-install GPU-coordination registry (``localm.gpu_registry``) - instead of the generic 'another GPU app' guess."""
        try:
            from localm.config import load_config
            from localm.discover import resolve_main_gpu_index
    ...
    def _bytes_per_token(model_bytes: int) -> int:
        """KV bytes per token, estimated from the model's size class (larger models have more layers and wider KV heads; sliding-window models need less, so this stays deliberately conservative)."""
    ...
    def _check_vram(self) -> None:
        """Warn - loudly and with options - when the model is unlikely to fit in currently free VRAM, and refuse outright when even a clean, otherwise- empty card could not hold weights plus the requested context's KV cache (a 'can never fit' case, distinct from 'something else is using the GPU' - freeing VRAM..."""
    ...
    def _check_context_fit(self, n_ctx: int, current_ctx: int = 0) -> Optional[bool]:
        """Decide WHERE the KV cache for a context of *n_ctx* tokens must live - wired as LlamaCpp's ``vram_check`` hook, consulted by ``_prefill_fresh_context()`` before it (re)creates a bigger context."""
    ...
    def _cached_layer_count(self) -> Optional[int]:
        """The model's true transformer layer count if a prior load cached it, else None (never loaded yet - the caller falls back to _ASSUMED_LAYERS)."""
    ...
    def _auto_gpu_layers(self) -> Optional[int]:
        """Pick how many layers to offload to the GPU from free VRAM, or None when VRAM is not measurable by ANY path - neither torch.cuda nor the isolated native probe (_free_vram_bytes' loader.gpu_memory_isolated() fallback, which answers independently of torch.cuda) can answer, e.g. no GPU is present at all..."""
        free, _split_total, split_devices = self._split_free_total_bytes()
        if free is None:
            free = self._free_vram_bytes()
            split_devices = 1   # single-device reading - the flat overhead
