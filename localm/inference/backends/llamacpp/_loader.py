...
# load_lib() has run. A build can load cleanly yet register zero backends, so
# this is tracked separately from _loaded_lib and read by setup-llama's
# load-test, which treats a runtime that loads but cannot compute as a FAILED
# provision.
...
def _candidate_dirs() -> List[Path]:
    ...
    try:
        ...
        # package with no lib_dir()) is reported here rather than left to
        # surface later as a misleading "Cannot find llama.dll".
        logger.warning(
...
def _add_to_search_path(directory: Path) -> None:
    ...
    if sys.platform == "win32":
        ...
        # build's own rpath are what actually resolve the deps.
...
def _register_ggml_backends(binary_dir: Path, lib: ctypes.CDLL) -> bool:
    ...
    * Some upstream prebuilts ship the backends as real ``GGML_BACKEND_DL``
    ...
    # registered. When the count symbol is queryable it is authoritative; only a
    # build without the symbol falls back to "did any plugin load".
...
# does NOT unset a variable, and ggml reads an empty string as still PRESENT,
# i.e. still disabled.
...
def load_lib() -> ctypes.CDLL:
    ...
    # can see them. Windows resolves per import table, not through a flat global
    # table, so it has no equivalent hazard.
    if sys.platform != "win32":
        ...
        try:
            ...
            # otherwise succeed, so fall through and let the ordinary load path
...
# ALLOWLISTS GPU rather than excluding iGPUs and accelerators by value. Anything
# needing another member must read the header for the runtime actually
# provisioned rather than add a constant here.
GGML_DEV_TYPE_CPU = 0
GGML_DEV_TYPE_GPU = 1
    ...
    with _PROBE_LOCK:
        ...
        if _PROBE_PROC is None or _PROBE_PROC.poll() is not None:
            ...
            try:
                ...
                # Logged rather than swallowed: a silent None here is
