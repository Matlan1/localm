...
def _arm_native_crash_trace(path) -> None:
    ...
    Armed as early as possible - before HFWorker or any native/torch library
    is anywhere near loaded - because a fault can only be captured by a
    handler that was already installed when it happened.
    ...
    try:
        ...
        if not faulthandler.is_enabled():
            logger.warning(
                ...
                "but is_enabled() is False - a native fault in this worker "
                "will produce no stack trace")
...
def _runner_entry(req_q, resp_q, ctrl_q, crash_trace_path=None) -> None:
    """Process target. Arms the native-fault trace first (see
    ``_arm_native_crash_trace`` - must happen before anything native loads),
    then wraps ``_runner_main`` so any exception escaping it is logged via
    the ``logging`` module before the process dies, not left to
    ...
    control there, so no ``except`` clause, including this one, can run.
    ``_arm_native_crash_trace``'s faulthandler capture is the one mechanism
    that CAN say where such a fault happened."""
...
class HFRunner:
    ...
    def _spawn(self) -> None:
        ...
        self._proc = ctx.Process(
            ...
            args=(self._req_q, self._resp_q, self._ctrl_q, self._crash_trace_path),
