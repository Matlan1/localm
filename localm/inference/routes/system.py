...
def register(app: FastAPI, ctx) -> None:
    ...
    async def health():
        ...
        # No engine is warm right now, which is not the same as nothing being
        # here: an eviction (unload_all_models) keeps the Engine in _engines for a
        # lazy reload, and get_engine's unnamed-request resolution recovers it on
        # the next chat turn. Mirror that same resolution here, and 503 only when
        # there is truly nothing to recover.
