...
def _do_shutdown() -> None:
    ...
    # frees ALL resident VRAM, not just the chat engines. Same best-effort
    # try/except as the engine unloads above: shutdown must complete regardless.
    try:
        from localm.inference import embedder as _embedder_mod
...
def _do_restart(*, update_watchdog: Optional[dict] = None) -> None:
    ...
    *update_watchdog*, when given (only by the post-update restart path - see
    ...
    # best-effort try/except as the engine unloads above: restart must proceed
    # regardless.
    try:
        from localm.inference import embedder as _embedder_mod
        _embedder_mod.reset_embedder()
