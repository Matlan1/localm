...
def check_grammar_structure(grammar: str) -> None:
    """Reject a grammar whose size or structural complexity could drive the native GBNF parser into stack overflow, BEFORE any of it reaches that parser."""
...
class _WaiterGate:
    """How many callers are QUEUEING for a probe slot right now, and a non-blocking way to join them."""
    ...
    def try_enter(self, limit: int) -> bool:
        """Take a waiter permit, or return False immediately."""
        with self._lock:
...
def _spawn_trigger_probe_daemon():
    """Start the long-lived trigger-pattern-probe daemon."""
    import subprocess
...
def _readline_with_timeout(stream, timeout: float):
    """stream.readline() with a timeout (Windows pipes offer no select()-style wait, so the read runs in a daemon thread and this waits on it via a queue - same pattern as backends/llamacpp/_loader.py's helper of the same name, reimplemented here rather than imported across a private backend-internal modul..."""
    import queue
...
def _slot_kill_and_prewarm(slot: "_ProbeSlot") -> None:
    """Best-effort kill of *slot*'s dead/hung daemon, then hand off to a BACKGROUND thread to pre-spawn its replacement, so the respawn cost usually lands on nobody's request instead of landing on whichever caller happens to check this slot out next."""
    with slot.lock:
...
def _static_shape_rejection(pattern: str) -> "str | None":
    """Cheap, in-process, microsecond-cost rejection of KNOWN catastrophic- backtracking shapes, run BEFORE the expensive daemon probe."""
...
def _probe_pattern_is_safe(pattern: str) -> "tuple[str, str]":
    """(verdict, reason) where verdict is _PROBE_SAFE / _PROBE_UNSAFE / _PROBE_UNDETERMINED."""
...
def _probe_on_slot(slot: "_ProbeSlot", pattern: str) -> "tuple[str, str]":
    """One probe round trip on a slot the caller OWNS."""
    first_spawn = False
...
def validate_trigger_patterns(patterns: "list[str]") -> None:
    """Reject a caller-supplied lazy-grammar trigger pattern list that is invalid or unsafe to run, BEFORE any of it reaches llama_sampler_init_grammar_lazy_patterns - matching check_grammar_structure's contract for the grammar string itself (raises the same typed InvalidGrammarError so a caller can treat..."""
