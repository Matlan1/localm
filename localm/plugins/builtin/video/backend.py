...
def settings(full_config: dict) -> dict:
    ...
    # when the caller passes NOTHING. _comfy.default_api_url() itself resolves
    # to the managed instance's URL whenever own_active is True.
    ...
    # rather than only the debug log.
...
def _comfy_model_roles(s: dict, roles: list) -> dict:
    ...
    One ComfyUI round trip and one registry read per call, both blocking, so
...
def _comfy_generate(s: dict, prompt: str, out_path: Path, *,
    ...
    # through the env var its containment step resolves from. This also cleans
    # up the uploaded image-to-video source, since the input/ dir is located
    # relative to the resolved output dir.
