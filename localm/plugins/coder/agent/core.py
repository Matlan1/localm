...
class Agent(
    ...
    def __init__(
        ...
        **gen_kwargs,
        ...
        # Live attribute lookup, so a patched agent.load_memory /
        ...
        # parent's real channel.
        self._interactive   = False
        # Capture writes as a unified diff instead of touching disk. Settable
        # here or assigned afterwards (the CLI's --patch-mode carries a FILE path
        # and checks truthiness of its own option, not this flag).
        ...
        # this agent. Only an inherited scope follows the parent's later changes;
        # an explicit one is left alone. The inherited value is still copied
        # above, so a child stays confined even if the parent reference goes away.
        ...
        # at dispatch, so a minted scoped key cannot run them.
        ...
        # at construction. The narrowing itself is applied after tool
        # registration, in _apply_role_toolset, and is strictly subtractive.
        ...
        # CLI uses its own outer --until loop instead. A RESTRICTED session must
        # never set it: those sessions have no process execution at all.
        ...
        # token, reasoning, tool_call, tool_result, turn, info. None means
        # terminal-only display. "reasoning" is a thinking model's reasoning text,
        # kept separate from "token" - see _call_llm.
        ...
        # fresh per Agent so two sessions in the same project never write the same
        # file, then overwritten by load_checkpoint() the moment this agent
        ...
        # REPL's /bg, and the GUI's per-session background list). The job registry
        # is process-wide, so this is the only thing that tells one session's
        # background work from another's in a single GUI server process.
        #
        # INHERITED from the parent, so a spawned sub-agent's background work
        # still belongs to the session that spawned it.
        #
        # Not _checkpoint_id: that one is overwritten by load_checkpoint() on a
        # resume, which would split one session's jobs across two owners.
        self.job_owner: str = (getattr(parent, "job_owner", None)
                               or uuid.uuid4().hex[:12])
        # The raw, pre-episodic-preamble text of this session's first task or
        # message - captured in loop.py's run_task/chat, restored by
        # resume_checkpoint(). save_checkpoint() turns it into a short display
        # title (checkpoint._derive_title) for a resume listing; kept RAW here.
        ...
        # unsupported). Never re-arms. Distinct from _force_tool_grammar, which is
        # a one-shot per-turn flag the escalation ladder sets and clears.
        ...
        # a stop). _loop re-arms it to True at the start of every run. The GUI
        # reports it per turn ("ok" on the final event) and the CLI turns it into
        # an exit code.
        ...
        # "the check could not run" is neither of _last_run_ok's two answers.
        # Consumers that want "was this verified" read this; _last_run_ok means
        # "did the run itself complete".
        self._last_verify_state: Optional[str] = None
        # Session-level: True once ANY run this session failed. The close-time
        # episodic reflection (session.py) reads this one, not the per-run
        # _last_run_ok.
        ...
        # circuit breaker). Both clear _last_run_ok, but only a genuine failure
        # triggers a close-time reflection.
        ...
        # 120s batch deadline usually outlives the whole turn too, so a per-call
        # list would be empty again on the next turn and a destructive tool would
        # launch straight into the still-running peer.
        ...
        # shows the cumulative change, not just the last edit.
        self._changed_files: dict[str, dict] = {}
        # Work done by ISOLATED children (their own git worktree), kept out of
        # _changed_files: those keys are relative to the writing agent's cwd and
        # session_diff re-resolves keys against ours. Surfaced as a separate
        # labelled section in the human-facing views only.
        self._delegated: list = []
        # Bounded trace of tool/command failures this session, fed into the
        # close-time episode reflection as what_failed. Newest kept, capped at
        # _MAX_ERROR_TRACE.
        ...
        # (git apply, formatters, codegen) that the write-tool tracker never
        # records are attributed to this session without pulling in a pre-existing
        # dirty tree. None until captured, and when cwd is not a git work tree.
        ...
        # The model's own task list (tools/tasks.py). Held here, not in _messages,
        # so compaction cannot summarise the plan away, and written into the
        # resume checkpoint so it survives pause/resume. The lock guards it
        # against the parallel non-destructive tool batch (loop.py).
        ...
        try:
            ...
            pass
        # Per-model harness profile: fill gen-kwarg defaults the caller did not
        # set; explicit caller values win. max_tokens is handled in the CLI
        # (see harness_profiles).
        ...
        # User-authored custom instructions: an explicit string (CLI --system)
        # overrides the .localcoder/system.md file; None means "read the file".
        # The override is kept so it survives a later set_cwd (which re-reads the
        # file from the new cwd). Distinct from project memory above.
        ...
        # system prompt is built so the model learns about them; each step warns
        ...
        # Same placement: a parent under an active skill's allowed-tools must not
        # be able to spawn a child that escapes it (see
        # _apply_inherited_skill_toolset).
        if self._inherited_skill_tools is not None:
            self._apply_inherited_skill_toolset()

        # After the toolset is final: warn once when --scope does not confine what
        ...
        # every later rebuild goes through the same helper, including the combined
        # mcp+plugin+skill tool docs.
    ...
    def _init_episodic_memory(self, cwd: Path) -> None:
        ...
        # restricted, shareable-key sessions. In privacy mode it is off too UNLESS
        # the user opted into read-only recall (memory_recall_in_privacy +
        # ..._coder): past lessons are RECALLED but the close-time write stays
        # blocked (session.py gates it on privacy). The store path resolves under
        # the localm home dir, never the project tree.
        self._episode_task: str = ""
        self._episode_store = None
        # Which past lessons this run recalled ({id, lesson, outcome}), and why
        # recall came back empty when it did, so injected text is traceable and
        # has an id to forget it by. The chat side keeps the same record in
    ...
    def _init_provenance(self) -> None:
        ...
        # tools as data-not-instructions and harden their boundary against
    ...
    def _init_reviewer(self) -> None:
        # Pre-done self-review: an optional reviewer model reads the diff before
        # the agent declares done and feeds blocking issues back. Off by default;
        # a network reviewer is gated off privacy/restricted (see
        # reviewer_for_agent).
    ...
    def _init_mcp_tools(self, cwd: Path) -> None:
        ...
        # system prompt is built so the model learns about them. Failures warn
        # and continue.
        self._mcp_docs: str = ""
        try:
    ...
    def _init_plugin_tools(self) -> None:
        print_warning = _agent.print_warning
        TOOL_REGISTRY = _agent.TOOL_REGISTRY
        # External plugin tools: register any tools exported by installed plugins,
        # the same way as MCP and before the prompt is built. External code
        # defaults to "destructive" (needs confirmation). Failures warn.
    ...
    def _apply_restricted_toolset(self) -> None:
        ...
        # disable every tool not in the allowlist (run_shell, run_tests,
        # git_commit/push, fetch_url, generate_image, read_env, spawn_agent, any
        # external tool). Default-deny: a newly-added tool is disabled here too.
    ...
    def _apply_role_toolset(self) -> None:
        ...
        # does not get their documentation either.
        self._mcp_docs = self._plugin_docs = self._skill_docs = ""
        self.disabled_tools = self.disabled_tools | (
    ...
    def _last_user_request(self, value: str) -> None:
        ...
        # getattr default: a subclass or a restored checkpoint could assign this
        # before __init__ has run its state block.
    ...
    def _emit(self, event_type: str, **data) -> None:
        ...
        try:
            ...
            # A broken sink must not kill the agent loop; log and continue rather
