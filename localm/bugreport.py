...
def collect_diagnostics(context: Optional[dict] = None) -> dict:
    """Gather a safe, useful environment snapshot."""
    ...
    try:
        ...
        # blanket "vulkan whenever any GPU is present" value and contradicts what
        # the installer would actually provision.
    ...
    # Guarded separately from the library listing above so one failing does not
    ...
    try:
        ...
        # Best-effort: the marker or config may be unreadable, or setup_llama may
        # fail to import on a broken install. Omitting these fields costs a
        # detail; raising would cost the whole report, on the failure path.
        pass
...
# The home/username policy lives in localm.pathscrub, so this scrubber and the
# API-response scrubber cannot drift apart. Replaces Path.home() with "~" in both
# separator forms, case-insensitively on Windows, and always applies the
# known-home-root username strip as a backstop.
#
# A report deliberately keeps the INSTALL dir, which pathscrub.scrub_paths would
# drop: the maintainer reading the report needs to know where the app is
...
def _scrub_url_creds(text: str) -> str:
    """Strip ``user:pass@`` credentials from any URL-ish value before it goes in a report (a SearXNG / ComfyUI / reviewer URL can carry an inline secret)."""
    if not text:
        return text
    return re.sub(r"(://)[^/@\s]+@", r"\1<redacted>@", text)


# Credentials are redacted by PARAMETER NAME, never by guessing the value's
# format: an opaque token has no reliable shape, but a parameter called
# api_key/token/secret is a name that can be trusted. The name is kept so the
# report still shows that the endpoint was credentialed and which endpoint it is.
#
# The name may sit anywhere a ``name=value`` pair can appear, not only after a
# ``?`` or ``&``. Hence three branches, all feeding ONE capture group so the
# substitution keeps the name and drops only the value:
#
#   1. after a literal ?/& - the historic set;
...
#
# The short generic names sit in branch 3 only: unprefixed they would eat an
# ordinary ``key=value`` line out of a pasted config dump, while omitting them
# entirely would leave ``SECRET_KEY=`` and ``x-auth=`` in the clear.
_QUERY_SECRET_RE = re.compile(
    ...
    # 3. Anywhere else, PREFIXED - matched from the SEPARATOR rather than from the
    #    prefix, so the pattern carries no repetition for a backtracking engine to
    #    walk. The prefix is never consumed and survives outside the match, so
    ...
    r")=)"
    # A short list of literal values that cannot be a secret are left alone, so
    # ``LOCALM_REQUIRE_AUTH=1``, ``require_auth=true`` and ``has_token=false``
    # survive - hiding whether auth was on removes the diagnostic a reader needs.
    #
    # The literal has to be the WHOLE value, so ``api_key=truesecret123`` and
    # ``api_key=10`` still redact. Closing markup may follow it, so
    # ``(require_auth=1)`` is still a flag, but anything after that markup is not:
    # ``api_key=1)SECRET`` stays redacted.
    r"(?![\"']?(?:true|false|none|null|nil|yes|no|on|off|enabled|disabled|[01])"
    r"[`\"'\)\]\}]{0,4}(?:[\s&#]|$))"
    # The value may be quoted, which is how a .env writes it more often than not.
    # An unterminated quote redacts to the next quote on the line, or to the end
    # of the line when there is none, and never across a newline.
    r"(?:\"[^\"\r\n]*\"?|'[^'\r\n]*'?|[^&\s#\"'\)\]\}]*)"
)

# The same credential can arrive as a pasted HTTP header line rather than a URL,
# e.g. "X-Api-Key: <value>". Kept to an explicit set of credential header names
# rather than a bearer-style catch-all, so an unrelated header is never touched.
...
def _scrub_query_and_header_secrets(text: str) -> str:
    """Redact credential-ish URL query parameters and HTTP header lines, by NAME (see the regexes above)."""
    if not text:
        return text
...
# Bearer tokens and API keys can appear in a pasted console error, a fetch log
...
def _scrub_secrets(text: str) -> str:
    """Run every scrubber over untrusted text: home paths (username), URL ``user:pass@`` credentials, credential-named query params / header lines, and bearer / API-key tokens."""
    if not text:
        return text
...
def _enabled_plugins() -> list:
    """Names of enabled engine plugins, from config."""
    try:
...
def _safe_config_subset() -> dict:
    """An allowlisted, scrubbed slice of the user config: enough to debug a setup problem (context sizing, ports, media URLs, privacy mode) without any secret."""
    try:
        from localm.config import load_config
        cfg = load_config()
    except Exception:
...
def _config_unreadable() -> bool:
    """True only when config.json (or its .bak) EXISTS and could not be parsed, in which case _safe_config_subset() is showing built-in defaults rather than the user's real settings - see config.load_config_checked."""
    try:
...
def _dependency_versions() -> dict:
    """Versions of the packages that shape a localm failure (native runtime, HF stack, server stack)."""
    out: dict = {}
    try:
...
def _format_error(error: Optional[BaseException]) -> str:
    ...
    # user-configured credentialed URL or a pasted token, and this traceback ships
    # in the uploaded "Error detail" body. _scrub_secrets adds the url-creds and
    # bearer/apikey strips the sibling log tail already applies.
...
def _find_run_log(home=None, pid=None):
    """The Path to a run's OWN log, matched by the pid embedded in the log filename (localm_<date>_<time>_<pid>.log), or None if none is found."""
...
# Why the log digest came back empty, when it did: no log file matched this run's
# pid, the file was found but could not be read, or the run genuinely logged
# nothing notable. The first two are failures to collect and the third is a clean
# result, so a caller attaches one of these and the report says which happened.
_LOG_UNAVAILABLE_NO_FILE = "no log file was found for that run"


def _log_failure_reason(exc: BaseException) -> str:
    """A one-line, PATH-FREE description of why reading the log failed, safe to put in a share-intended report."""
...
def _recent_log_tail_result(home=None, pid=None, max_chars: int = 6000) -> tuple:
    """``(digest, unavailable_reason)`` - the digest _recent_log_tail returns, plus WHY it is empty when it is empty."""
    try:
        ...
        # the report it is attached to. The caller is told, so an unreadable log
        # is not indistinguishable from a clean one.
...
def _recent_log_tail(home=None, pid=None, max_chars: int = 6000) -> str:
    """A digest of the crashed run's OWN log, matched by the pid embedded in the log filename (localm_<date>_<time>_<pid>.log): EVERY warning/error (with its full traceback) from the whole run, with runs of near-duplicate benign lines (e.g. routine ``GET /api/stats`` polling) collapsed to one line + a repe..."""
...
def _recent_hang_traces(home=None, max_chars: int = 8000, *, pid=None) -> str:
    """The event-loop hang trace captured by the run being reported (``<logs>/hang_<date>_<pid>.log``), if the always-on stall watchdog captured one - every thread's stack at the moment the server froze, which is exactly what diagnoses an intermittent 'it hung' report."""
    try:
...
def live_server_hang_trace(home=None) -> str:
    """The event-loop hang trace captured by a LIVE server of this install, for a user-initiated ``localm bug-report`` filed from a SEPARATE process (REG-736)."""
    try:
...
def _app_state_lines(diag: dict) -> list:
    """The 'what the app was doing' half of the report: loaded model, effective backend, session mode, debug state, and enabled plugins."""
    lines = []
...
def build_report(summary: str, reason: str = "",
    ...
    # The user-typed summary and reason are the only report fields not otherwise
    # scrubbed, and they ship in the uploaded body and, via the derived title, in
    # a public issue. Scrubbed at this choke point so every caller is covered.
    # _scrub_secrets is idempotent and no-ops on empty text.
    summary = _scrub_secrets(summary)
    reason = _scrub_secrets(reason)
    ctx = context or {}
    # A user-composed report can supply these three distinct fields via context,
    # the same threading pattern used for native_trace/recent_log_tail/hang_traces/
    # client below. Absent for every automatic report, which keeps their
    # "What happened" == summary+reason rendering unchanged. Scrubbed here, at the
    # same choke point as summary and reason above.
    ...
    if what_happened:
        ...
        # A user-composed report that answered "what I was doing" but skipped
        # "what happened". summary is derived from what_happened-or-description
        # upstream, so falling back to it would repeat what_i_did's first line as
        # a second section. Say the section was left blank instead.
        happened_body = "(not stated)"
    else:
        # No user-composed fields at all: an automatic crash or LocalmError
        # report, where summary and reason ARE the description of what happened.
    ...
    if config_subset:
        ...
        if diag.get("config_unreadable"):
            ...
            # defaults: a corrupt config.json would otherwise render identically
            # to a user with no config at all.
    ...
    # captured, and the crashed run's own log tail. Both are already home-scrubbed
    # at their source.
    ctx = context or {}
    ...
    if tail:
        ...
        # built path-free at its source and scrubbed again here.
...
def _report_footer() -> str:
    """The in-app disclaimer build_report() appends: true for a human reading the saved file or a downloaded copy (they genuinely can still edit it), false the instant the SAME text is what actually got sent - see _strip_report_footer(), the single place that fact is acted on."""
...
def _ring_activity() -> list:
    """The in-memory recent-activity buffer, or [] if unavailable."""
    try:
        ...
        try:
            ...
            if pre_log.exists():
                ...
                try:
                    ...
                    # into the report. A failed delete leaves the file on disk, so
...
def _client_lines(client: dict) -> list:
    """Render the GUI-supplied browser context (user agent, page, viewport, recent console errors) for the report."""
    lines = []
...
def save_report(text: str, when: Optional[str] = None) -> Optional[Path]:
    """Write the report to the data dir and return its path (or None on failure). *when* is a caller-supplied timestamp string (kept injectable for tests)."""
    try:
        from localm.config import home_dir
    ...
    try:
        ...
        try:
            ...
            # Best-effort hardening. chmod is a no-op on Windows and fails only on
            # POSIX filesystems without per-file perms, where the data dir's own
            # perms still apply. The report carries no secrets, so a world-readable
            # one leaks nothing sensitive.
            pass
        return path
    except Exception:
        return None
...
def report_title(summary: str, what_happened: str, description: str) -> str:
    """The final report/issue title: *summary* verbatim if the caller gave one explicitly, else derived from *what_happened* (a more useful issue title than 'what I was doing'), falling back to *description* - first line only, truncated to 120 chars, never empty."""
    if not summary:
...
# The GUI route offloads save_user_report() to a real worker thread, so two
# reports filed close together can run this function on two different threads at
# the same instant. Two hazards follow: _ring_activity() reads then deletes the
# shared pre_restart.log, and save_report() can open the same
# same-second-timestamped filename from two threads at once. This lock serialises
# both. Scoped to this function rather than shared with report_failure, which is
# the CLI/crash path and a separate process per invocation.
...
def save_user_report(description: str = "", *, summary: str = "",
    ...
    with _SAVE_REPORT_LOCK:
        ...
        # include_log: it exists only if the server actually froze. Scoped to our
        # own pid like the log tail above, so an old freeze from a previous run is
        # not presented as this report's diagnosis.
        import os as _os
        hang = _recent_hang_traces(pid=_os.getpid())
        # extra_hang_trace covers the ``localm bug-report`` CLI: a separate,
        # short-lived process whose own pid never froze, while the server that did
        # freeze is a different, still-running process. The CLI looks the live
        # server up itself and passes what it found in here.
        if extra_hang_trace:
...
def upload_config() -> tuple:
    """(url, token) for the upload channel from config, or (None, None) when not configured."""
    try:
        from localm.config import load_config
        cfg = load_config()
...
def upload_available() -> bool:
    """True when an upload endpoint is configured (the GUI uses this to decide whether to offer a 'Send to maintainer' button)."""
    return upload_config()[0] is not None


def _retry_after_from(headers, body_text) -> int:
    """Best-effort retry delay (seconds) for a 429: the ``Retry-After`` response header, else a ``retry_after`` field in the JSON body, else a 30s default."""
    val = None
    try:
...
def _classify_url_error(exc) -> tuple:
    """(stage, hint) diagnosing WHERE a failed upload broke, from the ACTUAL error only - we never contact a third-party host to test connectivity (offline-first + privacy). ``stage`` is offline_or_dns / tls / timeout / unreachable / unknown; ``hint`` is a friendly, actionable line (the caller adds the 're..."""
    import socket
    import ssl
...
def upload_report(title: str, body: str, *, url: Optional[str] = None,
                  ...
                  opener=None) -> dict:
    """POST a user-reviewed report to the upload endpoint and return its JSON response (e.g. ``{'url': '<issue url>'}``)."""
    import json as _json

    # Scrub the title at the upload boundary: it becomes a public GitHub issue
    # title, and callers pass the user's raw summary or description first line.
    # The single choke point every uploaded title flows through. Idempotent.
    title = _scrub_secrets(title)

    # Same for the body's trailing edit-disclaimer, which build_report()'s output
    # and the saved file both legitimately carry, and which stops being true at
    # this exact call. Stripped here, the one choke point every uploaded body
    # flows through.
...
def offer_to_send(summary: str, path: Optional[Path], text: str, *,
    ...
    """Offer to send an ALREADY-BUILT, already-saved report (upload / email / self), or send it immediately with ``auto_send``."""
...
def _handle_thread_exception(args) -> None:
    """threading.excepthook: a crash in a background thread used to vanish."""
    ...
    try:
        ...
        try:
            ...
            # The bug reporter itself failed AND the stdlib default hook failed.
            # An excepthook must not raise, which would mask the original crash.
            pass


def install_global_handlers(force: bool = False) -> bool:
    """Install the process-wide graceful-failure net (main thread + background threads)."""
...
def install_asyncio_handler(loop) -> bool:
    """Route an uncaught asyncio task/loop exception through the bug reporter (saved, never prompted - a server loop has no user at the keyboard)."""
    import asyncio
    try:
        def _h(loop, ctx):
...
# instance_id, the same per-process identity instances.py's registry uses.
# Running more than one localm server against the same LOCALM_HOME is supported,
# and an unscoped marker would let a second instance read the first's still-armed
# marker as a hard death, and let its own clean-shutdown disarm delete a
# third, still-live instance's marker.
...
def _trace_path_for_marker(d, marker):
    """The native-fault-trace file that belongs WITH *marker* (same instance), derived from the marker's own filename rather than a second parameter, so a caller iterating markers it did not itself name (check_and_report_prior_crash) never has to keep the two paths in sync by hand."""
    name = marker.name
...
def _all_crash_markers(d):
    """Every armed marker under *d*: one per instance that has ever armed against this LOCALM_HOME (plus the legacy unscoped name, if present)."""
...
def arm_crash_guard(context: Optional[dict] = None, home=None,
    ...
    """Mark that a server run is in progress and enable faulthandler so a native fault leaves a trace."""
    ...
    try:
        ...
        try:
            ...
            if not faulthandler.is_enabled():
                # enable() can return without raising yet still not be armed on
                # some platforms and file-object shapes; is_enabled() is the call
            ...
            # written, so a hard death is still reported next start, just without
            # the native traceback. Logged so an empty trace file is diagnosable.
            logger.warning(
...
def disarm_crash_guard(home=None, instance_id: Optional[str] = None) -> None:
    """Clean shutdown: drop THIS instance's own marker (never a sibling's) so the next start does not report a crash. *instance_id* must be the SAME id passed to the matching arm_crash_guard() call - see the module note above for why an unscoped delete is unsafe when more than one instance shares a LOCALM_..."""
    global _crash_trace_fh
    import faulthandler
    try:
        d = _crash_dir(home)
...
def _raw_tail_truncation_signal(home=None, pid=None) -> "tuple[bool, str]":
    """(truncated, last_line): whether the crashed run's OWN raw log file (see _find_run_log - the SAME file _recent_log_tail digests, read raw here instead) ends WITHOUT a trailing newline, and what that final (possibly mid-word) line is."""
    try:
        chosen = _find_run_log(home, pid)
        if chosen is None:
            return False, ""
...
def _classify_prior_death(*, native_trace: str, hang_trace: str,
    ...
    """(summary, reason) for report_failure(), classified from evidence actually collected - never the old blind 3-way guess when something more specific is available."""
...
def _report_one_crash_marker(d, marker, home, interactive: bool):
    """Report *marker* as a crash IF its recorded pid is no longer alive, then clear it."""
    import json
...
def check_and_report_prior_crash(home=None, interactive: bool = False):
    """Scan every crash marker left under this LOCALM_HOME's run/ dir (one per instance that has ever armed here) and report+clear any whose recorded pid is no longer alive - a hard crash: a native fault, an OS kill, or a force-closed window."""
    try:
        d = _crash_dir(home)
