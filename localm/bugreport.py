...
There is NO "open a GitHub issue in your browser" channel. Anyone who wants to
...
local debugging with --debug possible at all. That content must never reach a
REPORT: the digest builder (localm/_log_digest.py) drops every record from a
known debug_content_enabled()-gated write site (the raw model reply, a
memory-embed content snippet, a web-tool query) before it is ever collapsed or
classified, so none of them can appear in ``_recent_log_tail`` regardless of
...
def collect_diagnostics(context: Optional[dict] = None) -> dict:
    ...
    try:
        ...
        # blanket "vulkan whenever any GPU is present" value, which contradicts
        # the installer's actual policy (cuda for an NVIDIA-on-Windows box).
        # updater._installed_backend() and release_verify._default_backend() call
        # recommended_install_backend() too; det.recommended has no
        # known-correct caller left, only its own construction/contract tests.
    ...
    # cost the other. Recorded at provision time by setup_llama, so it is a
    # lookup, not a probe.
    try:
...
# drift apart. It replaces Path.home() with "~" in both separator forms
# (case-insensitively on Windows), and ALWAYS applies the known-home-root
# username strip as a backstop so a path that is not exactly Path.home() - a
# different account - is caught too.
#
# A report KEEPS the INSTALL dir (pathscrub.scrub_paths would drop it): the
# maintainer reading the report needs to know where the app is installed.
...
# is a name that can be trusted regardless of what it holds. The parameter NAME
# is kept so the report still shows THAT the endpoint was credentialed and which
# endpoint it is; only the value goes. Do not widen this by adding more
# value-shape regexes (sk-..., 24-char-opaque, ...): the name is the durable
# signal.
#
# The name may sit ANYWHERE a ``name=value`` pair can appear, not only right
# after a ``?`` or ``&``: a pasted .env fragment or shell line writes
# ``api_key=...`` at the start of a line, and a prefixed name
...
#
#   1. after a literal ?/& - the base set, so a bare ``?key=`` or ``?sig=`` in
#      a URL redacts;
...
#      prefix.
#
# The short generic names belong in branch 3 only. In branch 2 they would eat
# an ordinary ``key=value`` line out of a pasted config dump and an
# ``auth=none`` out of a log; absent from branch 3 they would leave
# ``SECRET_KEY=``, ``PRIVATE_KEY=``, ``APP_KEY=``, ``x-auth=`` and ``req_sig=``
# in the clear. With them in branch 3, ``SECRET_KEY=`` redacts while ``key=``
# and ``monkey=`` do not.
_QUERY_SECRET_RE = re.compile(
    ...
    #    short generic names, as described above.
    ...
    # the value-shape rule forbidden above: it only declines to redact a short
    # list of literals no credential is ever equal to. Without it the name-based
    # match eats ``LOCALM_REQUIRE_AUTH=1``, ``require_auth=true``,
    # ``digital_signature=True`` and ``has_token=false`` out of a report, hiding
    # whether auth was ON.
    #
    ...
    # are the same flag. Something else after that markup is NOT a flag, so
    ...
    # than not, so the match runs past the opening quote to the closing one. An
    # unterminated quote redacts to the NEXT quote on the line, or to the end of
    # the line when there is none, and never across a newline. Both land on
    # over-redaction, the safe direction for a line that opened with a
    # credential, but they are different spans.
    r"(?:\"[^\"\r\n]*\"?|'[^'\r\n]*'?|[^&\s#\"'\)\]\}]*)"
)
...
def _config_unreadable() -> bool:
    ...
    _safe_config_subset, which stays on load_config() and discards the signal
    this needs: a corrupt config must not render identically to no config at
    all. Never raises."""
    try:
...
def _format_error(error: Optional[BaseException]) -> str:
    ...
    # strips the sibling log-tail already applies.
...
# Why the log digest came back empty, when it did. An empty digest can mean
# THREE unrelated things - no log file matched this run's pid, the file was
# found but could not be read, or the run genuinely logged nothing notable. The
# first two are FAILURES to collect and the third is a clean result, so these
# are the reasons a caller can attach and the report SAYS which happened.
...
def _log_failure_reason(exc: BaseException) -> str:
    ...
    The obvious formattings leak the user's home directory, and therefore
    their username:
    ...
    So the text is built from the exception CLASS NAME plus ``strerror`` - the
    OS's own errno text ("Permission denied"), which carries no path and is
    absent on non-OSError, where the class name alone is all that is said. A
    detail containing a path separator is dropped outright rather than trusted,
    which makes a leak UNREPRESENTABLE. _scrub_secrets is a further backstop for
    the home-path form.
    """
...
def _recent_log_tail_result(home=None, pid=None, max_chars: int = 6000) -> tuple:
    ...
    try:
        ...
        # a debug_content_enabled() write with no header of its own, so
        ...
        # Swallowed - collecting a log is best effort and must never break the
        # report it is attached to - but the caller is TOLD, so an unreadable
        # log is not indistinguishable from a clean one.
...
def _recent_log_tail(home=None, pid=None, max_chars: int = 6000) -> str:
    ...
    count - see localm/_log_digest.py. Unlike a last-N-lines tail it keeps the
    failure however much routine activity followed it. Chat content (a raw model
    reply, a memory-embed snippet, a web-tool query) is dropped by build_digest
    upstream of this, whatever the content itself says. Home paths are
...
# whatever pid it carries (pids are reused). Generous: the pid match below is
# the precise filter, and this only backstops a pid collision.
...
def build_report(summary: str, reason: str = "",
    ...
    # this choke point so every caller (report_failure, save_user_report) is
    # covered. _scrub_secrets is idempotent and no-ops on empty text.
    ...
    # for every automatic (crash/LocalmError) report, which keeps their "What
    # happened" == summary+reason rendering. Scrubbed here, at the same choke
    # point as summary/reason above.
    ...
    if what_happened:
        ...
        # was doing" but skipped "what happened". summary is DERIVED from
        ...
        # section. Say plainly that this section was left blank instead of
        ...
        # and this is the report's ONLY account of it.
    ...
    # have no Python error object), without which the report is contentless: a
    # faulthandler native trace when one was captured, and the crashed run's own
    # log tail (the actionable bit when a window-close/OS-kill left no trace).
    # Both are already home-scrubbed at their source.
    ctx = context or {}
    ...
    if tail:
        ...
        # The reason is built path-free at its source - see _log_failure_reason -
        # and scrubbed again here like every other context-supplied string.
        ...
        try:
            ...
            if pre_log.exists():
                ...
                try:
                    ...
                    # file PERSISTS on disk, so the failure is surfaced rather
    ...
    *when* is a caller-supplied timestamp string (kept injectable for tests)."""
    ...
    try:
        ...
        try:
            ...
            # network mount), where the data dir's own perms still apply. The
            # report carries NO secrets (no API key, env, config secrets, or chat
            # content - see module docstring), so a world-readable report leaks
            # nothing sensitive.
            pass
        return path
    except Exception:
...
def report_title(summary: str, what_happened: str, description: str) -> str:
    ...
    with logic that could disagree with what the report itself says."""
    if not summary:
...
# separate process per invocation.
...
def save_user_report(description: str = "", *, summary: str = "",
    ...
    three DISTINCT fields, not one string echoed three times. *summary* overrides the
    ...
    *description* when that too is empty), since "what happened" makes a more
    ...
    with _SAVE_REPORT_LOCK:
        ...
        # our own pid like the log tail above: an old freeze from a previous run
        # is not this report's problem.
        import os as _os
        hang = _recent_hang_traces(pid=_os.getpid())
        # extra_hang_trace covers the ``localm bug-report`` CLI's own case: it
        # is a separate, short-lived process whose OWN pid never froze, while the
...
def upload_report(title: str, body: str, *, url: Optional[str] = None,
    ...
    reported as success. *opener* is injectable for
    ...
    # every uploaded title flows through, so scrubbing here covers all callers.
    # Idempotent; no-ops on empty text.
    ...
    # PUBLIC GitHub issue regardless of which caller forgot.
        ...
        if action == "upload":
            ...
            while True:
                ...
                try:
                    ...
                    # failed, keep the file, and offer to retry creating the issue.
                    console.print(f"[yellow]Could not send it.[/yellow] {e.hint or e.reason}")
...
#                                                                              #
#  Bugs are not confined to setup. These hooks route every Python-level       #
#  failure - a crash in a background thread (model preload, the jobs runner,   #
#  the coder loop), or an uncaught main-thread exception - through the same    #
#  "sorry X for Y" + report path.                                              #
#                                                                              #
...
def _handle_thread_exception(args) -> None:
    """threading.excepthook: a crash in a background thread would otherwise
    vanish. Log it and save a report (never prompt - a worker thread has no user
    attention), so a failure is visible without taking the whole app down."""
...
# native-fault-trace file, is therefore scoped per instance_id - the same
# per-process identity instances.py's own registry uses
# (``run/<instance_id>.json``) - so each running instance only ever arms,
# reports, and disarms its OWN file. One unscoped file per home would let a
# second instance read the FIRST instance's still-armed marker as "the previous
# run died hard", and its own later clean-shutdown disarm would then delete
# whatever marker existed by that point.
...
def arm_crash_guard(context: Optional[dict] = None, home=None,
    ...
    (``app.state.instance_id``, set by instances.advertise() ahead of this
    call) scopes the marker to THIS running instance so a sibling instance
    ...
    try:
        ...
        try:
            ...
            if not faulthandler.is_enabled():
                ...
                # raised". A failure to arm is surfaced rather than silent, so
                # an empty native-trace file is diagnosable.
                logger.warning(
            ...
            # without the native traceback. Logged rather than swallowed, so an
            # empty trace file is diagnosable.
            logger.warning(
...
# because this only needs the last line, not a digest.
...
def _classify_prior_death(*, native_trace: str, hang_trace: str,
    ...
    or force-closed window leaves no trace at all)."""
    ...
    try:
        ...
        try:
            if pid_alive(int(pid)):
                ...
                # that is simply still up, not a crash. Leave
        ...
        # would accumulate one per instance that has EVER armed, permanently.
        # Best-effort, same as the marker unlink above - a failure here must not
        # block or duplicate the report.
        try:
        ...
        if tail:
            ...
            # A crash report with no native trace and no log is contentless;
            # saying WHY the log is missing is the difference between an
            # actionable report and a dead end.
        ...
        # naming all three possibilities ("a native crash, an OS kill, or a
        # force-closed window") without saying which.
