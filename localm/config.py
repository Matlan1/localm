...
def _warn_unconfigured_home(path: Path) -> None:
    ...
    is VISIBLE instead of silently masked. stderr, not the logger: this runs at
    import time before logging is wired."""
    # LOCALM_SETUP=1 suppresses this warning; setup.bat / setup.sh set it while
    # they run setup-llama before the data dir is chosen. It is the only
    # suppressor, and it must never be set outside those scripts, where it would
    # mask a real lost-config warning at runtime.
...
def _detect_home() -> Path:
    ...
    Priority:
      ...
      3. Contained fallback: ``./home`` INSIDE the install, with a stderr
         warning, when NOTHING is configured (setup never ran, or the
         config/marker was lost). Never a shared ~/.localm outside the install.
    """
    ...
    if (repo_root / "pyproject.toml").is_file():       # source checkout only
        ...
        if cfg_marker.is_file():
            try:
                ...
                # the default data dir. stderr, not the logger - this runs at
                # import time before logging is wired, and importing debuglog
    ...
    # Default to the instance's OWN root (a contained ./home) and say so on
    # stderr; never a shared ~/.localm outside the install.
...
def cache_dir() -> Path:
    ...
    cache while provisioning the managed ComfyUI venv, the Whisper STT model - goes
    INSIDE the data dir rather than the per-user location those tools default to
    (under ``%LOCALAPPDATA%`` on Windows, ``~/.cache`` on POSIX).

    Derived from ``home_dir()``, so the cache follows LOCALM_HOME. NOT conditional
    on an ambient ``PIP_CACHE_DIR`` / ``HF_HUB_CACHE``: LOCALM_HOME is the only
    knob."""
...
def uv_cache_dir() -> Path:
    ...
    ``%LOCALAPPDATA%\\uv\\cache`` on Windows / ``~/.cache/uv`` on POSIX), and
    localm's installers try ``uv pip install`` BEFORE falling back to
    ``python -m pip``, so both are pinned. Same location root as pip's
    (``cache_dir()``), a sibling subdir since the two tools' cache formats
    differ."""
    return cache_dir() / "uv"
...
def contained_pip_env(base: Optional[dict] = None) -> dict:
    ...
    *base* defaults to a copy of the current process environment. localm's package
    ...
    and ``python -m pip install`` second, so BOTH ``PIP_CACHE_DIR`` and
    ``UV_CACHE_DIR`` are set here, OVERRIDING any ambient value. Callers pass the
    returned dict as ``subprocess``'s ``env=``; a subprocess that runs neither tool
    simply ignores the two extra vars."""
...
DEFAULT_CONFIG: dict = {
    ...
    # newest LAST: [{"backend": ..., "tag": ..., "at": <epoch>}, ...]. Read by
    # `setup-llama --rollback`, so a user need not remember a tag number. Capped
    # (see setup_llama._RUNTIME_HISTORY_MAX). Kept here rather than beside the
    # venv because LOCALM_HOME outlives the venv the runtime is installed into.
    ...
    #
    # A VRAM-FOOTPRINT dial, not a speed-up: at MATCHED VRAM it is
    # throughput-neutral, and what it buys is reaching a given speed in far
    # less VRAM, which matters when something else wants the card. Only
    # affects Mixture-of-Experts models; a dense model has no expert tensors
    # to move, so any value is a no-op there.
    "n_cpu_moe": 0,
    ...
    # refusing outright. It funds the KV cache's compute buffers and llama.cpp's
    # graph/scratch allocations, real memory the native loader needs. Lowering
    # it risks a native crash or a GPU driver hang (TDR) instead of a slower but
    # working load; only lower it for a model/context combination confirmed to
    # need less.
    "vram_overhead_mb": 1500,
    # Ceiling (seconds) for a GGUF model load in its isolated worker process
    # (see llamacpp/_runner.py) before it is treated as hung and killed. Raise
    # it only if a genuinely huge model on slow storage needs longer.
    "gguf_load_timeout_s": 900.0,
    # How long a reply may take to produce its FIRST token before the worker is
    # treated as hung. Covers prompt PREFILL, not one token's decode: on CPU,
    # under heavy partial offload, or with a very long prompt, prefill can
    # legitimately take minutes. Raise it only if a genuinely slow box needs
    # longer.
    ...
    # treated as hung and killed. HF loads read full-precision safetensors from
    # disk (no quantized mmap fast path), so this can legitimately need longer
    # than a GGUF load of similar size.
    "hf_load_timeout_s": 900.0,
    # How long an HF reply may take to produce its first token before the
    # worker is treated as hung. Covers prompt prefill, not one token's decode.
    # HF's CPU path is always dense full-precision (never quantized the way
    # GGUF's ggml kernels are), so this is unvalidated for very slow CPU-only
    # HF inference - raise it if a genuinely slow box needs longer.
    ...
    # quantizes. hf_embed_max_texts/hf_embed_max_chars below reject an
    # oversized request outright; this timeout bounds whatever is allowed
    # through.
    ...
    # None/empty/1 entry = off (single-GPU placement via main_gpu_index). A
    # device no longer detected at load time is dropped with a logged warning,
    # not trusted blindly (see discover.resolve_gpu_split).
    ...
    # share is proportional to its free VRAM at load time
    ...
    "top_k": 40,
    "repeat_penalty": 1.1,
    # Generation budget per reply: a runaway guard, not a cost control.
    # Thinking models (qwen3, deepseek-r1) spend most of it reasoning.
    "max_tokens": 4096,
    ...
    # browser tab even when the extra IS installed - an explicit opt-out (e.g.
    # to keep browser bookmarks/extensions). There is no "always window" value:
    # no setting may defeat run_native_window's fallback-on-failure.
    ...
    # --insecure override has NO config form, so an unauthenticated network bind
    # can only ever be caused by an operator typing it in a terminal, never by a
    # config write.
    "bind_host": "",
    ...
    # (default) treats a mapped drive exactly like a local one
    # (pathsafe.is_unc_or_device_path does not flag it, since "Z:" is
    # syntactically an ordinary local drive). Not a security floor - an
    ...
    # forces CPU offload on a VRAM-limited card. True (default) auto-corrects;
    # False submits your dequant choice verbatim.
    ...
    # (`localm comfy setup`); inert until then. "user" = always use the user's
    # own ComfyUI (comfy_workdir / comfy_api_url), even if a managed instance
    # exists. With no managed instance, both behave identically. This key only
    # routes; it never provisions and never modifies the user's own ComfyUI.
    ...
    # on the preferred one. OFF by default even on a multi-GPU box: the
    # second-card behaviour is UNPROVEN on real multi-GPU hardware, and
    # ComfyUI's gpu:N is a POSITION in a reordered visible list, so an
    # off-by-one would land a component on the wrong card and STILL RENDER (a
    # silent wrong result, not a crash). On one card, or an older ComfyUI, or
    # with the toggle off, media generation stays on a single card.
    ...
    # Q8) shares one card. 0 forces CPU; 99 forces full GPU offload regardless.
    # When unset, an explicit global n_gpu_layers other than 99 is inherited.
    ...
    # resolve_pooling_setting/_effective_pooling apply a per-model default -
    # MEAN for the bundled bge/nomic choices, which matches every existing
    # index they built, but LAST for a model that DECLARES last-token pooling
    # (a decoder-based embedder such as Qwen3-Embedding, gte-Qwen2). An explicit
    # choice here ("mean", "last", "cls", "none", or "auto" to always follow
    # whatever the GGUF declares) always overrides that default. Changing the
    # EFFECTIVE pooling invalidates already-embedded RAG collections and memory
    # vectors (same dimensions, different meaning), so re-index after changing it.
    ...
    # depth: it blocks nothing, it only labels.
    ...
    # soft-degrade). A config.json that stores False keeps it (saved wins);
    ...
    # OFF by default: a rendered remote image is the standard model-driven
    # exfiltration channel (the URL itself carries the payload), and turning
    # this on does not close that channel, it only makes the request come from
    # this machine's server rather than from the browser. With it on, the remote
    # host never sees the browser's IP, User-Agent or referrer, and the fetch is
    # subject to the same SSRF guard and domain lists as every other outbound
    # request.
    ...
    # `localm plugin setup` records the choice here. Read via .get(..., True),
    # so an absent key follows the default.
    ...
    # (Cloudflare rate limiting is the real control), can ONLY file an issue
    # (never read the repo), and is rotatable at the Worker.
    ...
    # and settings_schema.py's update_ignore_net_policy, admin_only). Defaults
    # to False: the update check obeys net_mode like everything else unless an
    # admin opts this one channel out.
...
def port_in_use(port: int, host: str = "127.0.0.1") -> bool:
    ...
    Resolves *host* first instead of assuming an address family, so an IPv6 host
    works. A name that resolves to several addresses (``localhost`` on a
    dual-stack box is both ``::1`` and ``127.0.0.1``) is in use when ANY of them
    answers: reporting a busy port free would hand the bind a collision, while
    reporting a free port busy only moves us to the next one in the range.

    An unresolvable host returns False rather than raising: the caller goes on to
    bind it, and the bind produces the accurate error naming the real problem. A
    config-driven bind has already been screened by ``cli._bind_preflight_error``
    before reaching here. The resolution failure is logged.
    """
    try:
    ...
    for family, socktype, proto, _canon, sockaddr in infos:
        try:
            ...
            # claiming a conflict we did not observe.
            continue
    return False
...
class PortInUseError(RuntimeError):
    ...
    ``pick_port`` raises this instead of binding a different port. The default (no
...
def _mkdir_or_explain(path: Path, *, is_home: bool) -> None:
    ...
    ``parents=True`` creates the whole path, so an explicit ``LOCALM_HOME`` a
    level or two below a not-yet-existing folder is created like ``mkdir -p``
    rather than crashing with WinError 3.
    ...
    ``click.ClickException``, which the CLI's cross-cutting handler passes
    straight through (a clean "Error: ..." line, exit 1) instead of routing to
...
def restrict_file_perms(path: Path, *, mode: int = 0o600) -> bool:
    ...
    *mode* defaults to 0o600 for a FILE. A caller locking down a DIRECTORY must
    ...
    The return value lets a caller doing the atomic temp+replace dance restrict
    the TEMP file (which already holds the whole payload) and skip a second call
    on the destination in the happy path: on Windows os.replace carries the
    source's ACL onto the destination, overwriting the destination's inherited
    one, and POSIX rename likewise carries the source's mode. Callers should
    still retry on the destination when this returns False, so a failed first
    attempt is not the single point of failure for a security property.
    """
    try:
        if os.name == "posix":
            os.chmod(path, mode)
            ...
            if r.returncode != 0:
                ...
                # and returned as False rather than read as a success.
        ...
        # without per-file perms) is reported at debug level and returned as
        # False rather than raised, so a caller can retry and nothing reads as a
        # success that did not happen. The home-dir scoping stays in effect.
...
def atomic_write_private(path: Path, text: str) -> bool:
    ...
    * ``os.open`` with an explicit 0600, so the file is never briefly at the
      umask default. POSIX-only: on Windows the mode argument writes no ACL at
      all.
    * :func:`restrict_file_perms` on the TEMP file before the rename covers
      ...
      retried only when the first attempt failed, so one failure is not the
      single point of failure for the property.
    ...
    The RETURN VALUE is the pre-rename ``ok``, passed through so a caller that
    logs its own subsystem-named warning on failure
    (``gpu_registry.write_entry``) keeps that signal.
    ...
    transient-sharing-violation retry sleeps up to ~1 s, and two of these callers
    (``sessions._save``, reached from session_login, create_key_ep and
    _gui_index) run ON THE ASYNCIO EVENT LOOP.

    ``os.O_BINARY`` must NOT be added. Without it ``os.open`` opens in TEXT mode
    on Windows, so ``os.write`` expands ``\\n`` to ``\\r\\n`` exactly as
    ``Path.write_text`` does; adding O_BINARY would switch every Windows
    install's credential and registry files to LF on the next write.
    ...
    count rather than raising), so a short write raises instead of silently
    truncating. Raising leaves the temp file behind and the destination
    untouched."""
...
# every launch. The helpers below make every write atomic (write a temp file in
# the same dir, fsync, then os.replace - readers see only the old or the new
# complete file, never a torn one) and make every read crash-proof (fall back to
# the .bak snapshot, then to the default).
...
def _is_transient_permission_error(e: OSError) -> bool:
    ...
    layer, so Windows keeps retrying that too.

    POSIX always returns False: open() raises EACCES/EPERM only when the mode or
    ACL genuinely denies us (a mode-000 or root-owned file left by an earlier
    sudo run), a stable state the retry can never clear, and the retry loop holds
    _io_lock, which serializes config access process-wide including the
    per-request auth path (auth.require_auth -> load_config). The failure is
    still surfaced by the caller, it is just not retried."""
    if os.name != "nt":
        return False
    # winerror is absent only if this was not raised by the Windows layer at all
    # (e.g. a test double); that counts as transient.
...
def _replace_atomic(src: Path, dst: Path) -> None:
    ...
    after the last attempt. On POSIX os.replace does not hit this, so the loop
    succeeds first try; if it does raise there it is a STABLE denial (see
    _is_transient_permission_error), re-raised at once rather than stalling ~1s
    under _io_lock AND the cross-process lock for a retry that cannot
    succeed."""
...
def _atomic_write_json(path: Path, data) -> None:
    ...
    The temp file MUST keep its UNIQUE per-write name (mkstemp), never a fixed
    ``<name>.tmp``. Two localm processes writing the same file concurrently (a CLI
    ``pull``/``config`` alongside the running GUI, or two CLI invocations) would
    otherwise both open the SAME ``<name>.tmp`` and collide - one save then crashes
    when the other's replace has already consumed the shared temp (Windows WinError
    2/32), or on POSIX their writes interleave into a torn temp. ``_io_lock`` only
    serialises writers WITHIN one process; _replace_atomic's retry rides out a
    locked DESTINATION but not two writers sharing one temp SOURCE. With a unique
    temp per writer, cross-process contention degrades to last-writer-wins on the
    final file, with no crash or torn write."""
    ...
    try:
        ...
        if path.exists():
            try:
                ...
                # The .bak snapshot is best-effort: a failure here is noted on
                # stderr and the primary write still proceeds below.
                # _replace_atomic already rode out the transient sharing
                # violation, so reaching here means a PERSISTENT problem (a lock
                # that outlasted the retries, disk full, a real permission error).
...
def _read_json_checked(path: Path, default):
    ...
    read-modify-write MUST NOT, because writing the default back replaces every
    setting the user had - see update_config / update_registry, which refuse on
    ``read_ok`` False.
    ...
    for candidate in (path, path.with_name(path.name + ".bak")):
        ...
        for attempt in range(_REPLACE_RETRIES):
            try:
                ...
                # can never succeed, and this loop runs under _io_lock, so the
                ...
                # immediately without wasting the retry budget.
                print(f"[localm] {candidate.name} is unreadable ({e}); "
                      "falling back.", file=sys.stderr)
            break
...
def instance_id() -> str:
    ...
    data directory is actually configured.

    Browser localStorage is scoped by ORIGIN (protocol+host+port) only, never by
    which backend DATA DIRECTORY is running behind it, and localm's server binds
    to a fixed default port that only changes when it is already busy - so a
    fresh install opened after a prior instance closed typically reuses the SAME
    origin, and therefore the SAME localStorage bucket, as an unrelated data
    directory. The GUI fetches this id from /v1/config and compares it against
    the id it last saw for this browser origin, so it can tell "a normal restart
    of the same install" apart from "a different install that happens to share
    this browser" and never render, merge, or re-upload the other install's
    cached conversation history into this one's own data directory."""
    ensure_dirs()
    ...
    with _io_lock:
        ...
        try:
            ...
            # client treats it as a new pairing and starts empty / re-syncs from
            # the server).
...
def _merge_stored_config(cfg: dict, stored) -> None:
    ...
    and that discard is warned about once per process. A MISSING file arrives
    here as the ``{}`` default (a dict) and is a normal no-op, not a warning; a
...
def _user_delta(cfg: dict) -> dict:
    ...
    DEFAULT_CONFIG value reach existing installs: an absent key means "follow
    the default" and load_config() reconstructs it at read time.
    ...
    Two documented consequences of delta persistence:
      ...
      - A stored value that DIFFERS from the current default cannot be told
        apart from a deliberate user choice, so it is KEPT rather than
        silently changed.
    """
...
# built on os.open(..., O_CREAT | O_EXCL) - an atomic create-only-if-absent op
# on both Windows and POSIX, so exactly one process at a time can hold the lock.
...
# This - not the pid recorded in the file - is what identifies a lock as ours: a
# pid NUMBER is not an identity, since the OS reuses pids freely across process
# lifetimes and a lock LEAKED by a crashed holder can carry the very pid the OS
# later hands to a new localm process. The uuid4 nonce in each token is exact -
# a leaked file can never match a token we are holding right now, whatever pid
# it records.
...
def _cross_process_lock(target: Path):
    ...
    (_CROSS_LOCK_TIMEOUT) rather than a fixed attempt count, since a full
    read-modify-write held by another process can legitimately take longer than a
    single os.replace. Timing out raises TimeoutError rather than proceeding
    unprotected.

    FENCING TOKEN: each acquisition writes a unique ``<pid>:<nonce>`` token into
    the lock file, not just a bare marker. Release only removes the file if it
    still holds the token THIS call wrote; if it does not (this call's lock was
    reclaimed as stale while still legitimately held - a critical section that
    outlasts _CROSS_LOCK_STALE_AGE without having crashed), the file is left
    alone. Whoever's token is on disk owns cleanup, so a stale-reclaim can never
    cascade into deleting a live holder's lock and letting a third writer in.
    ...
    here). Ownership is decided by that exact token, NEVER by the pid recorded in
    ...
    while True:
        ...
        # leaks an orphaned, unowned lock file that blocks every other
...
def update_config(mutator: Callable[[dict], None]) -> dict:
    ...
    with _io_lock, _cross_process_lock(CONFIG_FILE):
        ...
        if not read_ok:
            ...
            # defaults while the caller reports success, so it refuses instead.
            # The message names the FILE, never the path: it can surface in an
            # HTTP error body (inference/routes/config.py).
...
def update_registry(mutator: Callable[[dict], None]) -> dict:
    ...
    with _io_lock, _cross_process_lock(REGISTRY_FILE):
        ...
        if not read_ok:
            # Same refusal as update_config: this writes the WHOLE dict rather
            # than a delta, so an unreadable registry plus one registration
            # would leave a registry.json holding that single model and nothing
            # else, and the registry is not reconstructible from anything else
            # on disk.
            raise ConfigUnreadable(
