...
# vae, text-encoder and unknown carry no reliable TYPE signal on HF, so they
# narrow by FORMAT only: filter=safetensors returns the canonical diffusers VAEs
# and encoders, filter=gguf the GGUF ones. Their type comes from the result badge
# and, at pull time, from the checkbox the user searched under.
...
# so the fit badge and the loader agree on "does it fit". Module-level names, so
# fit_label still reads a local constant.
...
def hf_search(query: str = "", limit: int = 20, formats: Sequence[str] = ("gguf",),
    ...
    # Round-robin interleave by per-query rank, then trim to `limit`, so one
    # high-download query cannot crowd the others out of the top `limit`.
    out: list[dict] = []
    rank = 0
...
# production caller probes on it. It must sit above a legitimate cold driver
# init - the first torch.cuda or HIP call of a process takes seconds - because a
# timeout is served as [] or a frozen last-known-good, which a bare-list caller
# cannot tell apart from "no GPU at all". On a wedged driver the cost is one
# worker thread parked once; every concurrent caller gets an instant BUSY.
#
# There is NO freshness/TTL cache: every call re-probes. A stale "free" reading
# would defeat switch_engine's eviction loop, which polls free VRAM to confirm a
# native free has landed. The last-known-good value is the wedge fallback only,
# never a short-circuit for a live probe.
...
# A user-facing "no GPU" message MUST branch on this.
...
# probe thread is started, and cleared when it lands, so a PATIENT off-loop
# caller can JOIN the running probe instead of being handed an instant
# GPU_PROBE_BUSY. The default guard behaviour is still to refuse to pile a second
# probe on a driver already being probed (BUSY). Joining is opt-in (list_gpus'
# wait_for_inflight), for a caller already OFF the event loop that can afford to
# wait out a cold driver init.
...
# An abandoned probe is still running and will write its reading whenever the
# native call finally returns, which can be long after the reset. A stale
# thread's result is fenced out by epoch rather than raced against.
_gpu_probe_epoch = 0
...
def _reset_gpu_probe_cache() -> None:
    ...
    with _gpu_probe_lock:
        _gpu_last_good = None
        _gpu_probe_inflight = False
        # Cleared with the rest of the probe state, so a reset gives a clean
        # slate.
        _isolated_torch_unavailable = False
        _isolated_torch_broken_warned = False
        # Unpublish the join handles too: after a reset the slot reads free, so
        # no caller joins a probe from the epoch just retired. An abandoned thread
        # still holds its own local done/result and, epoch-mismatched, will not
        # touch these globals again.
...
def _list_gpus_with_status(deadline: float, wait_for_inflight: bool = False) -> tuple:
    ...
    with _gpu_probe_lock:
        if _gpu_probe_inflight:
            if wait_for_inflight and _gpu_probe_done is not None:
                # A patient off-loop caller JOINS the in-flight probe instead of
                # piling a second probe on a driver already being probed: it waits
                # on ITS completion event, bounded by our own deadline. Both
                # handles are captured HERE, under the same lock that observed the
                # in-flight slot, so a concurrent completion cannot null them
                # between this check and the wait below.
                join_done = _gpu_probe_done
                join_result = _gpu_probe_result
            else:
                # Default: never pile on. Hands back the last-known-good reading
                # so this caller stays free. No fresh reading was taken, so the
                # status is BUSY, not a clean OK; [] is the safe "unknown" answer
            ...
            # Published under the SAME lock that claims the in-flight slot (only
            # one probe is ever in flight, so there is only one deadline to
            # describe). The probe body reads it to decide whether it can afford a
            # cold device-global VRAM source without overrunning this deadline.
            # See _apply_device_global_free.
    ...
    if join_done is not None:
        if join_done.wait(deadline):
            ...
            # done.set(). That is a BUSY, not a fresh OK.
        ...
        # outcome as starting one that overran - serve last-known-good and report
        # TIMEOUT, never mistaken for "no GPU". Nothing was spawned, so this
        # never piled onto the wedge.
    ...
    def _run() -> None:
        ...
        with _gpu_probe_lock:
            # GPU_PROBE_INCONCLUSIVE propagates the distinction
            # _isolated_torch_unavailable already makes: that latch is set ONLY
            # when the isolated torch enumeration proved it could not answer this
            # round, never for an honest "torch answered, zero devices". Gated on
            # an EMPTY value: a non-empty reading came from a source that DID
            # answer (nvidia-smi found real hardware, or torch answered before the
            # latch engaged) and is conclusive regardless of the latch. Read under
            # this same lock, which is the one guarding _torch_gpus_isolated_once.
            ...
            if _gpu_probe_epoch != my_epoch:
                ...
                # state the owner has dropped, and the in-flight slot is no longer
                # ours to clear (a later probe may already own it). BOTH writes
                # are dropped, at debug rather than silently.
                ...
                _gpu_probe_inflight = False
                # Unpublish alongside the in-flight slot: a NEW caller must start
                # a fresh probe, not join one that has already landed. A joiner
                # that already captured its local handle waits on that same
                # `done`, which is set unconditionally just below.
                _gpu_probe_done = None
                _gpu_probe_result = None
                ...
                # expired deadline, which reads as "no budget left" and would skip
                # a cold source.
                _probe_deadline_at = None
        # OUTSIDE the epoch gate and unconditional: a caller still inside its
        # deadline - the starter OR any joiner - is waiting on `done`, and
        # withholding it would make it wait out the full deadline and report a
        # COMPLETED probe as a TIMEOUT.
        result["value"] = value
    ...
    try:
        ...
        # Could not spawn the probe thread (e.g. OS thread exhaustion). Resets
        # the in-flight guard so a LATER call can retry (never left stuck True
        # with no thread to clear it), surfaces it at debug, and degrades to the
        # last-known-good reading rather than propagating a 500. No fresh reading
        # was taken -> BUSY. Epoch-gated like the clear in _run: if a reset
        # retired this probe, the slot may already belong to a newer one.
        with _gpu_probe_lock:
            if _gpu_probe_epoch == my_epoch:
                _gpu_probe_inflight = False
                _gpu_probe_done = None
        ...
        # so it does not wait out its full deadline on a probe that will never
        # run. `result` has no "value" key (the thread never set it), which the
        # join path reads as BUSY.
        done.set()
    ...
    # spawns no further threads. The status is TIMEOUT, never "no GPU".
...
def _torch_gpu_probe_known_doomed() -> bool:
    ...
    if "torch" in sys.modules:
        # A resident torch (imported before the runtime loaded, or a test's
        # injected stand-in) makes `import torch` a plain cache hit: no rocm_sdk
        # preload runs, so the conflict cannot occur and the working enumeration
        # is kept. On the doomed combo torch can never BE resident - the faulted
        # module is evicted on every attempt - so this never defuses the real
        # guard.
        return False
    try:
...
# timeout=5, and sit above a legitimate cold driver init. A ceiling ABOVE the
# caller's deadline means a box whose torch wedges never reaches the fallback
# within the caller's window, so every probe costs the full deadline and reports
# TIMEOUT while nvidia-smi is never consulted.
...
# _gpu_probe_lock, cleared by _reset_gpu_probe_cache. Without it, a box whose
# torch import wedges pays the full timeout on EVERY probe. What is remembered
# is "torch cannot be asked here", never a VRAM number, so nothing stale can
# reach switch_engine's eviction loop.
_isolated_torch_unavailable = False

# Latched once the isolated probe has been reported BROKEN. Separate from
# _isolated_torch_unavailable: that one disables a capability, this one only
# suppresses a repeated log line. Broken isolation keeps retrying in-process, so
# the warning would otherwise repeat on every probe.
_isolated_torch_broken_warned = False
...
# Cap on how much of the child probe's stderr is kept. That stderr can carry
# either a warnings.warn() message (whose point comes AFTER the
# file:line: Category: prefix) or an uncaught exception's traceback (whose point
# is the LAST line), so the limit is generous rather than truncated from either
# end. Any truncation beyond it is marked, never silent.
...
def _torch_gpus_isolated() -> "Optional[list]":
    ...
    if not raw:
        ...
        # path, so empty stdout means it died before printing (killed, hard
        # crash, a native fault taking the process down). That is COULD NOT ASK,
    ...
    try:
        ...
        return None
    if err:
        # The child prints its own failure cause here before answering [], so
        # this stream is kept rather than discarded.
...
def _torch_gpus_isolated_once() -> list:
    ...
    try:
        ...
        with _gpu_probe_lock:
            if not _isolated_torch_unavailable:
                ...
                # Said once, not once per probe: the per-attempt reason is
                # already logged by _torch_gpus_isolated, and this line marks
                # where those stop appearing.
                logger.debug(
    ...
    if devices is None:
        ...
        if first:
            # ONCE per process, not once per probe: every /api/stats poll drives
            # a probe, so an unconditional warning here would repeat for the life
            # of the server. Later occurrences stay at debug.
            logger.warning(
            ...
            for line in proc.stdout.strip().splitlines():
                ...
                try:
                    out.append({
                        ...
                        # every process, so unlike the torch path above it needs
                        # no correction.
...
# skipped rather than risk overrunning the probe deadline, which would cost the
# caller its free reading entirely. See _apply_device_global_free.
...
# in-flight slot, so the probe body can tell how much budget is left before
# spending it on a cold source. Safe as a module global because
# _gpu_probe_inflight serialises probes: only ever one in flight to describe.
_probe_deadline_at = None
...
def _apply_device_global_free(gpus: list) -> None:
    ...
    # The scope to use when a device-global correction is NOT available for an
    # entry (source cold-skipped, unmappable, or failed). Tags PROCESS only
    # where the raw reading is KNOWN blind (Windows + an AMD ROCm/HIP torch
    # build); elsewhere on Windows the raw cudaMemGetInfo is device-global by
    # documentation (NVIDIA). Computed up front so it is defined on every path
    ...
    try:
        ...
        # gpu_usage unimportable must not crash a probe. Conservative default:
        # DEVICE - never assert a blindness that cannot be confirmed.
        uncorrected_scope = FREE_SCOPE_DEVICE

    try:
        from localm.gpu_usage import device_global_used_bytes, source_is_warm
        # This runs INSIDE the deadline-bounded probe and spends the same budget
        # the driver call already spent. Opening the source costs a driver init,
        # once per process; a warm read is effectively free. A COLD source is
        # skipped when the remaining budget is too thin to absorb it, and the
        # reading is then tagged with the uncorrected scope rather than left
        # silently uncorrected. A warm source always runs.
        ...
        # uncorrected scope (PROCESS only where the raw reading is known blind).
    ...
    for g in gpus:
        ...
        if u is None:
            ...
            continue
        total = int(g["total"])
        # Clamp: the used figure and `total` come from different sources (the
        # driver's total vs the adapter's dedicated usage), so their difference
        # can land just outside [0, total].
...
def native_gpu_devices() -> Optional[list]:
    ...
    for d in raw:
        ...
        # GPU from an integrated one or an accelerator. llama.cpp skips ACCEL
        # entirely and uses an iGPU ONLY when no discrete GPU exists. Absent
        # when the probe did not report one; a caller that NEEDS the distinction
        # must then decline rather than assume.
        t = d.get("type")
        if isinstance(t, int):
            entry["type"] = t
        out.append(entry)
...
def resolve_main_gpu_index(configured, *, gpus: Optional[list] = None) -> int:
    ...
    # gap, so "idx < len(gpus)" alone could wave through an idx that corresponds
    # to no detected device. Skipped entirely when the active backend is vulkan:
    # list_gpus() is blind to Vulkan-only devices, so a non-empty result is not
    # authoritative for THIS backend's index space.
...
# whole layers across devices proportional to tensor_split; ROW and TENSOR split
# individual tensors instead.
_LLAMA_SPLIT_MODE_LAYER = 1

# Fallback tensor_split array capacity when llama_max_devices() cannot be
# probed (an older build without the symbol). Matches LLAMA_MAX_DEVICES from the
# pre-dynamic-backend-registry era. tensor_split is a raw `const float*` with no
# length of its own, so under-allocating would be an out-of-bounds read.
_TENSOR_SPLIT_FALLBACK_CAPACITY = 16

# Sanity ceiling for a gpu_split_indices entry. Bounds the ctypes tensor_split
# allocation apply_gpu_split eventually drives: without it, [0, 500000] would
# attempt a 500,001-element allocation before the native loader is invoked.
# settings_schema.py's MAX_GPU_SPLIT_INDEX applies the same value at config
# WRITE time; this is the independent check at READ time, so a hand-edited
# config.json that bypasses schema validation is still bounded here.
#
# Also used by resolve_main_gpu_index below to bound a single main_gpu_index:
# both values reach the identical ctypes.c_int32 main_gpu field.
...
def resolve_gpu_split(configured_indices, configured_ratios=None, *,
    ...
    *gpus*, for tests) currently sees, returning ``[(index, ratio), ...]``
    ...
    if gpus and not _native_backend_has_vulkan():
        ...
        # backend is vulkan (list_gpus() cannot see Vulkan-only devices, so a
        # non-empty result would not be authoritative for this backend's index
        # space): same boundary as resolve_main_gpu_index - no cross-check is
        # possible either way, so the configured indices pass through.
        valid = deduped
    ...
    if _native_backend_has_vulkan():
        ...
        for i in idx_list:
            ...
            if not isinstance(d, dict):
                ...
                # needs different words. These devices are llama.cpp's own list
                # (integrated GPUs and accelerators already removed, the rest
                # renumbered - see _llama_visible_devices), so a configured index
                # can legitimately point past the end.
                return _fallback(
        ...
        if gpus is None:
            ...
            # probe rather than taking an instant BUSY plus a stale reading.
            # Safe here because every probing caller on this path is off the
            # event loop (executor / CLI thread).
        ...
        for i in idx_list:
            ...
            # PROCESS-scoped (or untagged) reading the way gpu_split_shortfall's
            # refuse-only gate does. Real list_gpus() output always carries this
            # tag (_apply_device_global_free sets it on every entry, every
            # platform), so a missing tag is rejected the same as an explicitly
            # PROCESS-scoped one.
...
def vram_info(*, return_status: bool = False, deadline: Optional[float] = None,
    ...
    if sys.platform == "win32":
        try:
            ...
            if best:
                out = {"total": int(best)}
                # The registry gives total but NO free. Torch-less builds land
                # here for EVERY VRAM query (list_gpus() is empty - no torch to
                # enumerate, and nvidia-smi is NVIDIA-only). Recovers a
                # DEVICE-GLOBAL free from the ADL/PDH usage source, which works
                # torch-less and in-process (ADL for AMD, PDH's WDDM counter as
                # the vendor-neutral fallback - the same source
                # _apply_device_global_free uses). It maps ONLY when unambiguous
                # (exactly one AMD adapter for an AMD-named GPU, or exactly one
                # WDDM instance); otherwise total-only stands. The synthetic
                # index 0 never feeds GPU SELECTION - it only carries the name so
                # the AMD pairing can be authorised.
                #
                # ONLY when the probe COMPLETED empty (torch-less: it returns []
                # fast, status OK) - never on TIMEOUT, BUSY or INCONCLUSIVE. A
                # timeout means the driver is wedged or cold and the box is
                # unmeasurable, and the pre-load gate treats that as "skip the
                # VRAM check". INCONCLUSIVE gets the same treatment:
                # gpu_usage.device_global_used_bytes' ADL/PDH mapping partly
                # depends on torch's pci_bus_id, so it is not independent of the
                # same trouble. status is None only when the caller did not ask
                # for it (return_status=False fit-badges), which never gates on
                # any of these.
...
def vram_capacity(config: Optional[dict] = None, *, return_status: bool = False,
    ...
    def _vi():
        ...
        # byte-identical to a bare vram_info() for the no-kwarg vram_info()
        # doubles (same reason as _list_gpus_kw).
        kw = {}
        if return_status:
            kw["return_status"] = True
        if deadline is not None:
    ...
    # resolve_gpu_split's own early return - skips a hardware probe (list_gpus()
    # -> torch/nvidia-smi). Under combined_only this is a conclusive, probe-free
    # "no combined figure exists", reported with GPU_PROBE_OK, same as
    # gpu_split_shortfall's no-split return.
    ...
    if len(split_gpus) < 2:
        ...
        # vanished or was never present). Same forwarding as the no-split path,
        # so a cold init on THIS path also completes or joins rather than timing
        # out. Under combined_only, returns {} with the probe's REAL status.
        if combined_only:
            return _ret({})
        return _vi()
    ...
    if combined_only:
        ...
        # this key reads as "not a combined figure". Gated so the classic shape
        # stays byte-identical for every existing caller and test.
        out["devices"] = len(split_gpus)
    frees = [g.get("free") for g in split_gpus]
    if all(f is not None for f in frees):
        out["free"] = sum(frees)
        # All-or-nothing, mirroring the "free" key above: a sum is only a
        # whole-board figure if EVERY device in it is. One process-scoped device
        # makes the whole sum process-scoped, because that device's other-process
        # VRAM is missing from it. Absent entirely when NO device reported a
        # scope, which means UNKNOWN.
...
def implicit_split_capacity(config: Optional[dict] = None, *,
    ...
    if _native_backend_has_vulkan():
        ...
        # found, so an iGPU's memory must not be summed into a budget llama.cpp
        # then places entirely on the discrete card.
        #
        # native_gpu_devices already applies this same filter (see
        # _llama_visible_devices), so on a real reading this pass is a no-op; it
        # still runs for callers that patch native_gpu_devices directly.
        #
        ...
        # numeric value of ACCEL differs between builds), while CPU=0 and GPU=1
        # have been stable. A device whose type the probe did not report fails
        # the filter, and if that leaves fewer than 2 the single-device reading
        # stands.
        #
        # llama.cpp keeps at most ONE integrated GPU, so an iGPU-only box can
        # never reach the 2+ devices this requires.
        from localm.inference.backends.llamacpp._loader import GGML_DEV_TYPE_GPU
    ...
    # INFO so it reaches the always-on ring buffer. Runs per LOAD - the callers
    # are the backend's load-time preflights (_check_vram, _auto_gpu_layers,
    # _auto_ctx_max) - not per poll: the GUI's polling routes reach
    # sysstats.estimate_vram instead, which borrows only the pure
    # _bytes_per_token helper and never this.
    logger.info(
...
def gpu_split_shortfall(vram_required: int, config: Optional[dict] = None,
    ...
    *deadline* is forwarded to the underlying ``list_gpus`` probe (None leaves its
    ...
    if _native_backend_has_vulkan():
        ...
        # nvidia-smi) cannot see or order: torch index N is NOT ggml-vulkan index
        # N. A per-device share check here would measure the WRONG cards, so it
        # is skipped, and the skip is logged at INFO rather than presented as a
        # check that passed. The GGUF load is subprocess-isolated, so an
        ...
        # and reports GPU_PROBE_OK: nothing was probed, and this is a
        # deterministic routing decision rather than an inconclusive reading.
        return _ret([], GPU_PROBE_OK)

    gpus, status = _list_gpus_reading(deadline)
    if status != GPU_PROBE_OK:
        # No FRESH reading this call: list_gpus served a frozen last-known-good
        # value (or []) after a probe TIMEOUT/BUSY. This gate's contract is a
        # LIVE per-device check, so it neither quotes that stale "free" as a
        # current figure NOR refuses on it. The check could not run this call ->
        # admit best-effort, surfaced via debug and the returned status, never a
        # silent success. The GGUF/embedder load runs in an isolated worker whose
        # native abort is contained to that child.
    ...
    # (resolve_auto_split_ratios, computed here from THIS SAME fresh reading, so
    # gate and shares come from one snapshot): a device's share is
    # needed_i = R * free_i / total_free <= free_i whenever the aggregate R
    # fits, so a non-empty result means the COMBINED estimate is short. Pinned
    # ratios use the static-share math instead. When auto declines (a device's
    # free unmeasurable, a configured index not detected) the equal-split math
    # is used and shares_adaptive stays False.
    ...
    for idx, ratio in pairs:
        ...
        if g is None or g.get("free") is None:
            ...
            # "probe could not run" handler (that is the status != OK branch
            # above). Under GPU_PROBE_OK, list_gpus emits an int "free" for every
            # device and DROPS a non-reporting one, so this only stops a
            # None-free entry from crashing the loop.
            continue
...
def resolve_preferred_device(config: Optional[dict] = None, *,
    ...
    if split:
        ...
        if len(pairs) >= 2:
            ...
            if visible:
                # Split devices ARE torch-visible but none reports free VRAM, so
                # the capacity-informed choice cannot be made. Lead with the
                # first visible one and SAY SO: it may not be the emptiest card.
                # Warned, not raised.
                logger.warning(
            ...
            # passes indices through UNVALIDATED on the vulkan llama.cpp build, so
            # these are likely ggml-vulkan indices, which mean something else to
            # torch. Say so and let ComfyUI default.
            logger.warning(
...
def visible_device_order(config: Optional[dict] = None, *,
    ...
    if not _device_choice_configured(cfg):
        ...
        # config and do NOT probe. This gate mirrors resolve_preferred_device's
        # own (both call the shared helper). Callers put this on the launch path
        # (comfy_client.comfy_child_env).
        return None
    devices = gpus if gpus is not None else list_gpus()
