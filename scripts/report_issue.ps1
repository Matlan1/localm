...
function Scrub([string]$t) {
  ...
  $t = [regex]::Replace($t, '(?i)((?:(?<=[?&])(?:api[_-]?key|key|token|secret|password|passwd|pwd|auth|access[_-]?token|sig|signature)|(?<![A-Za-z0-9])(?:api[_-]?key|token|secret|password|passwd|pwd|access[_-]?token|signature)|(?<=[A-Za-z0-9])[_-](?:api[_-]?key|token|secret|password|passwd|pwd|access[_-]?token|signature|key|auth|sig))=)(?!(?:true|false|none|null|nil|yes|no|on|off|enabled|disabled|[01])(?![^&\s#\"''\)\]\}]))[^&\s#\"''\)\]\}]*', '${1}<redacted>')
