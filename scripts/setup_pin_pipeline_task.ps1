...
# --pin <Pin> daily. Without -Register, this only PRINTS the task it would
...
#
# Usage:
#   pwsh scripts/setup_pin_pipeline_task.ps1                            # dry run: llama, print only
...
if ($TimeLimitHours -le 0) {
    $TimeLimitHours = if ($Pin -eq 'comfyui') { 4 } else { 2 }
}
...
$innerCommand = "& '$PythonExe' '$PipelineScript' --pin $Pin *>> '$LogFile'"
