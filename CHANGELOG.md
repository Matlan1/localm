...
Each release adds its section on top, and every section below describes a
version that has SHIPPED: if it is written here, it is in a build you can
install and test. Published sections are the permanent public record of what
shipped and are never rewritten. Work that has been merged but not yet released
