...
Each release adds its section on top, and those sections are the permanent
public record of what shipped: they are never rewritten. This file documents
RELEASED versions only. It ships inside the build and is what the app shows
under "Show changelog", so every entry here describes something the copy you
are running can actually do.
