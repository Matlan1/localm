...
- **Links that open a new window now work in the Browser tab, and the automated browser no longer leaves hidden windows running.** Clicking a link or button that opens a new window now shows that window in the live view in place of the page, since the automated browser keeps one page open; before, the window opened out of sight and kept running, so the click seemed to do nothing. A window a page opens on its own, without a click, is closed and listed with the refused destinations. A click, key press or typed text the browser does not finish in time is now stopped instead of carrying on after the error, and the status line names what timed out instead of showing an empty error. `POST /api/browser/type` now refuses more than 4096 characters, and `/api/browser/key` a key name over 64 characters, with a 422. Opening the browser twice in quick succession no longer starts two browsers, and `POST /api/browser/stop` sent while the browser is still starting now stops it once it is up and ends its job.
