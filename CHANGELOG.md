...
- **Closing the terminal (Linux, macOS), `kill`, or Ctrl+Break now stop the server cleanly instead of making the crash watchdog relaunch it.** Ctrl+C in app-window mode, which leaves the server running, no longer turns off crash recovery for the rest of that run, and a crash while the console window is closing is still captured and reported on the next start.
