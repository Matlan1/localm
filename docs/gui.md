...
- Read-aloud: every reply has a 🔊 button, and the "Speak replies aloud" toggle
  ...
  loaded.
- Conversation persistence follows the session mode. In `privacy` (the default) conversations live in memory only and vanish on reload. In `log`/`full` they are saved to `sessions/` in the localm data directory (with localStorage as a cache), so they survive reloads, browser profile wipes, and server restarts. Deleting one removes it everywhere.
