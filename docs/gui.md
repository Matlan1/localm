...
**Basic features:**
- Model selector in the sidebar switches between registered models. Switching unloads the old one and loads the new one. When another localm instance on this machine already has the model you pick loaded, localm offers to use that copy instead of loading a second one (asking for that instance's API key only if it has one); the model stays selected for as long as that instance keeps answering.
- A message the selected model cannot handle is answered by an installed model that can: an attached image it cannot read, web access with a model that cannot emit structured tool calls, or a conversation that has outgrown the window it was trained for (instead of being compacted). The reply names the model that answered, with a note saying why. **Pin this chat to** in the parameters drawer keeps a conversation on one model whatever it needs; a pinned chat that sends an image its model cannot read gets the image removed with an explanation.
