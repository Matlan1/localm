...
A named API key's RAG reach (indexing, querying, and listing collections) can
be confined to specific folders with `localm key create NAME --scope rag
--rag-root DIR` (repeatable) - see [API keys](#api-keys) and
[docs/rag.md](../docs/rag.md#per-key-folder-scoping).
...
- `--rag-root DIR` (repeatable) - confines the key's RAG indexing, querying, and collection-listing reach to exactly these folders instead of the global folder policy. Omit for no per-key restriction. See [docs/rag.md](../docs/rag.md#per-key-folder-scoping).
