...
CI runs that selection on every pull request. The `python-pr-gate` job in
`.github/workflows/ci.yml` (`scripts/run_affected_tests.py`) runs the
`--depth 1` selection on ubuntu after the lockfile check, the hygiene gate
and ruff. It never runs the whole suite: when the depth-1 selection is wider
than a quarter of the suite it runs the depth-0 selection instead, and it
fails when that is wide too, when the selector fails, prints nothing, or
names a path that is not an existing test file. A change that fails the
gate that way needs the two-platform matrix with coverage, which stays
