...
def _write_raw(target, text):
    """A marker written by hand, byte for byte - NOT through the recorder, so an old on-disk format is reproduced rather than re-derived from current code."""
...
def test_old_single_token_marker_reports_no_build(tmp_path):
    """None, not '' and not a guess."""
    t = _write_raw(tmp_path / "bin", "amd-rocm\n")
    assert _provisioned_build(t) is None


def test_old_format_still_satisfies_the_provision_guard(tmp_path):
    """The actual regression risk, asserted as the guard states it: an install whose marker predates this change must still short-circuit rather than re-download on every run."""
...
def test_two_token_marker_resolves_the_identical_backend(tmp_path):
    """Same answer as the one-token marker, which is what makes the tag safe to add: had this returned 'amd-rocm b1307', the guard would re-provision every single time."""
...
def test_an_unexpected_third_token_does_not_break_the_backend(tmp_path):
    """Forward compatibility: a marker written by some LATER release that adds a field must still resolve its backend here, not wedge an older localm into re-provisioning forever."""
...
def test_recording_without_a_build_writes_the_old_shape(tmp_path):
    """A backend whose tag costs a network call records one token, exactly as before - so this change adds no new failure mode for cuda/vulkan/cpu."""
    t = tmp_path / "bin"
    t.mkdir()
...
def test_the_positional_two_arg_call_still_works(tmp_path):
    """Existing call sites pass (target, backend) positionally."""
    t = tmp_path / "bin"
    t.mkdir()
...
def test_empty_marker_is_unknown_not_empty_string(tmp_path):
    """An empty string would compare unequal to every backend AND read as 'recorded', which is the worst of both."""
...
def test_ragged_whitespace_between_tokens_is_tolerated(tmp_path):
    """Split on arbitrary whitespace, not a single space: a marker that picked up a tab or a double space must not silently become an unknown backend."""
    t = _write_raw(tmp_path / "bin", "  amd-rocm \t b1307  \n")
    assert _provisioned_backend(t) == "amd-rocm"
    assert _provisioned_build(t) == "b1307"
