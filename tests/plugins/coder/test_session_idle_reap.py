...
"""NEW-CODER-NO-TOOLCALL-SILENT residual (E): a GUI coder session the user abandons without an explicit DELETE - a closed tab, a killed browser - never triggered close(), so it never wrote the 'session ended' audit record a normally-ended session gets."""
...
def test_reap_idle_never_touches_a_busy_session(tmp_path):
    """A long-running task (a slow model, a big verify) is still very much alive even though it has pushed no NEW event in a while at the exact moment reap_idle runs - reaping it mid-task would corrupt the run."""
    mgr = SessionManager()
    s = mgr.create(_session(tmp_path))
    s.busy = True
    now = s.last_activity_at + _IDLE_REAP_SECONDS + 1
...
def test_list_opportunistically_reaps_without_a_background_thread(tmp_path):
    """No timer to start or stop: an ordinary list() call (the GUI already polls this for the sidebar) is what performs the sweep."""
    mgr = SessionManager()
    s = mgr.create(_session(tmp_path))
