...
class TestFluxImageTool(unittest.TestCase):
    ...
    def test_uses_default_api_url_not_a_hardcoded_default(self, mock_gen):
        """This tool used to hardcode os.environ.get('FLUX_API_URL', 'http://127.0.0.1:8188'), bypassing default_api_url()'s localm-managed-instance routing entirely - so the coder agent's image tool never routed to a managed ComfyUI, managed- active or not (same bug family as NEW-COMFY-TARGET-OWN-DEFEATED-BY-..."""
        mock_gen.return_value = (True, "Image saved to x")
    ...
    def test_lora_name_traversal_rejected(self, mock_ensure):
        """This tool calls localm.image_gen.comfy.generate_image DIRECTLY - it never goes through the HTTP image route's plug.py._validate_lora_name (that guard only covers browser-originated requests), so a malicious lora_name reaching this tool must be caught by the SAME check inside _build_image_workflow it..."""
...
def test_agent_injects_privacy_for_generate_image(tmp_path):
    """BUG-10: the coder agent must inject _privacy=True for generate_image in privacy mode, so the tool suppresses the prompt sidecar."""
...
def test_repl_generate_image_privacy_no_sidecar(tmp_path, monkeypatch):
    """BUG-9: REPL /generate-image must suppress the sidecar in privacy mode and pass the resolved api_url (not the hardcoded 8188 default)."""
