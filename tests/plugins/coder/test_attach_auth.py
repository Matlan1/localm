...
"""`localcoder` attaching to an already-running localm instance must authenticate with the OWNER KEY when this install has one configured, not the discovered instance's raw per-instance attach token (``auth.verify()`` has no notion of instance tokens at all and 401s once any key is configured), and not..."""
...
def _capture_http_backend(monkeypatch, module=cli_main):
    """HTTPBackend is a plain top-level import in _main.py (unlike make_localm_backend, which has an explicit live-attribute-access indirection for exactly this reason) - patch it on the submodule that actually holds the reference, not the localm.plugins.coder.cli package re-export."""
    captured = {}
...
class TestAttachToAutoStartedInstance:
    """The other _tgt branch: no instance was running, this process spawns `localm gui` itself and then attaches once it comes up - same _api_key_explicit computation, a separate HTTPBackend construction site that must not silently diverge from the branch above (diff-review- discipline.md item 23: a fix te..."""
