...
def _outside_scope(tmp_path: Path, name: str = "x.txt") -> str:
    """An absolute path OUTSIDE the ``src/**`` scope, owned and created by the test in its own tmp_path."""
...
class TestScopeEnforcement:
    ...
    def test_mcp_tool_path_confined_by_scope(self, tmp_path):
        """CHK-MCP-SCOPE: a REGISTERED mcp_* tool's path arg is confined by the active scope, even though MCP tools are not in _SCOPED_TOOLS. (MCP tools are registered dynamically in production; here a stub reaches the scope gate.)."""
    ...
    def test_plugin_tool_path_confined_by_scope(self, tmp_path):
        """CHK-SCOPE-PLUGIN: a plugin_* tool's path arg is confined by the active scope, same as an mcp_* tool."""
        from localm.plugins.coder import agent as _agent
        agent = _make_agent(tmp_path, scope="src/**")
...
class TestScopeEnforcementNeverStatsTheValue:
    ...
    def test_the_recorder_still_catches_a_stat_of_the_value(
        ...
        """Fires-control for the anchor exclusion in :func:`_records_touches_outside`."""
        outside = tmp_path_factory.mktemp("outside-cwd")
        target = outside / "disposable-target.txt"
        target.write_text("disposable\n", encoding="utf-8")
    ...
    def test_an_in_cwd_absolute_path_in_scope_is_still_allowed(self, tmp_path):
        """BUG-6, unchanged: an absolute path INSIDE cwd that matches the scope must pass."""
        (tmp_path / "src").mkdir()
    ...
    def test_a_path_reaching_into_cwd_through_a_link_is_refused(
            self, tmp_path, tmp_path_factory):
        """The accepted COST of the fix, pinned so it cannot be reintroduced by accident or silently regress back to statting."""
        (tmp_path / "src").mkdir()
...
class TestScopeShellNotice:
    """run_shell / run_tests are deliberately left out of _SCOPED_TOOLS: they execute a process, and no path-arg check can confine arbitrary code."""
    ...
    def test_no_notice_when_the_shell_tools_are_disabled(self, tmp_path):
        """Nothing to warn about: with the shell gone the scope really is the boundary."""
    ...
    def test_a_sub_agent_does_not_repeat_the_notice(self, tmp_path):
        """A spawned child is a new Agent but not a new session: the user already saw the notice from the parent, and a delegation-heavy run would otherwise repeat it once per spawn until it reads as noise."""
...
class TestShellArgvScopeCheck:
    """Best-effort argv check: flag a shell command that references paths outside the scope."""
    ...
    def test_the_command_still_runs(self, tmp_path):
        """Warn, do not block: escalating a legitimate command into a hard failure would break working setups for a heuristic's benefit."""
    ...
    def test_in_scope_path_is_not_flagged(self, tmp_path):
        """Control: the check must be quiet when the command stays in scope, or it is noise the user learns to ignore."""
        (tmp_path / "src").mkdir()
        (tmp_path / "src" / "main.py").write_text("pass\n")
    ...
    def test_flags_and_non_path_tokens_are_not_mistaken_for_paths(self, tmp_path):
        """A regex, a flag, and a quoted message all contain slashes but none is a path; flagging them would drown the real signal."""
        _, warnings, _ = self._run_shell(
            tmp_path, "src/**",
    ...
    def test_run_tests_path_arg_is_checked_too(self, tmp_path):
        """run_shell is not the only unscoped executor; run_tests takes a target path that can point anywhere too."""
        from localm.plugins.coder import agent as _agent
...
class TestShellArgvScopeHeuristicPrecision:
    """A warning nobody believes is worse than no warning."""

    @pytest.fixture
    def repo(self, tmp_path):
        """An utterly ordinary layout: these directory names are exactly the ones common command verbs collide with."""
    ...
    def test_a_real_drive_path_is_still_flagged(self, repo, tmp_path_factory):
        """Fires-control for the case above: the same colon check, still loud on a genuinely drive-qualified path."""
    ...
    def test_a_path_valued_flag_is_caught_when_written_as_a_path(self, repo):
        """`git -C dir` moves the process's working directory, the strongest out-of-scope signal there is."""
    ...
    def test_a_colon_delimiter_holding_slashes_is_not_a_drive_path(self, repo):
        """A colon is chosen as the sed delimiter precisely BECAUSE the pattern contains slashes, so this is the common form."""
    ...
    def test_position_no_longer_changes_the_answer(self, repo):
        """The old check answered differently depending on where a word sat in the command line (program slot, subcommand slot, argument), which is what all the runner/wrapper/flag tables were for."""
    ...
    def test_an_explicit_path_in_command_position_is_still_flagged(
        ...
        """Running a script written out as an out-of-scope path is what the warning is for, wherever it sits in the command line."""
    ...
    def test_a_real_out_of_scope_reference_still_warns(self, repo, tmp_path_factory):
        """The #781 behaviour restated for the lexical rule: an explicitly written relative path, a parent traversal, and an absolute path outside cwd are all still reported."""
    ...
    def test_run_tests_path_is_a_declared_path_arg(self, repo):
        """run_tests' `path` is declared a path by the tool's own schema, so it needs no path-likeness guess at all: it is checked WHOLE (spaces and all) and a bare `tests` is still reported there, unlike the same word inside a command line. `extra_args` is free-form (a -k expression, a marker name), so it fol..."""
...
class TestPathLikeSyntax:
    """`_is_path_like` and `_looks_like_drive_path` are pure STRING classifiers, and they are the whole reason the scope check no longer needs the filesystem, so they are tested as strings."""
...
class TestShellScopeCheckIsPurelyLexical:
    """The scope warning evaluates a command the model has only PROPOSED: before any confirmation, before anything executes."""
    ...
    def test_the_guard_itself_fires(self, tmp_path):
        """Fires-control for `_no_filesystem`."""
    ...
    def test_the_coverage_exemption_is_narrow(self, tmp_path):
        """Control for `_called_by_coverage`."""
    ...
    def test_the_whole_warning_path_touches_no_files(self, agent, tmp_path_factory):
        """Not only the classifier: formatting the warning and recording the audit notice run on the same proposed command, so they must stay clean too."""
