...
"""A tool call recovered ONLY via parser.py's name-gated lenient fallback (no <tool_call> wrapper, no explicit fence, no marker of any kind - just a JSON blob whose shape happens to match a real tool name) must not be allowed to silently execute a destructive tool under auto_approve, even though an ord..."""
...
def test_a_lenient_recovered_destructive_call_is_denied_under_auto_approve(tmp_path):
    """THE FIX. auto_approve=True normally skips confirmation for a destructive tool - but a call recovered with no marker of tool-call intent at all must still be denied when nobody is available to confirm it (fail-closed, the same path an explicit always_confirm entry takes)."""
