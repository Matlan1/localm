...
"""CODER-4: the resume checkpoint lives under HOME/checkpoints/<digest>.json, not in the project tree (<cwd>/.localcoder/checkpoint.json)."""
...
def test_distinct_sessions_in_one_project_get_distinct_checkpoint_files(
    ...
    """NEW-CODER-RESUME-DESTROYS-SESSIONS: two sessions in the SAME project must land in two different files, not the one file every session used to share (and silently overwrite)."""
    import localm.config as cfg
    monkeypatch.setattr(cfg, "HOME_DIR", tmp_path / "home")
