...
class TestSpawnAgentConfirmationPropagation:
    ...
    def test_baseline_child_still_auto_approves_when_parent_does(self, tmp_path):
        # Sanity: the fix must not break the common case - a parent with
        # auto_approve=True (and no confirm gate) should still spawn a child
        # that executes destructive tools without friction.
