...
"""CPU-resident reviewer: a different small model loaded in-process on CPU, for a heterogeneous review that is still fully local + private (allowed in privacy mode)."""
...
def test_local_backend_supports_grammar_mirrors_the_engine(monkeypatch):
    """NEW-CODER-NO-TOOLCALL-SILENT residual: LocalEngineBackend must not compute its own guess at grammar support from the backend's TYPE (the old isinstance(engine._backend, GgufBackend) check) - it defers entirely to Engine.supports_grammar (engine.py), which is the honest, install-derived answer from t..."""
    from localm.plugins.coder.backends.local_engine import LocalEngineBackend
