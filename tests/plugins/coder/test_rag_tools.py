...
# --------------------------------------------------------------------------- #
#  SECURITY: retrieved text must be neutralised (ADR-0006's central finding - #
#  Collection.query() itself does NOT neutralise; only the HTTP route does)   #
# --------------------------------------------------------------------------- #
...
def test_rag_search_flags():
    ...
    # Non-mutating - destructive=True would also trigger dry-run-skip and
    # undo-snapshot machinery meant for state mutation (ADR-0006).
    assert td.destructive is False
...
# --------------------------------------------------------------------------- #
#  Real dispatch: restricted-session exclusion (ADR-0006's real boundary)     #
# --------------------------------------------------------------------------- #
