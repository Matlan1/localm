...
class TestSplitCommandRemovesQuotes:
    ...
    def test_hash_inside_an_argument_is_not_a_comment(self):
        """shlex's default commenter would silently truncate the message, so clearing it is load-bearing rather than tidy."""
...
class TestWindowsBackslashesSurviveSplitting:
    """Clearing shlex's escape character is what keeps these intact."""
...
class TestQuotedPathsReallyExecute:
    ...
    def test_argv_route_reads_a_quoted_path_with_spaces(self, tmp_path):
        """Interpreter, script and target are all quoted paths, two of them containing spaces - so argv[0] and a later argument are both covered."""
    ...
    def test_shell_route_reads_a_quoted_path_with_spaces(self, tmp_path):
        """The shell route is where the cmd.exe re-quoting defect lived."""
    ...
    def test_unquoted_path_without_spaces_still_works(self, tmp_path):
        """Regression guard: the unquoted form must not have been traded away to make the quoted one work."""
    ...
    def test_shell_operators_still_work_with_a_quoted_path(self, tmp_path):
        """Handing cmd the command line verbatim must not cost us the operators that are the whole reason for the shell route."""
        _, payload = _payload(tmp_path)
...
class TestTheQuotingDefectsFire:
    """The pre-fix mechanisms, run against the same real files, in the same run that shows the fix working."""

    def test_the_old_split_mode_got_the_token_boundary_wrong(self):
        """Cross-platform, no execution needed: posix=False splits an embedded-quote argument in two, where the fix keeps it whole."""
    ...
    def test_the_old_split_left_quotes_in_the_argument(self, tmp_path):
        """Defect A: posix=False does not remove quotes, so the process got a filename with literal quote characters in it and could not open it."""
        d, payload = _payload(tmp_path)
    ...
    def test_the_old_shell_wrapping_mangled_a_quoted_path(self, tmp_path):
        """Defect B: ['cmd', '/C', command] is rendered by list2cmdline, which escapes each embedded quote MSVCRT-style - syntax cmd.exe misreads."""
        _, payload = _payload(tmp_path)
