...
"""The tts plugin's server-side settings write surface: GET/POST /v1/tts/config plus the settings_schema helpers behind them."""
...
def test_script_url_fields_require_an_owner_admin_key(env):
    """library / wasm_paths are loaded as a script + wasm base by EVERY browser client, so a non-owner config:write key must not be able to set them (mirrors REC-MEDIA-CMD for a media backend's launch_cmd / api_url)."""
    from localm import auth, scopes
    from localm.inference.http_server import create_app
...
def test_get_hides_library_and_wasm_paths_from_a_config_read_only_key(env):
    """Regression for pentest finding LM-PT-002: a config:read-scoped, non-owner key must not learn library/wasm_paths (the script/wasm URL every browser loads) from GET /v1/tts/config, even though it may legitimately read every other tts setting."""
    from localm import auth, scopes
    from localm.inference.http_server import create_app
...
def test_post_response_hides_library_and_wasm_paths_from_a_scoped_writer(env):
    """The write endpoint's own response echoes the plugin's resolved fields back too (e.g. after saving voice) - same leak, same fix: a config:write key that is not an owner must not have library/wasm_paths' value echoed back even for a save that never touched either field."""
    from localm import auth, scopes
    from localm.inference.http_server import create_app
