...
"""B10: `localm rag query` is lexical-only by default; `--embed` opts into hybrid retrieval by embedding the query via a running localm server (matching the GUI)."""
...
def test_cli_embed_fn_uses_persisted_key(monkeypatch):
    """`rag add/query --embed` must authenticate to /v1/embeddings with the persisted owner key (auth.key), not the env var only."""
    import requests
...
def test_cli_embed_fn_sends_the_configured_model_name(monkeypatch):
    """`rag add/query --embed` must send the CONFIGURED embedding model name, the same as the GUI's server-side self-embed - NOT the hardcoded sentinel 'localm'. /v1/embeddings routes to the dedicated embedder only when the model name matches the configured `embedding_model` (or a registered embedder); 'lo..."""
    from localm.cli.rag import _cli_rag_embed_fn
    monkeypatch.setattr(
...
def test_cli_embed_fn_defaults_to_the_internal_embedder_not_localm(monkeypatch):
    """With no explicit embedding_model set, the CLI must still send the internal DEFAULT embedder name (bge-small-en-v1.5), which /v1/embeddings can route - never the bare 'localm' sentinel that falls through to the chat path."""
    from localm.cli.rag import _cli_rag_embed_fn
