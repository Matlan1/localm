...
"""scripts/check_hygiene.py check 7: no module-level import cycles between the top-level units under localm/."""
...
def test_peers_sharing_a_lower_unit_are_legal(tmp_path, monkeypatch):
    """image_gen / music_gen / video_gen all importing media is not an ordering violation - they are unordered peers, and no cycle exists."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
...
def test_function_local_import_does_not_count(tmp_path, monkeypatch):
    """A deferred import is the standard, deliberate way to break a cycle and is NOT a violation."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
        "inference/engine.py": (
            "def go():\n"
...
def test_unparseable_file_is_reported_not_skipped(tmp_path, monkeypatch):
    """A file the gate cannot read is one it cannot vouch for."""
    ch = _load_check_hygiene()
...
def test_relative_import_cycle_is_detected(tmp_path, monkeypatch):
    """A cycle built entirely from relative imports (``from ..x.y import z``) must be caught exactly like an absolute one."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
...
def test_relative_import_from_a_package_init_resolves_correctly(tmp_path, monkeypatch):
    """A relative import's anchor is ``__package__``, which for a package's ``__init__.py`` IS the package's own name - one level shallower than for a plain sibling module, whose ``__package__`` is its PARENT."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
...
def test_resolve_relative_import_is_package_vs_plain_module(tmp_path, monkeypatch):
    """Direct unit test of the resolver's arithmetic (diff-review-discipline: test the relation, not just a literal)."""
    ch = _load_check_hygiene()
...
def test_resolve_relative_import_beyond_top_level_returns_none(tmp_path, monkeypatch):
    """A relative import whose dots walk past the top-level ``localm`` package is invalid Python (fails at runtime with 'attempted relative import beyond top-level package') - the resolver must say 'nothing to resolve', never guess a plausible-looking but wrong package."""
    ch = _load_check_hygiene()
...
def test_module_level_try_import_is_detected(tmp_path, monkeypatch):
    """``try: import X except ImportError: ...`` at module level runs eagerly during import - it is the standard optional-dependency idiom, not a deferred import."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
        "inference/engine.py": (
            "try:\n"
...
def test_module_level_try_except_handler_import_is_detected(tmp_path, monkeypatch):
    """The FALLBACK import in an ``except:`` handler is just as eager as the one in the ``try:`` body it replaces - both run during import, only one of them on any given run."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
        "inference/engine.py": (
            "try:\n"
...
def test_module_level_if_else_branch_import_is_detected(tmp_path, monkeypatch):
    """Either branch of an ``if``/``else`` can be the one that actually runs, so either must be able to create a real edge - not just the ``if`` body."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
        "inference/engine.py": (
            "import sys\n"
...
def test_type_checking_guarded_import_does_not_count(tmp_path, monkeypatch):
    """``if TYPE_CHECKING:`` is False at runtime by definition - an import inside it never actually executes, and is the standard way to add a type-only import WITHOUT creating a real cycle."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
        "inference/engine.py": (
...
def test_def_inside_module_level_try_stays_deferred(tmp_path, monkeypatch):
    """A function DEFINED inside a module-level ``try:`` block is still a function: an import inside ITS body only runs when the function is later CALLED, exactly like a plain top-level ``def``."""
    ch = _load_check_hygiene()
    _pkg(tmp_path, {
        "inference/engine.py": (
            "try:\n"
