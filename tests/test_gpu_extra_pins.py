...
transformers 5.14 imports `transformers/distributed/fsdp.py` on the ordinary
...
so transformers 5.14 makes EVERY HF model load die at "loading processor..." - and the
lazy-import layer reports it as "Could not import module 'AutoTokenizer'", hiding the
real cause. transformers 5.13.x does not import fsdp on that path and works.
...
# The first version known to hard-import fsdp on the tokenizer path. Raise this ONLY
# with a cold-install re-verify (not a dev-venv check - see the module docstring).
_TRANSFORMERS_BREAKS_AT = "5.14"
