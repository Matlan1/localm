...
def test_pyproject_transformers_spec_excludes_the_fsdp_breaking_line():
    """The [gpu] pin must not ADMIT transformers 5.14+."""
...
def test_locked_transformers_cannot_break_the_hf_backend():
    """uv.lock ships inside the release zip, so the LOCKED version is what a real user installs."""
...
def test_locked_torch_is_the_pinned_rocm_wheel_on_windows():
    """Pins the OTHER half of the incompatibility, so the reason the cap exists stays visible: if torch ever moves to a build that HAS distributed, the transformers cap can be revisited (and this test is where you will be reminded to)."""
    packaging_requirements = pytest.importorskip("packaging.requirements")
