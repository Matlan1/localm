...
"""F9: jobs/runner.py's _run_memory and _run_chat must wrap their generation calls in http_server.driving_engine, or a scheduled job running on a quiet server (no concurrent HTTP traffic) leaves nothing marking the model as in-use - idle-unload can unload it mid-run."""
