...
"""Regression audit 2026-07-14: three defects that make localm's own diagnostics lie about what went wrong."""
...
# --------------------------------------------------------------------------- #
#  REG-619: an oversized error must be truncated, never dropped whole          #
# --------------------------------------------------------------------------- #

def _native_crash_block(pad_to, stamp="2026-07-13 15:24:50,000"):
    """A realistic deep native-load traceback, in the REAL log-line format the digest's own parser keys off (a timestamped LEVEL line starts a record, and unprefixed lines continue it) - otherwise this would parse as one anonymous blob and never exercise the multi-record path."""
...
def test_truncated_error_says_it_was_truncated():
    """Rule 5: a cut-down error must not look like the whole error - the reader has to know bytes are missing, or they will diagnose from a partial trace believing it complete."""
    from localm._log_digest import build_digest
...
def test_oversized_error_keeps_its_tail_not_its_head():
    """The tail is the actionable end of a traceback (the innermost exception), and is what the _recent_log_tail this replaced always surfaced."""
    from localm._log_digest import build_digest
...
def test_newest_oversized_error_wins_over_older_ones():
    """With several errors and no room, the MOST RECENT is the one to keep (partially): it is the one the user is reporting."""
    from localm._log_digest import build_digest
...
def test_errors_that_fit_are_still_kept_whole():
    """NEGATIVE CASE: the fix must not start truncating errors that fit."""
    from localm._log_digest import build_digest
...
def test_budget_is_never_exceeded_by_the_truncation_path():
    """NEGATIVE CASE: truncating must respect the budget it exists to satisfy - across a range of budgets, including tight ones."""
    from localm._log_digest import build_digest
...
# --------------------------------------------------------------------------- #
#  REG-542: a hang trace must belong to the run being reported                 #
# --------------------------------------------------------------------------- #

def _write_hang(home, pid, age_s, text="MainThread stack:\n  frozen_call()\n"):
    """A hang trace exactly as debuglog.hang_trace_path() names it: hang_<date>_<pid>.log."""
    d = home / "logs"
...
def test_hang_trace_from_this_run_is_still_attached(tmp_path):
    """NEGATIVE CASE, the important one: the fix must not throw the baby out."""
    from localm import bugreport
...
def test_hang_trace_from_the_crashed_run_is_attached_on_recovery(tmp_path):
    """NEGATIVE CASE: the crash-recovery path reports a PRIOR run, so it must attach THAT run's hang trace (matched by its pid, the way _recent_log_tail already matches its log), not this recovering process's."""
    from localm import bugreport
...
def test_a_reused_pid_on_an_old_trace_is_not_mistaken_for_this_run(tmp_path):
    """NEGATIVE CASE: pids are reused (that is REG-586's whole premise), so a pid match alone cannot prove a trace belongs to this run."""
    from localm import bugreport
...
def test_recent_trace_from_another_live_run_is_not_attached(tmp_path):
    """NEGATIVE CASE: recency alone is not enough either - a freeze in a DIFFERENT localm instance minutes ago is not this report's problem."""
    from localm import bugreport
...
def test_no_hang_files_is_still_empty_and_never_raises(tmp_path):
    """NEGATIVE CASE: the no-freeze case (the overwhelming majority) is unchanged and must never raise."""
    from localm import bugreport
...
# --------------------------------------------------------------------------- #
#  REG-605: the watchdog must probe the port the restart actually binds        #
# --------------------------------------------------------------------------- #

def _port_from_argv(argv):
    """The port a click command would resolve from *argv* (last -p/--port wins, which is click's own precedence), or None when no port token is present - exactly what the re-exec'd process passes to pick_port()."""
    port = None
...
def restart_harness(tmp_path, monkeypatch):
    """Capture what _do_restart hands to the watchdog and to execv, without re-execing or spawning anything."""
...
def test_restarted_process_binds_exactly_the_probed_port(restart_harness, monkeypatch):
    """The same defect proven through the REAL pick_port(), which is what actually decides the new process's port - not just the presence of a token."""
    import sys
...
def test_an_explicit_user_port_is_not_duplicated_or_contradicted(restart_harness,
                                                                 monkeypatch):
    """NEGATIVE CASE: when the user DID pass -p, the resolved port is the same one, and the argv must still resolve to it (click takes the last occurrence, so an appended token is consistent, not contradictory)."""
    import sys
...
def test_plain_restart_without_an_update_also_keeps_its_port(restart_harness,
                                                             monkeypatch):
    """The /v1/server/restart button has the same port-drift bug (its docstring makes the same 'comes back on the same port' promise), just without a watchdog to turn it into a rollback: today a plain restart can silently move the server, stranding the user's open GUI tab on a dead port."""
    import sys
...
def test_restart_without_a_known_port_is_unchanged(restart_harness, monkeypatch):
    """NEGATIVE CASE: a bare create_app() harness that never advertised has no instance_port, so there is no port to pin - the restart must proceed exactly as before rather than inventing one."""
    import sys
