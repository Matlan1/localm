...
"""JobManager must be able to say what is running, and must keep a job that just finished."""
...
def test_mark_finished_is_idempotent():
    """A second call must not move the stamp forward, or a cancel racing the worker's own exit would hand the job a fresh lease on the TTL."""
    job = Job(id="a", kind="pull", argv=[])
    job.mark_finished()
...
def test_gc_never_evicts_a_running_job_at_any_age():
    """Deliberate, not an oversight: evicting a live job would strand its SSE subscribers and lose the record while the work carries on."""
    mgr = JobManager()
...
def test_snapshot_finds_a_job_without_holding_its_id():
    """The whole point: a caller that never saw the POST response can still learn the operation exists."""
    mgr = JobManager()
...
def test_snapshot_never_leaks_argv_or_owner():
    """argv carries the resolved model spec and any host path the caller passed, and owner is a keystore hash."""
    mgr = JobManager()
...
def test_summary_omits_pct_entirely_when_nothing_reported_progress():
    """ADR-0008 R1: a pull that has not yet read a byte count is at an UNKNOWN percentage, not at 0 percent."""
    job = Job(id="a", kind="pull", argv=[])
    row = job.summary()
