...
def test_unload_uses_instance_token_when_no_key_configured(monkeypatch):
    """#1121-class fix: in open (keyless) mode, the caller's own instance token is sent in place of a key - mirrors self_request's test_open_mode_uses_instance_token_when_no_key_configured."""
    monkeypatch.delenv("LOCALM_API_KEY", raising=False)
    captured = {}
...
def _real_open_mode_server():
    """A real uvicorn instance, open (keyless) mode, with app.state.instance_token set the way instances.advertise() sets it in production - mirrors test_vram_handover_open_mode_auth.py's helper for the primary VRAM-handover path, applied here to the fallback unload."""
    app = create_app(None)
...
class TestUnloadRealHttpOpenMode:
    ...
    def test_no_token_still_403s_and_returns_none(self, monkeypatch):
        """Sanity/negative: the gate is not bypassable by omitting the credential either - proves the previous test's success is really about the instance_token, not the gate being permissive."""
        monkeypatch.delenv("LOCALM_API_KEY", raising=False)
        app, port, server, th = _real_open_mode_server()
        try:
            url = f"http://127.0.0.1:{port}/v1"
