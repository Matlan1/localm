...
"""H1: `localm pull <local path>` must register the file in place instead of mis-parsing a Windows drive-colon as owner/repo:file or rejecting 'Unknown spec'."""
...
class TestPullLocalSha256:
    """A user-supplied --sha256 on a LOCAL pull is a safety assertion: it must be verified against the real bytes, never silently ignored while reporting success (AGENTS.md rule 5: a safety step that fails must not report success)."""
...
class TestSafeFilenameFailsClosed:
    """The download-destination guard must fail CLOSED (return None) on unsafe input, never raise - a NUL byte used to escape the OSError-only guard as an uncaught ValueError."""
...
class TestSafeFilenameRejectsWindowsHazardClass:
    """A filename that passes the single-component/no-traversal checks can still be a Windows filename-CONFUSION hazard rather than a directory escape: it stays confined to base_dir (the CWE-22 property CodeQL's py/path-injection alert checks holds), but names something other than what it appears to."""
    ...
    def test_ads_candidate_never_enters_the_real_pick_pool(self, tmp_path, isolated_home):
        """End-to-end: a malicious HF repo listing offering an ADS-named 'mmproj' sibling must never be picked as a candidate by the real caller, not just rejected by the validator in isolation."""
    ...
    def test_alias_resolving_to_a_different_existing_file_is_rejected(
        ...
        """An OS-level alias - an NTFS 8.3 short name is the live-confirmed case (adversarial review, this session): on a volume with short-name generation enabled, 'LONGMO~1.GGU' resolves to a pre-existing 'LongModelNameThatIsVeryLong.gguf' - passes every earlier check, but names something other than what it..."""
        base = tmp_path / "models"
        base.mkdir()
    ...
    def test_same_name_repull_of_existing_file_still_accepted(
        ...
        """The alias check must not reject the ordinary, legitimate case of re-pulling a model that is already on disk under the exact name requested."""
        base = tmp_path / "models"
        base.mkdir()
    ...
    def test_case_variant_of_existing_file_still_accepted(
            self, tmp_path, isolated_home):
        """Windows path resolution returns the ON-DISK casing regardless of the requested casing (verified live: resolving 'MODEL.GGUF' against an existing 'model.gguf' yields dest.name == 'model.gguf') - a benign case-variant request for an already-downloaded file must not be mistaken for an alias-substitutio..."""
        base = tmp_path / "models"
        base.mkdir()
        (base / "model.gguf").write_bytes(b"CONTENT")
        assert mm._safe_models_filename("MODEL.GGUF", base) == "MODEL.GGUF"
...
class TestGetModelInfoPathologicalName:
    """get_model_info must return None for a pathological/nonexistent name, never crash run/benchmark/serve."""
