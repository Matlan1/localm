...
issue #1578. appface._com_init/_com_uninit and their wiring into _run are
the fix. Shared, module-level (not per-class) because run_native_window has
the identical need - see test_appface_native_window_com.py.
"""
...
def test_com_uninit_with_none_is_a_safe_noop():
    appface._com_uninit(None)   # must not raise
...
def test_com_init_returns_none_and_does_not_raise_off_windows_or_on_failure(
    ...
    _com_init must degrade to None rather than raise into the caller."""
...
def test_com_init_returns_the_ole32_handle_on_success(monkeypatch):
    ...
    fake_ole32.CoInitializeEx.assert_called_once_with(
        None, appface._COINIT_APARTMENTTHREADED)


def test_com_uninit_calls_couninitialize_on_the_returned_handle():
    fake_ole32 = MagicMock()

    appface._com_uninit(fake_ole32)
...
def test_com_uninit_never_raises_even_if_couninitialize_does():
    ...
    appface._com_uninit(fake_ole32)   # must not raise
...
def test_real_coinitializeex_on_a_fresh_thread_succeeds():
    ...
    def _worker():
        ole32 = appface._com_init()
        result["ole32"] = ole32
        appface._com_uninit(ole32)
