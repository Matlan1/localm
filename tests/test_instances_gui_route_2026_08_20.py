...
"""GET /api/instances + POST /api/instances/{id}/stop - the GUI form of `localm ps` / `localm stop <id>` (PARITY-AUDIT-CLI-GUI-2026-08-19.md, CLI-only gap #7)."""
...
def instances_app(tmp_path, monkeypatch):
    """Full stack (kernel + GUI) on a throwaway home. instances.py/auth.py both resolve LOCALM_HOME lazily via config.home_dir() on every call, so setting the env var is enough - unlike the model registry, nothing here caches a module-level path constant that would also need patching."""
...
def _write_entry(home, **kw):
    """Write a registry entry file directly, for an arbitrary (possibly fake) pid - instances.register_instance() always uses THIS process's own os.getpid() (it is meant for self-registration; see its docstring), so it cannot represent a different target process the way every test below needs to."""
    import json
    import os
...
class TestInstancesList:
    ...
    def test_bracketed_ipv6_address(self, instances_app):
        """The Address column must not merge an IPv6 literal's colons with the port separator (localm.bindhost.url_host's whole reason to exist) - mirrors cli/models.py's ps_cmd bracketing, same source of truth."""
        app, home = instances_app
...
class TestStopMatching:
    ...
    def test_a_short_unambiguous_prefix_matches(self, instances_app, monkeypatch):
        """Same id-prefix semantics as `localm stop <id>` - the 404/400 tests above already prove the disambiguation branch; this proves the happy path resolves a prefix to exactly one entry rather than requiring the caller to type the whole id. pid=-1 makes kill_pid report the target already gone (test_instan..."""
        app, home = instances_app
...
class TestAdminGate:
    def test_a_config_write_only_key_is_refused(self, instances_app):
        """The gate this whole route exists to get right: config:write is privileged but is NOT the owner, and must not reach a DIFFERENT instance's process - see the module docstring's CHK-UPDATE-ROLLBACK comparison in localm/plugins/gui/routes/instances.py."""
        app, home = instances_app
    ...
    def test_open_mode_passes_the_gate(self, instances_app, monkeypatch):
        """No key configured anywhere -> the trusted local owner, same as every other owner-gated route (CHK-UPDATE-ROLLBACK's own reasoning)."""
        app, home = instances_app
        monkeypatch.setattr(instances, "reap_stale", lambda *a, **k: [])
...
class TestStopRealHttp:
    def test_graceful_shutdown_reaches_the_real_target_route(
        ...
        """The real target's /v1/server/shutdown must actually be invoked - not merely a 200 from a mock. _do_shutdown's real body ends in os._exit(0); replaced with a recorder so this test does not kill the pytest worker, same substitution test_cli_unload_stop_open_mode_auth.py makes for the identical reason."""
        app, home = instances_app
        shutdown_calls = []
...
class TestStopGracefulDeclined:
    def test_a_declined_graceful_shutdown_falls_back_to_a_real_kill(
            self, instances_app, monkeypatch):
        """The 401/403 branch specifically (distinct from 'unreachable', covered separately by TestStopKillFallback against a real closed port): the target answers, but declines. graceful_denied must be reported True, and the route must still fall through to a real kill rather than treating the decline as fata..."""
        app, home = instances_app
...
class TestStopKillFallback:
    def test_an_unreachable_target_falls_back_to_killing_the_real_process(
            self, instances_app):
        """No server is listening on the registered port at all (connection refused, not a slow timeout) - the route must still confirm the stop by falling back to instances.kill_pid against a REAL subprocess, mirroring test_instances.py's own test_kill_pid_terminates_a_real_live_process rather than asserting..."""
        app, home = instances_app
...
class TestStopUnconfirmed:
    def test_502_when_neither_graceful_nor_kill_confirms(
            self, instances_app, monkeypatch):
        """instances.kill_pid itself is exhaustively tested for real in test_instances.py (including a real live process and timeout behaviour) - this test is about the ROUTE's response when that primitive genuinely cannot confirm, which is cheapest to force directly rather than construct a real unkillable pro..."""
        app, home = instances_app
        closed = _socket.socket(_socket.AF_INET, _socket.SOCK_STREAM)
        closed.bind(("127.0.0.1", 0))
        dead_port = closed.getsockname()[1]
