...
class TestConfinedUnder:
    ...
    def test_nesting_never_silently_renames(self, base):
        """The property the bug above violated: whatever the caller named as the final component is what comes back."""
    ...
    def test_drive_relative_is_rejected_on_every_platform(self, base):
        """Judged by WINDOWS rules regardless of host: a remote-supplied name must be judged by what it would mean on the worst platform, not the running one."""
        with pytest.raises(ValueError):
            confined_under(base, "Q:evil")

    def test_symlink_out_of_base_is_rejected(self, base, tmp_path):
        """Lexical checks are not sufficient: a symlink INSIDE base pointing out of it turns a well-formed name into an escape, which is why the check is on the RESOLVED location."""
    ...
    def test_backslash_is_a_separator_on_every_platform(self, base):
        """Matches _apply_update._unsafe_member."""
        with pytest.raises(ValueError):
    ...
    def test_a_rejected_path_reaches_NO_filesystem_call(self, base, bad,
                                                        monkeypatch):
        """ORDER, not verdict: a rejected input must be refused BEFORE any syscall, not after one."""
        seen = []
    ...
    def test_does_not_require_the_target_to_exist(self, base):
        """Callers check existence themselves (a delete target may already be gone); confinement must not depend on it."""
    ...
    def test_reserved_characters_are_rejected(self, base, bad):
        """Same NTFS Alternate Data Stream class #1068 fixed in model_manager/gguf.py's _safe_models_filename, and confined_name above - unfixed here until now."""
        with pytest.raises(ValueError):
            confined_under(base, bad)
    ...
    def _mock_alias_resolve(monkeypatch, alias_name, real_name):
        """Simulate an NTFS 8.3 short name: whenever a path being resolved has *alias_name* as one of its COMPONENTS - leaf or intermediate - replace that component with *real_name* before resolving for real."""
    ...
    def test_alias_intermediate_component_is_rejected(self, base, monkeypatch):
        """The leaf isn't the only place an alias can hide: `subfolder` is ComfyUI's own nesting feature, and a short name there resolves the same way a filename does."""
    ...
    def test_alias_rejection_matches_confined_name_wording(self, base, monkeypatch):
        """Not load-bearing on exact text, just confirms the new branch is what actually fires (not some other rejection) so this test cannot pass for the wrong reason."""
        victim = base / "LongModelNameThatIsVeryLong.gguf"
        victim.write_text("victim", encoding="utf-8")
        alias = "LONGMO~1.GGU"
        self._mock_alias_resolve(monkeypatch, alias, victim.name)
...
class TestConfinedAbsoluteOrUnder:
    ...
    def test_unc_reaches_no_filesystem_call(self, base, monkeypatch):
        """Same ordering discipline as confined_under's own test: the SMB dial is the vulnerability, so the refusal must come before resolve()."""
        seen = []
        real_resolve = Path.resolve
        monkeypatch.setattr(
            Path, "resolve",
    ...
    def test_alias_leaf_absolute_is_rejected(self, base, monkeypatch):
        """The same substitution, but *raw* is the ABSOLUTE form - the branch confined_under does not have at all."""
        victim = base / "LongModelNameThatIsVeryLong.gguf"
        victim.write_text("victim", encoding="utf-8")
        alias = "LONGMO~1.GGU"
        self._mock_alias_resolve(monkeypatch, alias, victim.name)
...
class TestIsUncOrDevicePath:
    ...
    def test_true_on_every_platform(self, raw):
        """NOT gated on os.name: this answers what the string MEANS, not where it is evaluated."""
    ...
    def test_false_for_ordinary_paths(self, raw):
        """Corpus contributed by the WS7 lane, which consumes this predicate for remote-supplied embedding specs."""
...
class TestRejectUnsafePathString:
    ...
    def test_backslash_unc_rejected_on_every_platform(self, raw):
        """Backslash-led forms are refused everywhere: a leading '\\\\' or '\\/' is not a meaningful local-path prefix on POSIX either, so nothing legitimate is lost by refusing them there."""
        with pytest.raises(ValueError):
    ...
    def test_forward_slash_unc_is_platform_split_by_design(self):
        """``//x`` is UNC on Windows but a LEGAL local prefix on POSIX (equivalent to ``/``)."""
        if os.name == "nt":
            with pytest.raises(ValueError):
    ...
    def test_makes_no_filesystem_call(self, monkeypatch):
        """The entire point: the syscall IS the vulnerability, so the check must run before one can happen."""
...
class TestIsMappedNetworkDrive:
    ...
    def test_false_without_any_win32_call(self, raw, monkeypatch):
        """No drive-letter prefix (or a non-Windows platform) must short- circuit before ever touching ctypes - proven by making the Win32 call itself explode if reached."""
        import ctypes
    ...
    def test_false_on_non_windows(self, monkeypatch):
        """A stale os.name check must be provably load-bearing, not merely coincidental with this machine's real drive table."""
        if os.name == "nt":
            import ctypes
    ...
    def test_drive_relative_spelling_still_queries_the_drive(self, monkeypatch):
        """'Q:x' (drive-relative, no separator) genuinely names drive Q - ntpath.splitdrive('Q:x') == ('Q:', 'x'), matching is_unc_or_device_path's own test corpus comment for the same string."""
        import ctypes
        seen = []
        monkeypatch.setattr(ctypes.windll.kernel32, "GetDriveTypeW",
    ...
    def test_real_local_drive_is_not_reported_remote(self, tmp_path):
        """Smoke test against the actual dev box, no mocking: tmp_path's own drive is a real local disk, never a network share."""
    ...
    def test_classification_gap_is_real(self, monkeypatch):
        """Pins the S9 finding itself: is_unc_or_device_path's own docstring says 'Z:' is the ordinary local-drive form, and it must keep saying so (this predicate's contract does not change) - is_mapped_network_drive is what tells a REMOTE 'Z:' apart from a local one, which is_unc_or_device_path was never des..."""
        raw = r"Z:\shared\docs"
        assert not is_unc_or_device_path(raw)
        if os.name == "nt":
            import ctypes
...
class TestRejectUnsafePathStringNetworkDrives:
    ...
    def test_ordinary_drive_allowed_regardless_of_the_flag(self, tmp_path):
        """Control, matches TestRejectUnsafePathString.test_makes_no_filesystem_ call's existing pin byte-for-byte: an ordinary drive letter must never be rejected merely because the new keyword exists."""
        reject_unsafe_path_string(r"Q:\ordinary")
    ...
    def test_real_local_drive_never_rejected_even_when_opted_in(self, tmp_path):
        """Control against this box's real (non-network) drive: opting in must not turn into a blanket rejection of every drive letter."""
