...
def _wire_http(monkeypatch, head_total: int, response):
    """Double netpolicy.pinned_request (the SSRF-REBIND transport seam the pull path now uses); return a dict that captures the GET headers."""
    captured = {}
...
class TestUrlPullResult:
    ...
    def test_size_head_policy_refusal_fails_closed(self, url_env, monkeypatch, capsys):
        """A NetworkPolicyError at the size-HEAD stage must SURFACE and fail closed, not be silently collapsed into total=0 (AGENTS.md rule 5: distinguish the benign connect error from a policy refusal)."""
