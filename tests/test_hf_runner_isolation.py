...
class TestLoadCrashContainment:
    ...
    def test_bad_load_payload_is_a_clean_error_not_a_crash(self):
        """The HFWorker(**payload) constructor call sits INSIDE the 'load' branch's try/except (see _hf_runner.py), so a malformed payload - a parent/child protocol bug, not a native fault - is a normal, clean error, not an uncaught crash that kills the process for no native reason at all."""
        r = HFRunner()
...
class TestChatStreamCrashContainment:
    def test_dispatch_crash_during_chat_stream_is_contained(self):
        """An uncaught exception ANYWHERE in the child's dispatch loop - not just a real native fault - must crash only the child, never the caller."""
        r = HFRunner()
        r._spawn()
        try:
            with pytest.raises(RuntimeError) as ei:
...
class TestSimpleRequestCrashContainment:
    ...
    def test_embed_crash_is_contained(self):
        """Same as test_count_tokens_crash_is_contained, for embed() - a real production call path (HFBackend.embed) that GgufRunner has no equivalent of (GGUF's can_embed is a fixed False), so it has no upstream test to mirror and needs its own coverage."""
        r = HFRunner()
        r._spawn()
        r._proc.kill()
        r._proc.join(timeout=5)
...
class TestCrashDiagnosticsReachDebugLog:
    """Mirrors test_gguf_runner_isolation.py's identical test: the parent's crash message points the user at the debug log, so the worker's actual exception detail must really reach that log, not just the parent's own generic message."""
...
class TestNativeSignalCrashDiagnosticsReachDebugLog:
    ...
    def test_load_crash_also_reports_its_captured_trace(self, monkeypatch):
        """The load path builds its own crash message, separately from chat_stream's, so it needs its own proof - a fix applied to one of two hand-written message sites is a fix to half the problem."""
        monkeypatch.setenv(runner_mod._FAULT_ENV, "abort")
        r = HFRunner()
        try:
            with pytest.raises(RuntimeError) as ei:
    ...
    def test_hard_killed_worker_does_not_imply_a_trace(self):
        """A worker killed outright (SIGKILL / TerminateProcess) runs no handler at all, so faulthandler cannot write anything - and the simple-RPC crash message must not imply otherwise."""
        r = HFRunner()
        r._spawn()
        r._proc.kill()
...
class TestExecutorPoolReclaim:
    def test_hung_call_through_run_in_executor_frees_its_pool_slot(self, monkeypatch):
        """Drive a hung HFBackend.load() through loop.run_in_executor(pool, ...) exactly as production code does (routes/chat.py:99, http_server.py's _generate_full via HFBackend.chat_stream, etc.), on a deliberately tiny (2-worker) pool so exhaustion is observable without needing 16 concurrent hangs."""
        monkeypatch.setenv(runner_mod._FAULT_ENV, "hang")
...
class _AliveProc:
    """Stands in for the worker process's liveness check only; the queues and the parent-side poll/drain loop under test are real."""
...
class TestCooperativeCancel:
    def test_cancel_stream_gets_done_without_killing_the_worker(self):
        """A worker that cooperates with cancel_stream (streams a couple of chunks, then confirms 'done' once it sees the ctrl_q signal - what the real control-thread + StoppingCriteria produce) must leave the process ALIVE."""
        r = _make_runner()
    ...
    def test_drain_timeout_falls_back_to_killing_the_worker(self, monkeypatch):
        """If the child never confirms cancel_stream (a genuinely wedged native call - the same uninterruptible-from-Python risk this module exists to contain), the drain must NOT assume success: it falls back to killing the worker, mirroring ModelRunner's identical fallback (tests/test_runner_stream_timeouts...."""
...
class TestCtrlMsgCancelsSeq:
    """_ctrl_msg_cancels_seq is what _control_loop calls to decide whether a drained ctrl_q message should actually stream_cancel_event.set() - the exact logic that closes the stale-cancel race a fresh review found: ctrl_q and req_q are independent queues, so a cancel meant for a stream that already finish..."""
    ...
    def test_stale_seq_does_not_cancel(self):
        """The exact scenario the fix exists for: a cancel meant for stream 7 drained after the dispatch loop has already moved on to stream 8 must NOT cancel stream 8."""
        assert _ctrl_msg_cancels_seq(("cancel_stream", 7), 8) is False

    def test_no_current_stream_does_not_cancel(self):
        """Before the first stream ever starts, current_seq is None - a cancel_stream (which always carries a real int seq from the real HFRunner) must never match a None current_seq."""
        assert _ctrl_msg_cancels_seq(("cancel_stream", 1), None) is False

    def test_missing_seq_on_the_message_does_not_cancel(self):
        """A malformed/legacy 1-tuple message carries no target seq at all - target_seq is None, and None must never match, even if current_seq also happens to be None (that would defeat the whole protection when neither side supplies a real seq)."""
...
class TestCancelCriteriaUnit:
    """Direct, model-free unit test of _hf_worker.py's _CancelCriteria - the StoppingCriteria transformers' generate() loop actually calls once per decode step."""
    ...
    def test_result_survives_stopping_criteria_list_or_reduction(self):
        """The actual contract that matters: StoppingCriteriaList.__call__ does `is_done = is_done | criteria(...)` starting from an all-False bool tensor - reproduce that reduction directly against a real StoppingCriteriaList (not just _CancelCriteria in isolation), so a future change to the class that techni..."""
        from localm.inference.backends.llamacpp import _loader
        if _loader.native_lib_loaded():
            pytest.skip("llama.cpp's native runtime is already loaded in this "
                         "process (a real compute-device probe ran earlier in "
