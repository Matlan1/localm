...
class TestFreshContextVramCheck:
    """CHK-KVCACHE-OVERFLOW (growth path): _prefill_fresh_context() recreates a BIGGER context whenever a conversation outgrows the live one - which happens on literally the first prompt for anyone on default settings, since the default max_tokens (4096) already exceeds the default base n_ctx (4096), forci..."""
...
class TestUnlimitedCtxMax:
    """R04: n_ctx_max=0 ('unlimited') must lift the _AUTO_CTX_MAX safety clamp when ctx_auto is on, so the window grows to the full VRAM-derived budget instead of being capped at 65536."""
