...
"""GgufBackend._vram_holder_hint(): the low-VRAM warning's 'who is holding this VRAM' attribution."""
...
class TestVramHolderHint:
    def test_self_is_never_blamed_as_another_instance(self, tmp_path, monkeypatch):
        """The ONLY registry entry on this GPU is this same process's own (pid == os.getpid())."""
    ...
    def test_genuine_other_instance_is_still_named(self, tmp_path, monkeypatch):
        """A live, identity-verified peer on a DIFFERENT pid must still be named exactly as before - the fix must not blind the hint to real siblings, only to itself."""
        d = _stub_registry(monkeypatch, tmp_path)
        gpu_registry.write_entry(
    ...
    def test_self_and_peer_both_present_names_the_peer(self, tmp_path, monkeypatch):
        """Self plus a genuine peer both hold entries on this GPU: the peer must win the attribution (it is the one a POST-unload can actually reach usefully). instance_id is chosen so the SELF entry sorts FIRST alphabetically (list_entries() reads entries in sorted filename order) - without pid-based exclusio..."""
        d = _stub_registry(monkeypatch, tmp_path)
        gpu_registry.write_entry(
    ...
    def test_self_entry_on_a_different_gpu_index_is_not_blamed(self, tmp_path, monkeypatch):
        """A self entry exists but on a different GPU device than the one being sized for - it must not be offered as the holder of THIS device's VRAM."""
        d = _stub_registry(monkeypatch, tmp_path)
        gpu_registry.write_entry(
            d, instance_id="self-iid", pid=os.getpid(), port=8642,
            host="127.0.0.1", scheme="http", model="gemma-4-12b",
