...
markup, not just inside a command's own literal ``[style]`` tags:
...
originate from the ComfyUI backend, a local but SEPARATE process whose content
this repo does not control: ``comfy_exec_error_message()`` echoes a
POLL_EXEC_ERROR payload straight from ComfyUI, and a preflight substitution
notice embeds model filenames read from ComfyUI's own ``/object_info``.

Each ``generate_image``/``generate_music``/``generate_video`` call is faked at
the CLI's own generator entry point, so the real print statements in media.py
execute unmodified and only the ComfyUI round-trip is replaced with a realistic
payload.

``_open_file``'s ``except Exception as e`` escape (media.py's 7th site) has no
dedicated test here: it is best-effort UI sugar reachable only through an
interactive terminal AND a failing OS file-open call, and shares the
``escape(str(e))`` shape TestGenerateOrAbortExceptionMarkupEscaping covers.
"""
...
class TestMusicCmdMarkupEscaping:
    ...
    def test_progress_text_survives_verbatim(self):
        ...
        never invoked by real ComfyUI-talking code. This calls the SAME
        on_progress kwarg music_cmd passed in, media.py's own lambda, with the
        text a real preflight substitution notice carries (comfy_client.py's
        preflight_models() embeds ComfyUI-reported model filenames)."""
...
class TestGenerateOrAbortExceptionMarkupEscaping:
    def test_interrupt_failure_message_survives_verbatim(self):
        ...
        failure to reach ComfyUI (interrupt_comfy/free_comfy_vram raising). A
        real KeyboardInterrupt is forced out of the generator and a real
        exception out of interrupt_comfy, rather than mocking the display
        layer."""
