...
class TestPosixSignalDecoding:
    ...
    def test_each_fault_family_is_named_distinctly(self, code, name):
        """Naming them is only useful if they are told APART - a decoder that collapsed these would be no better than the raw number, since choosing between these families is the entire diagnostic value."""
        assert name in describe_exit_code(code, posix=True)

    def test_a_clean_nonzero_exit_is_not_read_as_a_signal(self):
        """A non-negative POSIX code is an ordinary exit status and says nothing about signals."""
        out = describe_exit_code(4, posix=True)
    ...
    def test_a_negative_that_is_not_a_signal_says_what_is_known(self):
        """-99 is not a signal on any supported build."""
        out = describe_exit_code(-99, posix=True)
        assert "-99" in out
        assert "None" not in out

    def test_posix_codes_are_not_decoded_through_the_host_signal_enum(self):
        """REGRESSION GUARD for a real defect this file caught, so nobody 'simplifies' the decoder back to a bare signal.Signals() lookup."""
...
class TestWindowsNtstatusDecoding:
    def test_negative_is_not_read_as_a_signal_on_windows(self):
        """THE REASON THE TWO BRANCHES ARE SPLIT."""
        out = describe_exit_code(-1, posix=False)
        assert "SIGHUP" not in out
        assert "signal" not in out.lower()

    def test_the_measured_native_abort_code_is_named(self):
        """3221226505 == 0xC0000409, measured from a real os.abort() on this project's own CRT while building the worker crash-trace capture."""
    ...
    def test_illegal_instruction_is_named_on_windows_too(self):
        """The Windows equivalent of the reported SIGILL, so the same class of report from a Windows tester is equally readable."""
    ...
    def test_the_dll_conflict_code_self_identifies(self):
        """0xC0000139 is the documented signature of the torch/HIP DLL-identity conflict this codebase already guards against."""
...
class TestNativeFaultClassification:
    ...
    def test_an_ordinary_posix_exit_is_not(self, code):
        """1 is the one that mattered: multiprocessing's signature for an uncaught Python exception, and the code in the false message."""
    ...
    def test_an_ordinary_windows_exit_is_not(self, code):
        """-1 included on purpose: Process.terminate() produces it on Windows, and 3 is what an ARMED-faulthandler abort exits with - neither is classifiable from the code alone, so neither may be asserted as native."""
        assert not death_was_a_native_fault(code, posix=False)

    def test_a_captured_trace_settles_it_on_either_platform(self):
        """The strongest evidence, and the reason it leads rather than tiebreaks: faulthandler only fires on SIGSEGV/SIGFPE/SIGABRT/SIGBUS/SIGILL, so a trace means a native signal even when the exit code cannot say so - which is exactly the Windows armed-abort-exits-3 case."""
    ...
    def test_a_real_python_exception_death_is_not_called_a_native_fault(self):
        """END TO END, through a REAL child, on the exact fault the false message was reported for: chat_stream before any load -> worker is None -> AttributeError -> uncaught -> exit 1."""
        r = ModelRunner()
        r._spawn()
        try:
...
class TestNeverRaises:
    """This decorates a message on a path that is ALREADY failing."""
    ...
    def test_none_is_explicit_not_the_string_none(self):
        """_exitcode() returns None once the child is released. 'exit code None' reads like a bug in the reporter."""
        assert describe_exit_code(None) == "unknown"


class TestRunnerReportsTheDecodedCode:
    """The decoder existing is not the property; the RUNNER using it is."""
    ...
    def test_a_real_native_death_reports_a_decoded_code_end_to_end(
            self, monkeypatch):
        """The behavioural half, driven through a REAL child process killed by a REAL native abort - not a fake exitcode."""
    ...
    def test_no_user_facing_message_interpolates_the_raw_exitcode(self):
        """A grep-style guard on the source itself."""
        import inspect
