...
"""Tests for scripts/check_coverage_floors.py - the per-module trust-boundary coverage ratchet that complements pyproject.toml's global fail_under."""
...
class TestCheckFloors:
    ...
    def test_passes_at_exactly_the_floor(self):
        """The comparison is '<', not '<=': a module sitting exactly on its floor must not fail - only a genuine drop BELOW it should."""
        floors = {"localm/a.py": 90}
        report = {"files": {"localm/a.py": _entry(90.0)}}
        assert ccf.check_floors(report, floors) == []

    def test_fails_when_a_floored_module_is_missing_from_the_report(self):
        """A module absent from coverage.json entirely (never imported/exercised by the run) is a worse signal than a low score and must not pass silently."""
        floors = {"localm/a.py": 90}
        report = {"files": {}}
        problems = ccf.check_floors(report, floors)
        assert len(problems) == 1
    ...
    def test_windows_backslash_keys_are_matched_not_reported_missing(self):
        """REGRESSION (found live, 2026-07-29): coverage.json's file keys are the file_reporter's relative_filename(), which uses the HOST's native path separator."""
    ...
    def test_real_module_floors_pass_against_their_own_measured_baseline(self):
        """Each shipped floor is one point below the ACTUAL value from a real --cov run on merged master (1fa3af2c, 2026-07-29, Windows, FROM A WORKTREE) - see the module docstring for why a rounded display integer is not safe to floor against directly (netpolicy and portmux both would have failed on day one a..."""
        baseline = {
    ...
    def test_every_shipped_floor_is_one_point_below_its_measured_baseline(self):
        """The CONVENTION itself, asserted as arithmetic rather than trusted to stay true by hand."""
        baseline = {
            "localm/bindhost.py": 100.0,
            "localm/scopes.py": 100.0,
            "localm/pathsafe.py": 93.1034,
...
class TestMain:
    ...
    def test_failing_report_exits_one_and_names_module_floor_and_actual(self, tmp_path, capsys):
        """The failure message contract this script promises (AGENTS.md rule 5: a gate must name what failed, not just that something did)."""
...
class TestReportRows:
    ...
    def test_absent_module_reports_none_rather_than_a_fabricated_zero(self):
        """Absent and zero are different facts: zero means the module was measured and nothing ran, None means the run never saw it at all."""
        floors = {"localm/a.py": 90}
        assert ccf.report_rows({"files": {}}, floors) == [("localm/a.py", None, 90)]

    def test_windows_backslash_keys_are_matched_here_too(self):
        """Same host-separator hazard check_floors carries."""
        floors = {"localm/pathsafe.py": 90}
...
class TestReportFlag:
    ...
    def test_report_does_not_suppress_a_real_failure(self, tmp_path, capsys):
        """THE POINT OF THE FLAG'S DESIGN. --report is additive: it prints and then enforces."""
    ...
    def test_report_names_a_floor_that_has_drifted_far_below_reality(
        ...
        """The stranding this flag exists to prevent: a floor left far under its module's real value protects nothing, and nothing else surfaces it."""
    ...
    def test_a_floor_at_its_intended_one_point_gap_is_not_called_stale(self, tmp_path, capsys):
        """FIRES-CONTROL for the staleness notice: the shipped convention is floor = int(measured) - 1, so an ordinary well-maintained floor sits just under two points below and must NOT be flagged."""
    ...
    def test_report_flag_works_without_an_explicit_path_argument(self, tmp_path, capsys, monkeypatch):
        """--report must not be mistaken for the positional coverage.json path; stripping the flag has to leave the default-path branch reachable."""
...
class TestHomeDirConfounderNote:
    """The failure message invites lowering a floor."""
    ...
    def test_no_note_without_a_home_dir(self, tmp_path, capsys, monkeypatch):
        """FIRES-CONTROL: in a worktree or CI clone there is no `home/`, so a config.py failure there is a REAL regression and must not be softened by a hint that points at the wrong cause."""
        repo = tmp_path / "repo"
    ...
    def test_no_note_for_an_unrelated_module_even_with_home_present(
            self, tmp_path, capsys, monkeypatch):
        """The confounder is measured for config.py only."""
        repo = tmp_path / "repo"
        (repo / "home").mkdir(parents=True)
        monkeypatch.setattr(ccf, "REPO", repo)
