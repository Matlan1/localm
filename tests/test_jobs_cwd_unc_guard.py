...
"""A coder job's ``cwd`` reached Path.expanduser()/.is_dir()/.resolve() in runner._run_coder with only a non-empty check in front of it (store.py's 'coder jobs require a cwd'). is_dir()/.resolve() are not read-only: on Windows a UNC target dials outbound SMB and auto-authenticates with the host's net-N..."""
...
# A UNC path at a non-routable RFC5737 documentation address (TEST-NET-1) - never
# routes anywhere, so even a total failure of the fix cannot reach a real host
# from a developer's machine or from CI. Mirrors tests/test_admin_fs_routes.py.
...
def fs_spy(monkeypatch):
    """Record every path string that reaches a filesystem call, and hard-fail the call when it is UNC/device syntax."""
    seen: list = []
...
def _fake_agent_capture(monkeypatch):
    """Patch the coder backend + Agent so _run_coder runs for real up through the cwd guard, without needing a real coder backend or LLM."""
...
# --------------------------------------------------------------------------- #
#
# These are EARLY-REJECTION tests, not syscall-avoidance tests: neither route
# ever calls Path.is_dir()/.resolve() on cwd at write time (only _run_coder
# does, at run time), so there is no filesystem call to spy on here, fixed or
# not - only the status code changes. Do not "strengthen" these with an
# fs_spy assertion; it would never fire and would look like coverage it is
# not. The syscall-avoidance proof lives in section (3)/(4) below, against
# run-now and the scheduler tick, where the guarded call actually happens.
...
def test_put_jobs_accepts_ordinary_cwd_change(home, tmp_path, monkeypatch):
    """Fires-control for PUT specifically: a legitimate cwd CHANGE (not just 'the original value survives a rejected update') must actually apply."""
    from localm.plugins.builtin.jobs.store import JobStore
...
# --------------------------------------------------------------------------- #
#
# Same caveat as section (1): early rejection only, no filesystem call to spy
# on here (job_add never touches cwd via Path before persisting it).
...
def test_run_now_refuses_a_persisted_unc_cwd_without_touching_filesystem(
    ...
    """A row poisoned straight in the store - exactly what a build predating the write-time gate, or a direct store write, would leave behind - must still be refused when run on demand, and the fs boundary must never see the UNC string."""
    from localm.plugins.builtin.jobs.store import JobStore
    ...
    with _jobs_client(home, monkeypatch) as c:
        ...
        assert r.status_code == 200, r.text
        jid = r.json()["id"]
        # Poison it directly in the store, bypassing plug.py's PUT gate - the
        # "row predates the write-time check" scenario the finding is about.
...
# --------------------------------------------------------------------------- #
#
# This is the primary reachability concern the finding is about: the scheduler
# fires roughly every 30s with NO caller at all, so a hostile cwd stored once
# keeps dialing SMB on a timer until someone notices. A fix verified only
# through POST /run would leave this path, the one that actually recurs
# unattended, unverified.

@pytest.mark.parametrize("bad", [UNC, UNC_FWD, DEVICE])
def test_scheduler_tick_refuses_a_persisted_bad_cwd_without_touching_filesystem(
        home, monkeypatch, fs_spy, bad):
    """Drives the REAL scheduler tick -> real runner.run_job -> _run_coder (only the coder Agent/backend are faked), exactly like test_jobs_shell_key_liveness.py's scheduler test - so this exercises the actual autonomous path with no request/caller in sight."""
    ...
    store = JobStore()
    # Persisted the way an older build (or a direct store write) would have -
    # bypassing plug.py's _check_cwd entirely.
...
def test_scheduler_tick_runs_ordinary_cwd_normally(home, tmp_path, monkeypatch):
    """Control: with an ordinary cwd, the scheduler tick runs the job for real, proving the refusal above is specific to UNC/device syntax."""
    from localm.plugins.builtin.jobs.scheduler import JobScheduler
    from localm.plugins.builtin.jobs.store import JobStore
