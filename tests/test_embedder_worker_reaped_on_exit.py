...
def test_release_for_exit_kills_a_real_busy_worker_process():
    """The release must terminate the real child."""
...
def test_release_for_exit_never_takes_the_load_lock():
    """The whole point of this function (REG-650 follow-up): every other way to make the release decision - active_requests(), reset_embedder() - takes _LOCK, which get_embedder() holds for the FULL duration of an embedding-model load."""
...
def test_idle_embedder_is_closed_politely(monkeypatch):
    """An IDLE worker still gets the polite shutdown command (so the child frees the model cleanly), just without taking the load lock: the exiting process has no use for reset_embedder()'s singleton clearing, and reaching for it is what made this path able to hang (see test_stop_during_an_embedder_load_ d..."""
    runner = _RecordingRunner()
