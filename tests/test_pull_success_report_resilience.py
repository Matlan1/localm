...
"""Item #25: a genuinely successful pull was reported as FAILED, because the final status print - a bare console.print of a green checkmark - crashed AFTER the download, checksum verification and registry write were already done."""
...
class TestCliPullExitCode:
    """The GUI job runner (localm/plugins/gui/jobs.py) decides success/failure from the SUBPROCESS EXIT CODE alone, not from any Python-level return value it never sees."""
...
class TestLocalPathSurvivesACrashingVerifiedPrint:
    """pull_model()'s local-path branch (pull.py, is_local_path with --sha256): the checkmark sits between the real digest check and the add_local() call that actually registers the file."""
...
class TestPullGgufFileSurvivesACrashingVerifiedPrint:
    """_pull_gguf_file()'s FAC-5 --sha256 branch: the checkmark sits between the real digest check on the freshly-downloaded bytes and the metadata probe + _register() call that follows it."""
...
class TestPullUrlSurvivesACrashingVerifiedPrint:
    """_pull_url()'s --sha256 branch: the checkmark sits between the real digest check on the downloaded bytes and the duplicate-check + _register() call that follows it (distinct from TestPullUrlSurvivesACrashingSuccessPrint above, which covers the function's TRAILING checkmark and never passes expected_s..."""
