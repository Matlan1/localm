...
"""Hang-capture instrumentation: the event-loop stall watchdog and the loopback-only GET /debug/stacks endpoint."""
...
def test_hang_watchdog_env_semantics(monkeypatch):
    """On by default (so a tester needs no setup); 0/false/off opts out; 1/true/on also turns on the verbose extras."""
...
def test_diagnostics_allowed_respects_privacy_and_toggle(monkeypatch):
    """Privacy mode writes no automatic trace unless keep_diagnostics is on; the log/full modes always allow it."""
...
def test_watchdog_quiet_while_loop_ticks(tmp_path, monkeypatch):
    """No stall -> no dump, and (lazy file) NO trace file created at all."""
    from localm.inference import http_server as hs
    trace = tmp_path / "hang.log"
...
async def test_watchdog_catches_a_real_event_loop_block(tmp_path, monkeypatch):
    """End-to-end: the REAL 1s heartbeat coroutine + the off-loop watchdog must catch a genuine event-loop block (a synchronous call that stalls the loop - exactly the diagnosed hang), not just a manually-staled heartbeat."""
    from localm.inference import http_server as hs
    trace = tmp_path / "hang.log"
...
def test_watchdog_skips_the_check_before_the_first_heartbeat_tick(tmp_path, monkeypatch):
    """The watchdog thread must not crash (TypeError subtracting None) or fabricate a stall dump against a baseline that was never real."""
    from localm.inference import http_server as hs
    trace = tmp_path / "hang.log"
    monkeypatch.setattr(hs, "_hb_monotonic", None)
    stop, thread = hs._start_hang_watchdog(
...
def test_watchdog_resumes_normal_checking_once_ticked_after_a_cold_start(tmp_path, monkeypatch):
    """Once _hb_monotonic transitions from None to a real value (the heartbeat's first tick), the watchdog must go back to detecting a genuine stall normally - the cold-start skip must not become permanent."""
    from localm.inference import http_server as hs
    trace = tmp_path / "hang.log"
    monkeypatch.setattr(hs, "_hb_monotonic", None)
    stop, thread = hs._start_hang_watchdog(
...
async def test_real_heartbeat_transitions_from_cold_start_to_a_real_reading(monkeypatch):
    """End-to-end with the REAL heartbeat coroutine (not a manually-set value): force a genuinely cold _hb_monotonic (simulating a fresh module import, since the module-level global otherwise persists real values across earlier tests in this same process), confirm loop_lag reads None while cold, then confi..."""
    from localm.inference import http_server as hs
...
def _heartbeat_starts_with_watchdog_thread_disabled(tmp_path, monkeypatch, *, config_json):
    """Shared drive for both configs below: builds a real app in a throwaway LOCALM_HOME, temporarily lifts the 'pytest' not in sys.modules guard for exactly this one lifespan run (restored by monkeypatch on teardown - see the module-level comment above _hb_monotonic for why this guard exists and why bypas..."""
    from localm.inference import http_server as hs
...
def test_heartbeat_starts_when_watchdog_env_disabled(tmp_path, monkeypatch):
    """Config A: LOCALM_HANG_WATCHDOG=0."""
...
def test_heartbeat_starts_in_privacy_mode_on_default_config(tmp_path, monkeypatch):
    """Config B: privacy mode, keep_diagnostics left at its default (False) - the documented, supported 'privacy mode, debug log on' combination (debuglog.py's debug_content_enabled docstring: operational lines stay on debug_enabled() even in privacy mode)."""
    monkeypatch.delenv("LOCALM_HANG_WATCHDOG", raising=False)
    hb_after, watchdog_calls = _heartbeat_starts_with_watchdog_thread_disabled(
...
def test_loop_lag_seconds_is_real_scheduling_delay_not_time_since_tick(monkeypatch):
    """Deterministic formula check: ~0 at every point in a healthy cycle - including right at the next tick boundary, the worst case for the raw (unfixed) formula - and the real overshoot only when a tick was genuinely late (the loop was actually blocked)."""
    from localm.inference import http_server as hs
...
async def test_loop_lag_seconds_stays_at_floor_within_a_measured_safe_window():
    """End-to-end proof, driving the REAL heartbeat coroutine (not a manually staled _hb_monotonic): every sample taken while LESS than one full interval has MEASURABLY elapsed since a confirmed real tick must read exactly 0.0 - the pre-fix formula would show up to ~1s of fake 'lag' here purely from sampli..."""
    from localm.inference import http_server as hs
...
async def test_loop_lag_seconds_is_clearly_positive_after_a_real_stall():
    """The other half of the property: _loop_lag_seconds() must not just unconditionally read 0.0 - a genuine event-loop block (not mid-cycle sampling) must produce a clearly positive value. time.sleep() blocks the real OS thread regardless of asyncio contention, so under real box load this block can only..."""
    from localm.inference import http_server as hs
...
def test_debug_request_log_renders_cold_start_as_na_not_zero(caplog, monkeypatch):
    """Before this fix, a request served before the heartbeat's first tick logged 'loop_lag=0.00s' - identical to a genuinely healthy reading, because the producer hid 'no reading yet' behind the same float a healthy loop reports."""
    monkeypatch.setenv("LOCALM_DEBUG", "1")
    from localm.inference import http_server as hs
    monkeypatch.setattr(hs, "_hb_monotonic", None)
...
def test_debug_request_log_renders_a_real_reading_as_before(caplog, monkeypatch):
    """The other half: once there IS a real reading, the log format is unchanged from before this fix (%.2fs), never 'n/a'."""
    monkeypatch.setenv("LOCALM_DEBUG", "1")
    from localm.inference import http_server as hs
...
def test_debug_stacks_renders_cold_start_as_json_null_not_zero(app, monkeypatch):
    """/debug/stacks' loop_lag_s field must be JSON null for a cold-start reading, never 0.0 - the same 'no reading yet' vs 'healthy' collision ADR-0008 U6 fixes at the debug-log site, checked here for the OTHER consumer so a fix to one call site cannot leave the other one wrong."""
    from localm.inference import http_server as hs
    monkeypatch.setattr(hs, "_hb_monotonic", None)
