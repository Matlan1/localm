...
"""localm.inference.http_server.rekey_loaded_model: re-keys every in-memory record of a loaded model's identity after its registry entry is renamed, so a still-loaded/serving engine is not orphaned under its old name."""
...
def test_rekey_also_moves_the_startup_and_last_active_pointers():
    """Found by the live repro, not by reasoning: after renaming the model a server was STARTED with, GET /v1/models still listed a row for the old name, because list_models adds _default_model_name whenever the registry lacks it."""
    _reset()
    eng = _FakeEngine("old")
    hs._engines["old"] = eng
...
def test_rekey_leaves_another_models_pointers_alone():
    """The pointers move only when they NAME the renamed model - renaming a background model must not steal the startup identity of a different one."""
    _reset()
...
def test_rekey_fixes_the_stale_active_model_guard_hazard():
    """The concrete hazard this function exists to close: active_model() reads _engine.display_name, and the GUI's remove-model guard is exactly `req.model == active_model()`."""
    _reset()
    eng = _FakeEngine("old")
    hs._engine = eng
    hs._engines["old"] = eng
...
def test_remove_still_allows_an_alias_of_a_loaded_file(models_home, gui_client):
    """Not over-refusing: while a SECOND registered name points at the file, removing one of them keeps the bytes (remove_model returns early), so there is no data loss to guard against and the request must still succeed."""
    app, started = gui_client
    gguf = _make_model_file(models_home)
...
def test_guard_ignores_an_unloaded_engine_and_an_unowned_path(models_home):
    """Two ways the guard must answer None, both of which would otherwise make it refuse removals that destroy nothing."""
    gguf = _make_model_file(models_home)
...
class _UnresolvableEngine:
    """A loaded engine whose path will not resolve - a UNC share that is momentarily unreachable, a permission error, an embedded NUL."""
...
def _raise_on_resolve(monkeypatch, bad: str, exc=OSError("unreachable")):
    """Make Path.resolve() raise for one specific path and behave normally for every other, so the test does not have to disable resolution process-wide (which would take the target side down with the engine side)."""
    import os as _os
...
def test_remove_refuses_when_a_loaded_engines_path_will_not_resolve(
    ...
    """THE fail-open: `except (OSError, ValueError): continue` skipped a LOADED engine and let the loop fall through to 'nothing holds this file'."""
    app, started = gui_client
    gguf = _make_model_file(models_home)
...
def test_remove_refuses_when_a_loaded_engine_has_no_recorded_path(
        models_home, gui_client):
    """The second fail-open, three lines up from the first: `if not mpath: continue`."""
    app, started = gui_client
    gguf = _make_model_file(models_home)
    _register(models_home, {"victim": {"path": str(gguf), "source": "local"}})
...
def test_remove_refuses_when_the_models_own_path_will_not_resolve(
        models_home, gui_client, monkeypatch):
    """The third case, and it is NOT a data-loss one - stated precisely because the other two are and it would be easy to bundle them."""
    app, started = gui_client
    gguf = _make_model_file(models_home)
    _register(models_home, {"victim": {"path": str(gguf), "source": "local"}})
...
def test_a_proven_holder_is_named_ahead_of_an_unresolvable_one(models_home):
    """Both refuse, but only one of them can say truthfully WHICH engine has the file, so the scan completes rather than returning the first unknown it trips over."""
    gguf = _make_model_file(models_home)
    _register(models_home, {"victim": {"path": str(gguf), "source": "local"}})
...
def test_an_unloaded_engine_with_a_broken_path_is_not_a_refusal(models_home):
    """Only LOADED engines can hold a file open, so a broken path on an unloaded one is not an unknown - it is irrelevant."""
    gguf = _make_model_file(models_home)
    _register(models_home, {"spare": {"path": str(gguf), "source": "local"}})
...
def test_guard_sees_the_startup_engine_that_is_not_in_the_engine_map(models_home):
    """`localm serve <path.gguf>` can leave the running engine as the module singleton without an _engines entry."""
    gguf = _make_model_file(models_home)
...
def _core_app(engine):
    """The real core app, carrying the per-instance attach token a local management client presents."""
...
def test_v1_rename_route_moves_the_registry_and_rekeys_the_engine(models_home):
    """On the /v1 surface, not only the GUI's /api one: a headless `localm serve` has no GUI routes, and it is exactly as capable of holding a model open."""
    gguf = _make_model_file(models_home)
...
def test_cli_rename_asks_a_running_server_instead_of_renaming_behind_its_back(
        monkeypatch):
    """The fix for the data loss: this process cannot re-key another process's engine map, so it must not move the registry entry on its own while a server is up."""
    import requests
...
def test_cli_rename_falls_back_locally_when_no_server_is_running(monkeypatch):
    """The ordinary offline case must be untouched: no server, no network call, plain local rename."""
    import requests
...
def test_cli_rename_does_not_redo_a_rename_that_a_lost_reply_hid(
    ...
    """A read timeout means the request WAS sent and the answer was lost, so the rename may well have been applied."""
    import requests
...
def test_cli_rename_falls_back_when_a_lost_reply_hid_nothing(
        models_home, monkeypatch, capsys):
    """The other half of the same unknown: the registry shows the rename was NOT applied, so the user's rename is still to do."""
    import requests
...
def test_cli_rename_stops_when_the_server_rejects_the_rename_itself(monkeypatch):
    """A 409 'name already taken' is the server's verdict on the rename, not a transport problem - retrying locally would move the entry the server just refused to move."""
    import requests
...
def test_cli_rename_treats_a_missing_route_as_fall_back_not_as_a_refusal(
        monkeypatch, capsys):
    """A server too old to have /v1/models/rename answers 404 with FastAPI's bare 'Not Found', which by STATUS alone is indistinguishable from this route's own 'Model not registered'."""
    import requests
...
def test_cli_rename_warns_out_loud_before_falling_back_past_a_live_server(
        monkeypatch, capsys):
    """A 401 (a keyed server this CLI has no credential for) still leaves the user's rename to do, so it happens locally - but that strands the running server on the old name, and saying nothing about it is exactly the silent degradation that produced this bug."""
    import requests
...
def test_cli_rename_over_real_http_rekeys_the_live_engine(models_home, monkeypatch):
    """End to end: `localm rename` against a REAL running server moves the registry entry AND re-keys the engine, so the file the engine is serving is not left orphaned under a name the registry no longer has."""
