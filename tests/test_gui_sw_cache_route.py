...
def fake_static(tmp_path, monkeypatch):
    """A static dir shaped like the real one: sw.js with the real placeholder shape, a couple of cacheable assets, and one path under the never-cached prefixes sw.js's own fetch handler returns early on."""
...
class TestDeterminism:
    ...
    def test_etag_is_identical_across_repeated_requests(self, fake_static):
        """The ETag is what actually gates the 304 in TestConditionalGet - a body that is byte-identical across requests proves nothing about caching efficiency on its own if the ETag it is hashed into varied per request (e.g. from something non-deterministic sneaking into the digest)."""
...
class TestChangeDetection:
    ...
    def test_renaming_a_file_changes_the_value(self, fake_static):
        """Same bytes, different path: still a real change to what a client sees at each URL, so it must not hash identically to the pre-rename state - the digest includes each file's path, not just its content."""
        before = web._compute_sw_cache_value()
    ...
    def test_editing_an_uncached_path_does_not_change_the_value(self, fake_static):
        """Mirrors sw.js's own fetch handler, which never caches /api/* (and /v1, /plugins, /localm-ca.crt) - a change there is correctly outside this digest's business."""
        before = web._compute_sw_cache_value()
    ...
    def test_editing_sw_js_itself_does_not_change_the_value(self, fake_static):
        """sw.js gates the others and cannot gate itself: its own bytes changing already triggers the browser's independent byte-diff reinstall check, and folding it into its OWN digest would only make the served value depend on comment/formatting churn unrelated to what a client has cached."""
        before = web._compute_sw_cache_value()
...
class TestMissingFileIsSafeNotSilent:
    """A file listed a moment ago but gone by read time (a genuine TOCTOU window, not simulated via chmod - Windows does not enforce POSIX-style unreadable-but- present permissions the way this failure mode needs) must still force a cache-bust rather than either crashing the request or silently serving a d..."""
...
class TestSubstitution:
    ...
    def test_two_matching_lines_fails_loud_not_a_silent_wrong_substitution(
            self, fake_static):
        """re.subn(..., count=1) reports n=1 whether the pattern matched once or several times (it caps how many it REPLACES, not how many it FOUND), so checking that return count alone cannot detect a second match - it would silently substitute the FIRST occurrence (e.g. an illustrative example string in a fu..."""
        (fake_static / "sw.js").write_text(
            'const CACHE = "localm-shell-dev";\n'
...
class TestConditionalGet:
    """_RevalidatingStatic's own contract for every other asset: no-cache does not mean no caching, an unchanged file still gets a cheap 304 via ETag."""
    ...
    def test_etag_reflects_sw_js_own_logic_changing_even_when_no_watched_asset_did(
            self, fake_static):
        """The ETag must be over the FINAL served body, not just the computed cache value - an edit to sw.js's own logic changes what is served even though _compute_sw_cache_value (which excludes sw.js itself) does not change, and a stale ETag there would wrongly 304 an actually-different response."""
        first = web._sw_js_response()
...
class TestRealStaticTree:
    """Pins the mechanism against the REAL, unmocked static/ directory - a synthetic-fixture-only suite could keep passing forever after a real regression (e.g. STATIC_DIR pointed somewhere empty)."""
