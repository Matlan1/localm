...
"""A5 idle-unload TTL: the inference server frees the model from VRAM after ``idle_unload_seconds`` of no request, then reloads it lazily on the next one."""
...
def _isolate_engine_registry(monkeypatch):
    """_idle_unload_once scans the _engines REGISTRY, not just _engine, and that registry is module state that outlives a test: switch_engine/get_engine write it and nothing clears it afterwards."""
