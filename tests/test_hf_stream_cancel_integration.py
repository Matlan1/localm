...
# contents) lets chat_stream's apply_chat_template run - prompt plumbing
# only, unrelated to what this test actually checks.
_MINIMAL_CHAT_TEMPLATE = "{% for m in messages %}{{ m['content'] }}\n{% endfor %}"


@pytest.fixture(scope="module")
def hf_backend(tmp_path_factory):
    # exc_type=ImportError: transformers' own internal tokenizers
    # version-gate, or a version-clashed torch build, can raise a plain
    # ImportError (not ModuleNotFoundError) for a reason other than "not
    # installed" - pytest 9.1 narrowed importorskip's default to
    # ModuleNotFoundError only, so without this these hard-fail/error
    # instead of skipping on exactly the case this skip exists for.
    ...
    # snapshot (never the shared HF hub cache directory snapshot_download
    # returned - mutating that would leak across every other test/use of
    # this cached model on the machine). Mirrors
    # test_hf_grammar_integration.py's hf_backend fixture exactly.
...
def test_midstream_close_keeps_the_same_worker_process_alive(hf_backend):
    ...
    # A cooperative cancel confirms within one drain poll of the child
    # stopping - generously bounded well under the 5s drain timeout so a
    # regression to kill-and-respawn (which also completes, just via a
    # different, still-fast path) does not make this assertion useless;
    # the real proof is the pid/liveness checks below, not this timing.
