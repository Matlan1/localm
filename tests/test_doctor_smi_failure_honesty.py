...
# The reported nvidia-smi wording for a driver updated without a reboot. Used
# verbatim because it is the realistic payload: a plausible-looking line that a
# stdout-only check happily renders as a device NAME.
...
def test_a_failing_smi_never_prints_its_error_as_a_device_name(monkeypatch, capsys):
    """The visible defect: a green tick reading 'NVIDIA GPU: Failed to initialize NVML...'."""
    _smi(monkeypatch, returncode=9, stdout=_NVML_ERROR)
...
def test_a_failing_smi_is_surfaced_not_silenced(monkeypatch, capsys):
    """Rule 5: surface it. 'Installed but broken' is actionable and must not be collapsed into the same silence as 'not installed'."""
    _smi(monkeypatch, returncode=9, stdout=_NVML_ERROR)
...
def test_a_tool_that_fails_with_only_stderr_still_reports_what_it_said(
        monkeypatch, capsys):
    """Some builds put the reason on stderr instead."""
...
def test_a_clean_smi_is_still_reported_as_a_gpu(monkeypatch, capsys):
    """No regression: exit 0 with a device name is exactly what this check is for, and must still return True and print the name."""
...
def test_an_absent_tool_stays_silent(monkeypatch, capsys):
    """The normal case on nearly every box."""
...
def test_a_clean_exit_with_no_output_is_not_a_gpu(monkeypatch, capsys):
    """Exit 0 and nothing to say proves nothing."""
...
# --------------------------------------------------------------------------- #
#  The downstream harm - the reason this rates above cosmetic                  #
# --------------------------------------------------------------------------- #

def test_a_failing_smi_does_not_suppress_the_cpu_only_verdict(
        cli_runner, monkeypatch):
    """The load-bearing one. _check_gpu_verdict's step (3) returns EARLY when smi/torch claim a GPU, so a broken driver counted as a GPU ALSO swallowed the 'No GPU detected ..."""
...
def test_a_working_smi_still_suppresses_the_cpu_only_verdict(
        cli_runner, monkeypatch):
    """The other direction, so the fix cannot be 'always warn'."""
    monkeypatch.setattr(cli, "find_binary_dir", lambda: None)
    monkeypatch.setattr(doctor_mod, "_check_native_abi", lambda: None)
    monkeypatch.setattr(doctor_mod, "_check_vram_torch", lambda: False)
    monkeypatch.setattr(doctor_mod, "_check_hf_backend_usable", lambda *a, **k: None)
