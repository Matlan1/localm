...
def test_a_data_folder_that_is_not_deleted_is_not_reported_as_removed(tmp_path, venv_template):
    """A symlinked data folder is never deleted. Asked to delete the saved
    data, setup.sh says what it did not do and exits 2."""
...
def _data_probe(clone: Path) -> Path:
    """setup.sh's say/ask helpers and its data-folder section, verbatim."""
    text = (ROOT / "setup.sh").read_text(encoding="utf-8")
    start = text.index("say() {")
    ask = re.search(r"(?m)^ask\(\) \{", text)
...
def _run_probe(clone: Path, stdin: str):
    env = dict(os.environ, PYTHONPATH=str(ROOT))
    return subprocess.run([shutil.which("bash"), str(_data_probe(clone))], cwd=str(clone),
                          input=stdin, capture_output=True, text=True, timeout=120, env=env)


def _venv_clone(tmp_path: Path, template: Path) -> Path:
    clone = tmp_path / "clone"
    clone.mkdir()
    shutil.copytree(template, clone / ".venv", symlinks=True)
    return clone
...
def test_a_repair_keeps_the_data_folder_in_use(tmp_path, venv_template):
    ...
    assert "LocaLM's data for this folder is in:" in out.stdout and str(custom) in out.stdout
    assert "Where should localm keep its data" not in out.stdout
    assert "PROBE_DONE" in out.stdout, (out.stdout, out.stderr)


def test_a_repair_that_declines_asks_where_and_keeps_the_old_folder_on_record(
        tmp_path, venv_template):
    clone = _venv_clone(tmp_path, venv_template)
    custom = tmp_path / "my data"
    im.prepare_data(clone, data_dir=str(custom))
    out = _run_probe(clone, "n\n1\n")
...
def test_a_first_install_is_not_asked_to_keep_anything(tmp_path, venv_template):
    clone = _venv_clone(tmp_path, venv_template)
