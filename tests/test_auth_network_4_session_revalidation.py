...
"""AUTH-NETWORK-4 (security): resolving a cookie session to its owning key's hash was implemented twice with different revalidation behavior."""
...
def test_revoked_key_session_no_longer_resolves_a_principal(monkeypatch):
    """The security-relevant case: mint a session for a scoped key, then have the key stop being live (revoked/expired) WITHOUT the session itself being explicitly revoked (e.g. natural TTL expiry never runs revoke_by_key_hash - see auth.revoke_key's docstring: key_hash_live re-validation is the PRIMARY en..."""
...
def test_admin_owner_session_exempt_from_key_liveness(monkeypatch):
    """An ADMIN/owner session is deliberately NOT gated on key_hash_live (an owner key roll must not log the owner out) - only SCOPED-key sessions are."""
    monkeypatch.setenv("LOCALM_API_KEY", "ownersecret")
...
def test_principal_id_cookie_branch_matches_principal_from_token(monkeypatch):
    """The two 'resolve a cookie to its owning key hash' implementations must now be ONE implementation, not two that can silently diverge again."""
    monkeypatch.setenv("LOCALM_API_KEY", "ownersecret")
    key = auth.create_key("A", [scopes.MODELS_READ])["key"]
    key_hash = auth._hash_key(key)
    sid = sessions.create(scopes={scopes.MODELS_READ}, key_hash=key_hash, fs_access="none")
