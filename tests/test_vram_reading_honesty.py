...
def _list_gpus_double(gpus, status):
    """A faithful double of list_gpus()'s real two-shape contract: a bare list by default, ``(gpus, status)`` when return_status=True."""
...
def _impatient_wait(read_free, **kw):
    """The REAL wait_for_vram_release with its clock compressed - same logic, same branches, without spending the endpoint's full 5s poll budget per test."""
    kw["timeout_s"] = 0.05
...
class TestLiveFreeReader(unittest.TestCase):
    """The reader wait_for_vram_release() polls must refuse to pass a served-stale number off as a current one."""
    ...
    def test_returns_none_when_the_probe_was_busy(self):
        """BUSY is the same class of non-reading as TIMEOUT: no fresh probe was taken, so the served value is last-known-good, i.e. possibly frozen."""
        with patch("localm.discover.list_gpus",
...
class TestWaitForVramReleaseCannotClaimNotReleased(unittest.TestCase):
    """'released=False' is a positive claim that VRAM did NOT drop."""
    ...
    def test_live_reading_with_no_rise_still_reports_false(self):
        """The honest negative must survive: a real, fresh reading that genuinely did not rise is still a truthful vram_freed=false."""
        released, final = wait_for_vram_release(
...
class _UnloadCase(unittest.TestCase):
    ...
    def assertNoUnqualifiedNotFreed(self, body):
        """The contract for an unverifiable outcome: never a bare 'VRAM was not freed', and never an unflagged number."""
...
def _probe_dies_after_first_read():
    """list_gpus() answers the endpoint's BEFORE read, then loses the driver for every AFTER poll - serving the frozen value the before-read just cached."""
    calls = {"n": 0}
...
class TestUnloadAllEndpointHonesty(_UnloadCase):
    ...
    def test_live_probe_still_reports_the_real_numbers(self):
        """The fix must not blind the working path: a live probe showing a real rise must still report vram_freed=true with both byte counts, unflagged."""
    ...
    def test_live_probe_with_no_rise_still_reports_the_honest_false(self):
        """A genuinely-not-freed result must survive the fix intact, unflagged - but ONLY on a DEVICE-GLOBAL reading, which is now asserted, not assumed."""
        with patch("localm.discover.list_gpus",
    ...
    def test_process_scoped_no_rise_is_flagged_not_an_honest_false(self):
        """The complement, and the whole reason scope had to be added: on a PROCESS-SCOPED reading a no-rise is NOT proof that nothing was freed."""
        with patch("localm.discover.list_gpus",
                   side_effect=_list_gpus_double(
    ...
    def test_box_with_no_vram_telemetry_says_nothing_at_all(self):
        """The benign case must NOT be dressed up as the fault."""
        with patch("localm.discover.list_gpus",
...
class TestUnloadOneEndpointHonesty(_UnloadCase):
    """unload_one_model() carries its own reader + reporter - the per-model Unload button."""
    ...
    def test_process_scoped_no_rise_is_flagged_here_too(self):
        """The per-model button must not lie where 'unload all' is honest."""
        from localm.inference import http_server as hs
        with patch.dict(hs._engines, {"test-model": self.engine}, clear=False), \
...
class TestUnloadEmbedderEndpointHonesty(_UnloadCase):
    """_unload_embedder_if_matches() is the THIRD unload path, and the one the two classes above do not exercise: the per-row Unload button for a resident EMBEDDING model, which lives outside _engines on its own lifecycle."""
    ...
    def test_reading_lost_after_a_live_before_is_not_reported_as_not_freed(self):
        """#694's shape (fresh device-scoped BEFORE, driver lost for the AFTER polls) on the embedder path."""
        calls = {"n": 0}
    ...
    def test_process_scoped_no_rise_is_flagged_here_too(self):
        """The embedder button must not lie where the other two unload paths are honest."""
...
class TestMediaSwapMessageHonesty(unittest.TestCase):
    """The media VRAM handover is where a user actually SEES this. #693 fixed the API body but not this line, so image/music/video generation still told the user 'VRAM has not dropped yet' on the strength of a reading the server had just flagged as possibly stale."""
    ...
    def test_in_use_is_not_reported_as_unloaded(self):
        """Regression: a pinned chat engine (mid-generation) makes unload_all_models() return {'status': 'in_use', 'vram_freed': 0} without raising and without a non-2xx - so resp.ok is True and the old code fell straight through the already_unloaded check into the generic 'Chat model unloaded.' branch (0 is n..."""
        ok, lines = self._push_lines({
    ...
    def test_partial_unload_with_pinned_chat_is_not_reported_as_unloaded(self):
        """Regression: unload_all_models() reports {'status': 'unloaded'} whenever ANYTHING was freed (an idle embedding model is enough - released_anything wins the status), even when the chat model itself was skipped as in-use."""
        ok, lines = self._push_lines({
...
class TestMediaSwapHonorsPinEndToEnd(_UnloadCase):
    """The regression test above hand-crafts the {'status': 'in_use', ...} body - it pins the CONTRACT but not that unload_all_models() actually produces that body for a genuinely pinned engine."""

    class _Adapter:
        """Wraps the TestClient's httpx response in the requests.Response shape unload_chat_for_media actually touches (.ok/.status_code/.json())."""
    ...
    def test_partial_unload_pinned_chat_plus_freed_embedder_end_to_end(self):
        """The partial shape driven through the REAL route: the chat engine is pinned (active_requests set like an in-flight request) while a resident embedding model frees successfully, so unload_all_models() genuinely returns status 'unloaded' + skipped_in_use=[engine]."""
        self.engine.active_requests = 1          # a request is mid-generation
...
class TestMcpModelSwitchHonesty(unittest.TestCase):
    """The MCP server's model switch runs the same before/after cycle, and made the same false 'VRAM free did not rise' claim."""
    ...
    def test_live_probe_with_no_rise_still_says_it_did_not_rise(self):
        """The honest negative survives: both ends live AND device-global, genuinely no rise."""
    ...
    def test_process_scoped_no_rise_cannot_claim_it_did_not_rise(self):
        """The scope complement (#697 follow-up): a fresh but PROCESS-scoped reading cannot see the previous model's VRAM in its isolated worker, so a no-rise proves nothing."""
    ...
    def test_stale_before_with_a_recovered_after_still_cannot_claim_no_rise(self):
        """The INVERSE shape, and just as ordinary: the before-read times out on a cold driver init - which is itself what WARMS the driver - so the after-polls answer."""
        from localm.plugins.mcpserver.server import EngineCache
        calls = {"n": 0}
