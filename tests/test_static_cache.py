...
"""Seamless updates: the GUI shell and its assets must be served with a revalidation cache policy so a new app.js / index.html / sw.js is picked up automatically, without the user (or a tester) clearing the browser cache."""
...
def _a_served_js() -> str:
    """A served GUI JS module path that exists, robust to the ESM split (app.js and pages.js became app/*.js + pages/*.js)."""
...
def test_assets_revalidate():
    """JS/CSS assets must carry Cache-Control: no-cache so new code is picked up without a manual cache clear."""
    with _client() as c:
...
def test_unchanged_asset_can_304():
    """no-cache is not no-store: an unchanged asset still revalidates to a cheap 304 (the ETag round-trip), so caching efficiency is preserved."""
    with _client() as c:
        asset = _a_served_js()
        r1 = c.get(asset)
