...
"""The import-time LOCALM_HOME must be cleaned up, and the cleanup must not go silently blind."""
...
# Loads the SHIPPED tests/conftest.py by path under its own module name - the
# same thing the two guard test modules do, which is the leak's real-world
# trigger - then reports the directory it made and applies one variation.
_PROBE = r'''
...
class TestTheDirectoryIsCleanedUpWhenTheProcessExits:
    """Half (i), and the actual defect: a process that merely IMPORTS the conftest must not leave a directory behind."""
    ...
    def test_the_temp_root_is_left_empty(self, tmp_path):
        """Nothing else lingers either: not a stray marker file, not a partial tree."""
    ...
    def test_cleaning_up_twice_is_harmless(self, tmp_path):
        """pytest_sessionfinish cleans up at the end of a session and atexit cleans up again at exit, so the second call always meets a directory that is already gone."""
        home, _ = _run_probe(tmp_path, "twice")
        assert not os.path.exists(home)


class TestAWrongTempRootIsDetectedAndItsEvidenceKept:
    """Half (ii): the replacement oracle."""
    ...
    def test_a_worker_is_judged_against_what_the_controller_stamped(self, tmp_path):
        """The worker path: the expectation arrives in workerinput, over execnet's channel, and is what the worker is measured against."""
        root = tmp_path / "temproot"
    ...
    def test_a_stamped_root_beats_the_env_seam(self, tmp_path):
        """The env var can only ADD a check to a run that had none."""
        root = tmp_path / "temproot"
        home, out = _run_probe(tmp_path, "configure-worker", temp_root=root,
                               stamped_root=tmp_path / "the-other-drive",
    ...
    def test_an_UNSTAMPED_worker_reports_that_the_check_is_not_armed(self, tmp_path):
        """The one way this oracle can silently fail to exist: a worker whose controller never loaded tests/conftest.py, so nothing stamped the expectation."""
        root = tmp_path / "temproot"
        home, out = _run_probe(tmp_path, "configure-worker", temp_root=root,
    ...
    def test_nothing_is_checked_when_no_root_was_declared(self, tmp_path):
        """An ordinary single-process ``pytest`` declares no intent and must not be nagged: no expectation, no complaint, and still no leak."""
        home, out = _run_probe(tmp_path, "configure")
...
class TestARealRunGoesRedAndKeepsTheEvidence:
    """The end-to-end half: a real pytest process, through the shipped hooks and the shipped fixture, not a hand-built config object."""
    ...
    def test_only_the_first_test_is_failed(self, pytester, sub_temp_root,
                                           monkeypatch):
        """A misplaced worker still ran its tests correctly; it just wrote to the wrong drive."""
        monkeypatch.setenv("LOCALM_TEST_EXPECTED_TEMP_ROOT",
                           str(sub_temp_root / "not-where-we-landed"))
        _with_real_conftest(pytester)
        pytester.makepyfile(
...
class TestTheExpectationReachesTheWorkersUnderXdist:
    """The claim the whole design rests on: the expectation travels over execnet's workerinput channel rather than the environment, so a worker that lost the env still gets judged."""
    ...
    def test_the_controller_stamps_its_own_resolved_root(self, tmp_path):
        """Unit-level companion to the run above, so a delivery failure and a wrong-value failure are distinguishable rather than one red blob."""
        root = tmp_path / "temproot"
...
class TestThisSessionsOwnGuardIsArmed:
    ...
    def test_the_running_session_is_not_sitting_on_an_unreported_mismatch(
            self, request):
        """If this session's temp root were wrong, the autouse fixture would have failed the first test in this process rather than this one, so asserting it here makes the expectation legible instead of implicit."""
        conftest = _this_sessions_conftest(request)
    ...
    def test_the_cleanup_handler_is_registered_in_this_session(self, request):
        """atexit exposes no API for listing its callbacks, so the registration itself is pinned statically, against the SHIPPED file."""
        conftest = _this_sessions_conftest(request)
