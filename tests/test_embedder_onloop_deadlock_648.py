...
"""Regression tests for #648: get_embedder()/evict_chat_for_embedder must never freeze the server event loop."""
...
def test_evict_on_loop_does_not_freeze_the_server(hsclean):
    """Calling evict_chat_for_embedder ON the server-loop thread (as an async memory route does via a synchronous get_embedder()) must return promptly, NOT block the whole loop for the timeout."""
    ran = {"unload": False}
    ...
    # It must NOT have blindly scheduled+blocked (which is the deadlock). The
    # guard degrades to loading the embedder alongside the chat model ('skipped').
...
def test_evict_off_loop_still_evicts(hsclean):
    """The working off-loop path (executor thread / media-job path) must be UNCHANGED: evict still routes the unload through the loop and completes."""
    ran = {"unload": False}
...
def test_memory_append_resolves_embedder_off_the_loop(tmp_path, monkeypatch):
    """POST /api/memory/append must not resolve/load the embedder on the event loop: get_embedder()/embed() has to run on an executor thread."""
