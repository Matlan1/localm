...
class TestEngineLoadSerialization(unittest.TestCase):
    """Model loads must serialise process-wide, even across separate Engine instances (server chat vs a background job), to avoid the concurrent-load VRAM storm that can freeze the machine (issue B1)."""
...
class TestEngineGpuPlacement(unittest.TestCase):
    """Engine.gpu_placement: whether the last load's transformer layers ended up on GPU or CPU, so a caller can tell a full GPU load from a silent CPU fallback (AGENTS.md rule 5) instead of a bare 'loaded' that hides it."""
