...
"""The chat<->media VRAM handover (vram.py's unload_chat_for_media / reload_chat_after_media) must actually authenticate on a keyless (open-mode, default) server."""
...
def _real_open_mode_server():
    """A real uvicorn instance on a real loopback socket, open (keyless) mode, with app.state.instance_token set the way instances.advertise() sets it in production (unconditionally, for the life of the process) - so the fix under test sees exactly what a real server would hand it."""
    app = create_app(None)
...
def test_unload_chat_for_media_authenticates_with_instance_token_open_mode():
    """The regression test: WITH the fix's instance_token threaded through, the real round trip must get PAST _origin_guard (no 403) and reach the real /v1/models/unload handler, which reports nothing loaded."""
...
def test_unload_chat_for_media_still_403s_with_no_token_open_mode():
    """Sanity/negative: the gate was never bypassable by omitting the credential either - this is what the bug looked like before the fix, and proves the fix is additive (a real credential is still required)."""
    app, port, server, th = _real_open_mode_server()
    try:
        self_url = f"http://127.0.0.1:{port}/v1"
        job = _FakeJob()
...
def test_reload_chat_after_media_authenticates_with_instance_token_open_mode():
    """Same fix, the sibling function/endpoint (/v1/models/load)."""
    app, port, server, th = _real_open_mode_server()
    try:
        self_url = f"http://127.0.0.1:{port}/v1"
        job = _FakeJob()
