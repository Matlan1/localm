...
"""Tests for scripts/mutation_scope.py - the merge-base diff check behind the
mutation-scope CI notice: whether a change touches a [tool.mutmut]
only_mutate module or the mutation gate itself, and whether the notice says
the mutation gate runs on the pull request.
...
class TestMain:
    ...
    @pytest.mark.parametrize("gate_runs", ["true", "false"])
    def test_notice_is_printed_only_on_a_hit(self, repo, capsys, gate_runs):
        _commit(repo, "localm/auth.py", "def f():\n    return 2\n")
        argv = ["--base", "master", "--repo", str(repo), "--notice", "--gate-runs", gate_runs]
        assert ms.main(argv) == 0
        out = capsys.readouterr().out
        assert "::notice title=Mutation gate" in out and "localm/auth.py" in out
    ...
    def test_notice_without_the_label_says_the_gate_did_not_run(self, repo, capsys):
        _commit(repo, "localm/auth.py", "def f():\n    return 2\n")
        assert ms.main(["--base", "master", "--repo", str(repo),
                        "--notice", "--gate-runs", "false"]) == 0
        notice = [ln for ln in capsys.readouterr().out.splitlines() if ln.startswith("::notice")]
        assert len(notice) == 1
        assert notice[0].startswith("::notice title=Mutation gate did not run::")
        assert "weekly schedule" in notice[0]

    def test_notice_with_the_label_says_the_gate_runs_on_this_pull_request(self, repo, capsys):
        """NEGATIVE: a labelled pull request is running the shards and the
        gate; its notice must not say the gate did not run."""
        _commit(repo, "localm/auth.py", "def f():\n    return 2\n")
        assert ms.main(["--base", "master", "--repo", str(repo),
                        "--notice", "--gate-runs", "true"]) == 0
        notice = [ln for ln in capsys.readouterr().out.splitlines() if ln.startswith("::notice")]
        assert len(notice) == 1
        assert "did not run" not in notice[0]
        assert notice[0].startswith("::notice title=Mutation gate runs on this pull request::")
        assert "localm/auth.py" in notice[0]

    def test_notice_without_gate_runs_is_a_usage_error(self, repo, capsys):
        """NEGATIVE: the notice's text depends on the label; a caller that does
        not say whether the gate runs gets an error, not a guess."""
        _commit(repo, "localm/auth.py", "def f():\n    return 2\n")
        with pytest.raises(SystemExit) as exc:
            ms.main(["--base", "master", "--repo", str(repo), "--notice"])
        assert exc.value.code == 2
        captured = capsys.readouterr()
        assert "::notice" not in captured.out and "touched=" not in captured.out
        assert "--gate-runs" in captured.err
