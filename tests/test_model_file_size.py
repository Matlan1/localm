...
"""``_model_file_size()`` (localm/inference/http_server.py) - the size feeding ``_gpu_registry_sync``'s ``vram_estimate_bytes``, a field OTHER localm instances read as 'how much VRAM does this peer hold' (see gpu_registry.py's docstring and its one consumer's ``isinstance(e, int) and e > 0`` guard in t..."""
...
def test_single_file_that_is_genuinely_empty_returns_zero_not_none(tmp_path, monkeypatch):
    """A single FILE that stat() confirms is 0 bytes is a real, measured answer (unlike the directory case below, where 0 means 'found nothing to sum') - it must stay 0, not be papered over into None."""
    model = tmp_path / "model.gguf"
    model.write_bytes(b"")
    _patch_get_model_info(monkeypatch, model)
...
def test_empty_directory_returns_none_not_a_suspicious_zero(tmp_path, monkeypatch):
    """The regression this file exists for: an empty directory (or one whose weights rglob never reaches) must report 'not measured' (None), not a zero that a reader could mistake for 'this peer legitimately holds 0 bytes of model on GPU'."""
...
def test_directory_with_only_subdirectories_returns_none(tmp_path, monkeypatch):
    """rglob('*') matches the subdirectory entries themselves, but the is_file() filter drops every one of them - same 'nothing measured' shape as a flat empty directory, reached via a different path through the generator."""
    d = tmp_path / "model-dir"
