...
class TestClearApiKeyReportsWhatSurvived:
    ...
    def test_the_failure_is_also_written_to_the_local_log(
        ...
        """The HTTP route deliberately logs NOTHING of its own, on the grounds that clear_api_key already records each failure here with the path and the OS error."""
        import logging
        runner.invoke(main, ["key", "generate"])
        key_name = auth.key_file().name
        _unlink_denied_for(monkeypatch, key_name)
    ...
    def test_label_is_path_free_so_a_network_caller_can_use_it(
        ...
        """``what`` must never carry the path: it is the ONLY field the HTTP route is allowed to put on the wire, so if it embedded the path the route's disclosure boundary would be defeated silently."""
        runner.invoke(main, ["key", "generate"])
        _unlink_denied_for(monkeypatch, auth.key_file().name)
...
class TestKeyClearCliDoesNotClaimSuccess:
    ...
    def test_failed_clear_does_not_print_a_success_tick(self, runner, monkeypatch):
        ...
        # The pre-fix output was exactly this sentence, unconditionally.
        assert "api key cleared - open mode" not in out


class TestClearRouteDoesNotClaimSuccess:
    """The API contract matters more than the CLI string: the GUI renders open-mode state from ``cleared``, so a false true is shown to the user as a keyless install."""
