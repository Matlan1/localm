...
"""Regression pin for #617 and its follow-up: a GGUF model load (or the voice/STT worker) must not fail when running under the branded LocaLM.exe launcher - neither at spawn time nor on the worker's first real use."""
...
class TestRealRenamedLauncherEndToEnd:
    """Builds an ACTUAL renamed-copy launcher (mirroring applaunch.py's make_windows_launcher construction byte-for-byte) and drives a REAL multiprocessing.Queue round trip through it - the exact shape ModelRunner/voice._spawn_worker use (send a command, get a response back)."""

    @staticmethod
    def _build_fake_launcher(tmp_path: Path) -> Path:
        """A fake venv (pyvenv.cfg pointing at the REAL base interpreter's own directory) with a renamed copy of that base interpreter one level under it - exactly applaunch.py's ``<venv>/localm-app/LocaLM.exe`` layout, so it reproduces CPython's OWN sys._base_executable derivation bug (```<base_prefix>/<basen..."""
...
class TestInterpreterForLocalmChildren:
    """interpreter_for_localm_children() - the exe for PLAIN subprocess children that must import localm and its venv packages (the VRAM-probe daemon)."""
    ...
    def test_worker_state_finds_venv_python_from_sys_path_windows(
        ...
        """The mp-spawn worker shape: base-interpreter exe (prefix == base_prefix) with the venv's site-packages injected into sys.path."""
        venv = tmp_path / "venv"
    ...
    def test_no_venv_on_sys_path_falls_back_to_sys_executable(
            self, monkeypatch, tmp_path):
        """A genuinely venv-less setup (system python + PYTHONPATH): keep today's behavior. sys.path is replaced for the call window so the REAL test venv's site-packages cannot leak in and satisfy the walk."""
        monkeypatch.setattr(sys, "prefix", str(tmp_path / "base"))
        monkeypatch.setattr(sys, "base_prefix", str(tmp_path / "base"))
    ...
    def test_site_packages_without_pyvenv_cfg_is_not_a_venv(
            self, monkeypatch, tmp_path):
        """A bare site-packages (a system python's own, or a stray dir named that) must not be mistaken for a venv."""
