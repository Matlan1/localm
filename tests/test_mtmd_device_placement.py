...
def _no_inherited_env(monkeypatch):
    """MTMD_BACKEND_DEVICE is process-global and these tests assert on its exact presence/absence, so start every one of them from a known-unset state."""
...
class TestResolveBackendDeviceName:
    ...
    def test_cpu_device_position_does_not_shift_the_index(self, monkeypatch):
        """The CPU device is skipped wherever it sits in the registry."""
    ...
    def test_integrated_gpu_first_also_refuses(self, monkeypatch):
        """Same divergence, opposite direction: with the iGPU enumerated FIRST, llama.cpp's index 0 is the DISCRETE card while this list's index 0 is the integrated one."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", IGPU), ("Vulkan1", GPU)])
        assert mtmd_mod._resolve_backend_device_name(1) is None

    def test_accel_device_present_refuses(self, monkeypatch):
        """llama.cpp skips ACCEL entirely when building model->devices, so an ACCEL entry ahead of a GPU shifts every later index by one."""
    ...
    def test_empty_device_name_refuses(self, monkeypatch):
        """An unnamed device cannot be handed to MTMD_BACKEND_DEVICE at all; setting it to '' would read as unset to clip anyway."""
    ...
    def test_refusal_is_logged_with_its_reason(self, monkeypatch, caplog):
        """Rule 5: the user asked for device 1 and the projector is NOT going there."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", IGPU)])
...
def _ctx(lib, gpu_index):
    """A real MtmdContext with its native-loading __init__ bypassed (the same convention test_mtmd_vision.py uses), so _open runs for real."""
...
class TestOpenSetsTheEnvironment:
    ...
    def test_env_is_cleared_even_when_the_native_call_raises(self, monkeypatch):
        """Process-global state, so the finally is load-bearing: a leaked value would silently pin every LATER mtmd load in this process too."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", GPU)])
    ...
    def test_ambiguous_registry_never_sets_the_env(self, monkeypatch):
        """The guard, observed through _open rather than through the resolver - so a future refactor that resolves correctly but forgets to honour None still fails."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", IGPU)])
        lib = _FakeLib()
        _ctx(lib, 1)._open(use_gpu=True)
        assert lib.seen_env is None
    ...
    def test_cpu_attempt_never_sets_the_env(self, monkeypatch):
        """clip gates the env read on use_gpu, so the CPU attempt and retry_on_cpu() must not touch process-global state to no purpose."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", GPU)])
        lib = _FakeLib()
    ...
    def test_a_user_set_value_is_neither_overwritten_nor_deleted(self, monkeypatch):
        """never-override-user-selection: an explicit export outranks anything localm derives from config, and must still be there afterwards."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", GPU)])
    ...
    def test_an_exported_empty_value_is_still_the_users(self, monkeypatch):
        """An exported-but-empty MTMD_BACKEND_DEVICE is the user's variable, and it is NOT equivalent to unset for clip (it takes the getenv branch, fails to init by that name, warns, and falls back)."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", GPU)])
    ...
    def test_localm_mtmd_cpu_escape_hatch_still_short_circuits(self, monkeypatch):
        """The pre-existing opt-out must keep skipping the GPU attempt entirely, env resolution included."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", GPU)])
...
class TestConstructorAppliesItInTime:
    """Every test above sets ``_gpu_index`` by hand, so none of them can catch the constructor setting it too LATE - after ``_open`` has already run."""
    ...
    def test_unusable_index_warns_and_keeps_vision(self, monkeypatch, caplog):
        """A bad index must not cost the user vision, and must not be silent either (rule 5)."""
        _devices(monkeypatch, [("CPU", CPU), ("Vulkan0", GPU), ("Vulkan1", GPU)])
        lib = self._fake_lib_class(_FakeLib())
        monkeypatch.setattr(mtmd_mod, "_load_lib", lambda: lib)
        monkeypatch.setattr(mtmd_mod, "_input_text_class", mtmd_mod._MtmdInputTextV2)
...
class TestLlamaCppPassesTheResolvedDevice:
    ...
    def test_load_mmproj_forwards_the_resolved_main_gpu(self, monkeypatch):
        """The wiring. mp.main_gpu is read AFTER apply_gpu_split, which forces it inside the configured split set - so this is the index the projector must follow, and it has to actually arrive at MtmdContext."""
    ...
    def test_load_mmproj_defaults_to_zero_when_unset(self, monkeypatch):
        """_load_mmproj is unit-tested against instances that never ran __init__, so the attribute can be absent; 0 is the leave-clip-alone value."""
        from localm.inference.backends.llamacpp import llama as llama_mod
        seen = {}
