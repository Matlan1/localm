...
"""SRV-5: debug mode must make the SERVER CONSOLE verbose - mirror debug logs to stderr and raise uvicorn's log level - not just write to a file."""
...
def test_console_stream_is_a_private_stderr_dup():
    """LOG-1: the console mirror must write through a PRIVATE duplicate of stderr, not fd 2 itself."""
...
def test_console_stream_survives_fd_redirect(tmp_path, monkeypatch):
    """LOG-1 regression: the mirror must keep writing WHILE its underlying fd is redirected elsewhere (what _quiet_stderr/_capture_stderr do to fd 2 around a model load / generation)."""
