...
def test_pywin32_is_declared_for_windows():
    """The actual gap. gpu_usage.py reaches for win32pdh, so something has to put it on a clean Windows install."""
    import tomllib
...
def test_a_missing_source_does_not_become_a_zero_reading(no_pywin32, monkeypatch):
    """The specific wrong answer worth naming: 0 bytes used reads as 'the card is empty', which is the opposite of unknown and would let a caller load a model onto a full GPU."""
    monkeypatch.setattr(gpu_usage, "_adl_used_by_bus", lambda: {})
    monkeypatch.setattr(gpu_usage, "_adl_state", {}, raising=False)
    gpus = [{"index": 0, "name": "Non-AMD GPU", "total": 8 * 1024 ** 3}]
