...
class _FakeFn:
    """A stand-in for a bound ctypes function: accepts restype/argtypes and returns a fixed value when called."""
...
def test_main_gpu_garbage_refuses(builder):
    """A pointer low-word landing in main_gpu is what a shifted layout looks like; before this check model_params had no bound on that field at all."""
    mp = builder()
...
def test_all_valid_load_modes_allowed(mode):
    """LLAMA_LOAD_MODE_{NONE,MMAP,MLOCK,MMAP_MLOCK,DIRECT_IO} are all real enumerators - narrowing this set would refuse a legitimate build."""
    mp = good_model_v2()
    mp.load_mode = mode
    assert evaluate(mp, good_ctx()).status == "ok"
...
def test_v2_detection_survives_a_drifted_default():
    """The value fingerprint may go INCONCLUSIVE (a future build changes a default) without that becoming a refusal - only a conclusive CONTRADICTION refuses."""
    mp = good_model_v2()
...
def test_ctx_v2_bytes_read_as_v1_would_have_been_missed_before_and_are_caught_now():
    """The exact silent-corruption case the context_params split exists for - same shape as test_v2_bytes_read_as_v1_..._are_caught_now above, one axis over."""
...
def test_ctx_v2_detection_survives_a_drift_in_each_fingerprinted_field(
        field, badval):
    """A legitimate ctx_v2 build that drifts ONE fingerprinted default must still be IDENTIFIED as ctx_v2 - a determination, not the V1 fallback."""
    cp = good_ctx_v2()
    setattr(cp, field, badval)
    layout, notes, assumed = _abi.detect_context_params_layout(
...
def test_ctx_v1_detection_survives_a_drift_in_each_fingerprinted_field(
        field, badval):
    """The ctx_v1 side of the same property, so the fix cannot be a ctx_v2 special case that quietly costs ctx_v1 builds their determination."""
    cp = good_ctx_v1()
    setattr(cp, field, badval)
    layout, notes, assumed = _abi.detect_context_params_layout(
        _FakeLib(good_model(), cp))
...
def test_ctx_v2_keystone_drift_still_refuses_and_names_the_real_field(field):
    """The complement, so the fix cannot have bought its silence by weakening the gate: the three keystones evaluate() hard-refuses on still refuse when they genuinely drift, and the message names the field that actually moved."""
    cp = good_ctx_v2()
    setattr(cp, field, 0)
    with pytest.raises(AbiMismatch) as ei:
        verify_abi(_FakeLib(good_model(), cp))
...
def test_context_fingerprint_is_unambiguous_on_the_real_builds(builder, want):
    """The measured baseline the scoring rests on, mirroring test_fingerprint_is_unambiguous_on_the_real_builds for model_params."""
    cp = builder()
    raw = bytes(bytearray(
...
def test_context_fingerprint_still_says_inconclusive_when_it_genuinely_is():
    """All-zero bytes support neither layout's ctx_type-not-(-1) check (0 != -1 passes for BOTH candidate ctx_type offsets, and the -1 runs both fail since everything is 0) - must stay inconclusive rather than picking a winner, exactly the same never-false-positive posture as the model_params fingerprint."""
...
def test_context_params_layout_falls_back_to_v1_when_inconclusive():
    """detect_context_params_layout has no second (symbol) signal to fall back on, unlike model_params - an inconclusive fingerprint goes straight to the V1 fallback (the layout localm shipped before this field existed anywhere), with `assumed=True` so callers know not to treat it as a determination."""
...
def test_unknown_third_layout_still_fails_safe():
    """The guarantee documented on verify_abi(): a layout NEITHER V1 nor V2 (e.g. a hypothetical future upstream insertion on top of today's V2) must never silently misbind."""
    cp = good_ctx_v2()
...
def test_main_gpu_write_lands_at_the_layout_correct_offset():
    """The concrete corruption: on a V2 build, a main_gpu written at V1's offset 24 silently becomes load_mode - changing how weights are mapped and dropping the user's GPU selection, with no error."""
    v1 = good_model_v1()
    v1.main_gpu = 3
...
def test_set_use_mmap_preserves_an_mlock_request_on_v2():
    """V2 folded mmap and mlock into one enum, so a naive mapping of 'no mmap' -> LLAMA_LOAD_MODE_NONE would silently drop a caller's mlock."""
    from localm.inference.backends.llamacpp._structs import (
...
def test_set_use_mmap_on_v2_never_touches_the_v1_boolean_block():
    """Belt and braces: every byte from vocab_only onward must be unchanged except through load_mode, so no V2 boolean is collaterally flipped."""
...
def test_penalties_arity_is_cached_not_re_probed_per_call():
    """ggml_version() is a C call plus a restype/argtypes rebind on a SHARED function object; re-doing it per request is pure overhead for an answer that cannot change while one library handle is held."""
...
def test_unknown_arity_warns_once_not_per_request(monkeypatch, caplog):
    """A per-request warning is how a real warning gets ignored."""
    import logging
...
def test_ctx_v2_with_a_minus_one_ctx_type_is_a_known_undetectable_misbind():
    """The context_params axis's OWN residual - so its fail-safe is not read as absolute either."""
    cp = good_ctx_v2()
    cp.ctx_type = -1
    v = verify_abi(_FakeLib(good_model(), cp))
    assert v.status == "ok", v.failures
...
def test_evaluate_does_catch_a_misaligned_read():
    """The capability the checks above DO have: a pointer low-word, or a -1 in a field whose valid range excludes it, is refused - in either layout."""
    mp = good_model_v2()
...
def test_determined_v1_still_proves_4arg():
    """The complement, so the guard above cannot silently cost real lemonade b1288 users their repetition penalty: a genuine pre-reorder build exports no llama_load_mode_* symbols and fingerprints cleanly as v1."""
...
def test_fingerprint_survives_one_drifted_default(builder, want):
    """Inconclusive is the state where localm binds on the symbol probe alone, with nothing able to corroborate OR contradict it - so it must be rare."""
    mp = builder()
...
def test_fingerprint_still_says_inconclusive_when_it_genuinely_is():
    """Scoring must not manufacture confidence: bytes that support neither layout stay inconclusive rather than picking a weak winner, because a wrong 'conclusive' answer would trigger a false contradiction and refuse a build that is actually fine."""
...
def test_fingerprint_drift_no_longer_forces_the_symbol_probe_to_stand_alone():
    """End to end: a V2 build with a drifted default is now CORROBORATED rather than resting on symbols alone, so it carries no 'symbol probe alone' note."""
    mp = good_model_v2()
...
def test_penalties_arity_boundary_is_exact():
    """The two ggml bounds are load-bearing and adjacent, so pin the exact edges rather than only sampling the middle of each band. 0.18.0 is the one version that must stay undecidable; the versions either side of it must not."""
    def arity(v):
        _abi._detected_layout = None
        _abi._layout_assumed = False
        _abi._detected_arity = None
...
def test_abi_report_still_reports_ok_when_the_check_actually_ran(monkeypatch):
    """The complement: preserving the verdict must not turn every report into 'skipped'."""
    from localm.inference.backends.llamacpp import _loader
...
def test_abi_report_carries_the_probe_notes_a_re_derivation_would_drop():
    """The stored verdict is strictly more informative than a re-derived one: it keeps the layout-probe notes, which evaluate() never sees."""
