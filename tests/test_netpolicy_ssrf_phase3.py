...
def _no_dns(monkeypatch):
    """Make getaddrinfo unavailable so the test exercises the literal-IP normalization path, not host resolution."""
...
def test_numeric_loopback_forms_refused(monkeypatch, host):
    """The adversarial case from the spec: each numeric/short IPv4 that maps to loopback is refused when private access is off, even when DNS would not resolve it."""
...
def test_numeric_loopback_message_names_canonical_ip(monkeypatch, host):
    """The refusal should classify the canonical 127.0.0.1, so the message is actionable rather than echoing the obfuscated literal."""
    _with_config(monkeypatch, {"net_mode": "allow", "net_allow_private": False})
    _no_dns(monkeypatch)
...
def test_numeric_loopback_allowed_when_private_enabled(monkeypatch):
    """With net_allow_private True the same obfuscated loopback must pass: the normalization must not over-block when the operator opts in."""
...
def test_normal_public_host_still_passes(monkeypatch):
    """A genuine public hostname is unaffected: inet_aton rejects it, so the code falls through to ordinary DNS-based classification."""
    _with_config(monkeypatch, {"net_mode": "allow", "net_allow_private": False})
    monkeypatch.setattr(
        "socket.getaddrinfo",
...
def test_public_numeric_ip_passes(monkeypatch):
    """A numeric form that maps to a public address must NOT be blocked by the new normalization (no false positives)."""
    _with_config(monkeypatch, {"net_mode": "allow", "net_allow_private": False})
    _no_dns(monkeypatch)
