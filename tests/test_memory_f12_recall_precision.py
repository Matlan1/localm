...
def _measure_embed(texts):
    """3-axis one-hot topic stub (measurement / editor / other), so distinct topics are ORTHOGONAL - a paraphrased measurement query cosine-matches a metric fact (no shared token) but an editor query and generic fillers do NOT (each on its own axis)."""
