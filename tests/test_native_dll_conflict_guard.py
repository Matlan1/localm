...
"""A real, root-cause fix for the torch/ROCm DLL-identity conflict documented in VramSizingMixin._torch_rocm_init_broken (llamacpp/_sizing.py): once llama.cpp's own native runtime is loaded in a process (always true inside GgufWorker or the embedder's isolated worker - each loaded it themselves to run..."""
...
class TestFreeTotalVramBytesSkipsTorchWhenNativeLibLoaded:
    def test_never_touches_torch_once_native_lib_is_loaded(self, monkeypatch):
        """The actual regression this fix closes: a fresh worker process has ALWAYS just loaded llama.cpp's native lib (that is what a worker is for), so this condition is true on every single worker load, not a rare edge case."""
    ...
    def test_still_attempts_torch_when_native_lib_not_loaded(self, monkeypatch):
        """Control: the parent process (GgufBackend, before any child is spawned) never loads llama.cpp's native lib itself, so its own preflight VRAM checks must keep using the fast in-process torch.cuda read exactly as before - this fix must not affect that path."""
    ...
    def test_still_short_circuits_on_the_pre_existing_broken_cache(self, monkeypatch):
        """The backstop this fix keeps: if _torch_rocm_init_broken was ALREADY set (some other path hit the conflict and cached it), this method must still skip torch even when native_lib_loaded() says False."""
        monkeypatch.setattr(_loader, "native_lib_loaded", lambda: False)
...
class TestVramLevelsSkipsTorchWhenNativeLibLoaded:
    """VramSizingMixin._vram_levels() must get the SAME proactive skip as its sibling _free_total_vram_bytes() above: it also does a bare `import torch` under a broad except, so a resident native runtime makes it re-hit and re-trace the identical STATUS_ENTRYPOINT_NOT_FOUND fault on every call (observed li..."""
...
def _torch_stub_with_poisoned_cuda(side_effect):
    """A torch stand-in for a FULL `doctor()` CLI run (not just the unit-level _check_vram_torch() call): a bare MagicMock() does not survive _check_packages()'s later ``importlib.import_module('torch')`` re-fetch of the already-cached module - first ``ValueError: torch.__spec__ is not set`` (a Mock has no..."""
...
class TestCheckVramTorchSkipsTorchWhenNativeLibLoaded:
    """`localm.cli.doctor._check_vram_torch` is the FOURTH call site for the same DLL-identity conflict as the two classes above and discover._torch_gpu_probe_known_doomed: it also did a bare `import torch` under only a narrow `except ImportError`."""
    ...
    def test_non_importerror_torch_failure_does_not_escape(self, monkeypatch, caplog):
        """The other half of this fix: a torch import/probe failure that is NOT a plain ImportError (the doomed DLL-identity conflict itself, reached some other way, or any other native fault) must degrade torch_gpu_found to False rather than propagate out of _check_vram_torch() - the original bug this pins, s..."""
        import logging
    ...
    def test_doctor_cli_survives_the_doomed_combination_end_to_end(
        ...
        """Wires the exact doctor() concern through the REAL CLI command: with native_lib_loaded() True, `doctor()` must still run every later check (proven by the managed-comfy hint, its very last print) instead of dying at the GPU verdict - and torch.cuda must still never be touched, proving the guard itself..."""
    ...
    def test_doctor_cli_survives_a_non_importerror_torch_failure_end_to_end(
            self, cli_runner, monkeypatch):
        """The property that matters most about this fix, pinned at the CLI level and kept separate from the guard test above: a torch-probe failure that is NOT the DLL-conflict guard's precondition (native_lib_loaded() False here, so the guard does not fire at all) must still not stop doctor() from running an..."""
        from localm import cli as _cli
...
class TestCheckPackagesSkipsTorchWhenNativeLibLoadedAndNotResident:
    """`localm.cli.doctor._check_packages` is a FIFTH call site for the same DLL-identity conflict, found live while verifying the fourth (_check_vram_torch above): its own bare ``importlib.import_module('torch')`` (under only ``except ImportError``) raised the real ``OSError: [WinError 127]`` when test_do..."""
    ...
    def test_keeps_an_already_resident_torch_even_if_native_lib_loaded(
            self, monkeypatch):
        """The regression this refinement guards against: an ALREADY-resident torch (real success earlier in this process, or a test double) must not be discarded just because native_lib_loaded() is True."""
    ...
    def test_non_importerror_torch_failure_does_not_truncate_later_packages(
            self, monkeypatch):
        """The widened except half, independent of the guard (native_lib_loaded() False here): a torch import failure that is NOT a plain ImportError must not stop LATER packages (transformers, the last in the loop) from still being checked."""
        self._no_torch_resident(monkeypatch)
        monkeypatch.setattr(_loader, "native_lib_loaded", lambda: False)
        monkeypatch.setattr(doctor_mod, "console",
                            Console(file=io.StringIO(), force_terminal=False))
