...
def test_unload_embedder_if_matches_syncs_registry_off_the_loop(probe, monkeypatch, tmp_path):
    """The targeted-unload counterpart: it already offloads loaded_path(), the cheap active_requests() precheck, reset_embedder(force=False) (which now makes the final busy/idle decision atomically under embedder._LOCK - see its docstring) and the VRAM wait, each with a comment naming the event-loop freeze..."""
