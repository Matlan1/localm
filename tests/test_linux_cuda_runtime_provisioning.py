...
"""Self-contained CUDA on Linux end to end - see dev-notes/ADR-0010: - fetching NVIDIA's CUDA runtime libraries (cudart/cublas) as plain PyPI wheels (Unit 1); - resolving the compiled binary from a third-party prebuilt (hybridgroup/llama-cpp-builder), since upstream publishes none for Linux and localm..."""
...
def _make_wheel_zip(path: Path, so_names: tuple, extra_names: tuple = ()) -> None:
    """A real, valid zip shaped like an nvidia-* wheel: some .so* files under a package dir, plus non-.so metadata files that must NOT be copied out."""
    import os
...
def test_pypi_wheel_url_and_sha_picks_linux_x86_64_wheel(monkeypatch):
    """Matches the REAL PyPI JSON shape (confirmed live against pypi.org this session) - must pick the x86_64 linux .whl, never the aarch64 wheel or the sdist tarball that also satisfy a looser match."""
...
def test_fetch_pypi_runtime_lib_refuses_wrong_checksum(monkeypatch, tmp_path):
    """THE NEGATIVE CASE: a checksum mismatch must refuse the file entirely - proves the guard actually guards, not just that the happy path works."""
    wheel_src = tmp_path / "src" / "fake.whl"
    wheel_src.parent.mkdir()
...
def test_fetch_cuda_runtime_libs_stops_on_first_failure(monkeypatch, tmp_path):
    """A partially-assembled CUDA runtime is worse than none - must not swallow one package's failure and silently continue with the rest."""
    calls = []
...
def test_cuda_13_line_uses_unsuffixed_package_names():
    """Pins the naming-migration finding from this session (verified live against PyPI: nvidia-cublas-cu13 etc. are deprecated stubs) so a future edit cannot silently regress to the deprecated -cu13 names."""
...
def test_no_line_fetches_nccl():
    """hybridgroup/llama-cpp-builder's actual binary was checked directly - none of its 26 shared libraries reference libnccl at all (unlike an earlier prototype against a different build - see dev-notes/ADR-0010) - so fetching it would be pure waste."""
...
def test_resolve_linux_cuda_asset_picks_cuda13_line(monkeypatch):
    """THE cuda_line-AWARENESS CASE: hybridgroup publishes both a cuda-12 asset (bare '-cuda-x64.tar.gz') and a cuda-13 one ('-cuda-13-x64.tar.gz') - a Blackwell GPU (NvidiaInfo.cuda_line == 'cuda-13') must get the cuda-13 asset, never the cuda-12 default."""
    monkeypatch.setattr(sl, "_platform_key", lambda: "linux")
    monkeypatch.setattr(
        sl, "_latest_tag",
        lambda: pytest.fail("the Linux CUDA branch must resolve the pin, "
...
def test_resolve_linux_cuda_asset_never_picks_the_sha256_sidecar(monkeypatch):
    """THE NEGATIVE CASE: the release also lists a .sha256 sidecar file - it must never be mistaken for the tarball itself (which would then get 'extracted' as an archive and fail confusingly downstream)."""
    monkeypatch.setattr(sl, "_platform_key", lambda: "linux")
    monkeypatch.setattr(
        sl, "_latest_tag",
        lambda: pytest.fail("the Linux CUDA branch must resolve the pin, "
...
def test_resolve_linux_cuda_asset_raises_when_not_yet_built(monkeypatch):
    """A real, occasionally-expected state (hybridgroup has not built this exact upstream tag yet) - must raise click.ClickException, which _provision_with_fallback's caller already turns into the vulkan fallback, never construct a guessed URL (a guessed URL against a third party's naming is even less trus..."""
    monkeypatch.setattr(sl, "_platform_key", lambda: "linux")
...
def test_resolve_backend_asset_windows_cuda_unaffected_by_linux_branch(monkeypatch):
    """Regression guard: the linux-cuda special case must never intercept the EXISTING Windows cuda path, which has its own resolver (_resolve_cuda_pair, exercised in test_cuda_setup.py) predating this."""
...
def test_provision_backend_linux_cuda_never_calls_windows_cuda_pair(monkeypatch, tmp_path):
    """Regression guard: the Linux branch must take its own path, never fall through into the Windows-only _resolve_cuda_pair/cudart-bundle logic directly above it in the same function."""
    monkeypatch.setattr(sl.sys, "platform", "linux")
    monkeypatch.setattr(sl, "_resolve_backend_asset",
...
def test_resolve_linux_cuda_asset_real_network(monkeypatch):
    """Not mocked at all beyond forcing the linux branch: a real GitHub Releases lookup against hybridgroup/llama-cpp-builder, proving the actual asset-name convention and repo are correct right now, not just matching a fixture of them."""
    monkeypatch.setattr(sl, "_platform_key", lambda: "linux")
    url, sha, _tag = sl._resolve_backend_asset("cuda", cuda_line="cuda-12")
...
def test_fetch_pypi_runtime_lib_real_network(tmp_path):
    """Not mocked at all: a real PyPI JSON lookup and a real wheel download, proving the actual mechanism this feature depends on, not a fixture of it."""
    target = tmp_path / "runtime"
    target.mkdir()
    n = sl._fetch_pypi_runtime_lib("nvidia-cuda-runtime-cu12", target)
    assert n > 0
