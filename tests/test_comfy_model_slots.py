...
"""workflow_model_slots() / apply_model_overrides() (the model-picker feature): enumerate every model-file combo slot in a workflow against ComfyUI's live /object_info, and let a caller override any of them by node_id/input_name."""
...
class TestWorkflowModelSlots:
    ...
    def test_slot_surfaces_when_comfyui_has_none_of_that_file_type_installed(self):
        """The reproduced bug: ComfyUI has ZERO files of a given type installed, so its live /object_info reports that combo's options as [] - which alone can never look like model files (_looks_like_model_files([]) is False)."""
        info = _object_info()
...
class TestApplyModelOverrides:
    ...
    def test_unknown_input_name_on_a_real_node_is_a_noop(self):
        """A stale/malformed override (naming a field that does not exist on that node) must never CREATE a new key - only ever write an EXISTING string input."""
        wf = _workflow()
        changed = comfy_client.apply_model_overrides(
    ...
    def test_cannot_overwrite_a_non_string_input(self):
        """A number/link input must never be clobbered by a string override - this is the graph-corruption guard, not just a UX nicety."""
        wf = _workflow()
...
class TestPreflightModelsStillWorksAfterRefactor:
    """preflight_models() now builds on workflow_model_slots() internally - pin that its own observable behavior (substitution + missing-file message) is unchanged."""
