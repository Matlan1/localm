...
matching comment in routes/chat.py's embeddings handler): an omitted model on
/v1/embeddings now resolves like /v1/chat/completions and /v1/completions -
"no preference", fall back to the active/default model, refuse only when
nothing can be resolved at all. This is not mirroring the other two routes
just for consistency's sake: the dedicated-embedder fast path (a registry/
config exact-name match) is provably unreachable with an empty model (both
checks require a truthy match), so relaxing the gate can only ever route an
omitted request into get_engine()'s general resolution - the exact chain
chat/completions already use, which itself already normalises "localm" and ""
identically (get_engine's own name check). And Engine.embed()'s own contract
already degrades honestly: a resolved engine that cannot itself embed
transparently falls back to the dedicated on-device embedder, raising
NotImplementedError (-> 422, actionable message) only when no embedding path
exists at all - so relaxing the gate cannot produce a silently-wrong 200,
only a real answer or an honest failure.
