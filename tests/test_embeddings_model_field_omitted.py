...
"""An omitted 'model' field on /v1/embeddings must be distinguishable from an explicit 'localm' - the sibling fix to test_chat_model_field_omitted.py (checkup item 17), deliberately split out because this route also has its own unconditional required-model gate that the sentinel default made unreachabl..."""
...
def server(monkeypatch):
    """A server started with a chat model already loaded and active, that CAN embed directly (no dedicated embedder configured) - so an unnamed request must reach the general get_engine() fallback and use this engine's own .embed(), the same shape /v1/chat/completions' fallback reaches."""
...
def test_explicit_localm_still_echoes_localm_on_embeddings(server):
    """The documented behaviour that must NOT change (matches the chat/completions contract): a client that actually sends 'localm' gets 'localm' back, not the resolved engine's real name."""
    client, engines = server
...
def test_omitted_model_still_400_when_nothing_can_be_resolved(monkeypatch):
    """The gate decision's negative half, identical contract to test_chat_model_field_omitted.py's equivalent test: with no model loaded and none ever configured, an unnamed embeddings request is genuinely unserveable regardless of whether 'unnamed' means the field was left out or sent as ''."""
...
def test_omitted_model_honest_422_when_resolved_engine_cannot_embed(monkeypatch):
    """The gate decision's other guardrail: relaxing the gate must not turn into a silently-wrong 200."""
    monkeypatch.setattr("localm.config.load_registry",
...
def test_omitted_model_prefers_the_configured_embedder_over_an_active_chat_model(monkeypatch):
    """The load-bearing guarantee raised in review: an omitted model must not silently drift onto whatever chat model happens to be active."""
    registry = {
