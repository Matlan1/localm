...
# loop cannot hang the suite: it terminates and the recorded call list shows how
# many times the peer was actually asked. Assertions below pin that count.
_MAX_COOPERATE = 4
...
def coordinated(monkeypatch):
    """A registered, coordination-enabled instance whose GPU is too full for the incoming model, with no local eviction candidate (nothing else loaded)."""
...
def _install_peers(monkeypatch, peers, on_request):
    ...
    def _request(peer, **kw):
        ...
        if len(calls) > _MAX_COOPERATE:
            return False        # break an unbounded loop so the test cannot hang
...
def test_peer_is_not_re_asked_forever_when_it_keeps_advertising_a_model(
    ...
    """The non-progressing loop: a peer whose post-unload registry write failed keeps advertising a model and keeps answering 'already_unloaded' (True), so the caller must NOT keep asking it."""
...
def test_peer_models_are_not_yanked_when_freeing_them_cannot_help(
        coordinated, monkeypatch):
    """The fit check: the peer holds only 1 GB, so freeing it (2 + 1 = 3 GB) still leaves a 13 GB requirement short."""
...
def test_split_instance_still_asks_a_peer_on_its_other_split_device(
        coordinated, monkeypatch):
    """The same-GPU filter must offer EVERY device this load can actually use, not one scalar index."""
...
def test_cooperation_still_happens_when_it_can_actually_free_enough(
        coordinated, monkeypatch):
    """The feature must still work: a peer holding enough IS asked, and once it frees the VRAM the load proceeds."""
    peer = {"instance_id": "peer-1", "port": 8082, "model": "big",
            "vram_estimate_bytes": 12 * GB, "gpu_index": 0,
            "coordination_token": "x"}
    freed = {"yes": False}
