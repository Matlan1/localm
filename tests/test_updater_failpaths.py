...
"""Failure-path coverage for the self-updater that the existing suite did not yet pin (TEST-2)."""
...
def _signed_build(version, deps):
    """A build zip (bytes) + its base64 signature under the test key; returns ``(download_opener, signature_b64)`` so apply()'s signature gate accepts it."""
    buf = io.BytesIO()
    pyproject = ('[project]\nname = "localm"\ndependencies = ['
...
def test_apply_rolls_back_when_post_step_raises(tmp_path, monkeypatch):
    """The runner RAISING (not just exiting non-zero) is a distinct branch: it must still roll back and surface a 'crashed' LocalmError, never a half-applied tree."""
...
def test_apply_crash_then_rollback_failure_demands_manual_recovery(tmp_path, monkeypatch):
    """Worst case: the post step CRASHES and rollback ALSO fails. apply() must say manual recovery is needed - never hide a broken install behind a clean message."""
    monkeypatch.setattr("localm.config.load_config", lambda: {"bugreport_upload_url": "https://w"})
    monkeypatch.setattr("localm.config.home_dir", lambda: tmp_path / "home")
    _pin_sig_env(monkeypatch)
    inst = _fake_install(tmp_path, deps=("click",))
...
def test_installed_backend_preserves_what_is_actually_installed(tmp_path, monkeypatch):
    """THE regression test: an install with amd-rocm actually provisioned must stay on amd-rocm across an update, even though the CURRENT hardware recommendation has since moved to something else entirely."""
...
def test_installed_backend_falls_back_to_recommendation_when_nothing_provisioned(
    ...
    """No marker on disk (a fresh install, or one predating the marker) means there is nothing to preserve, so this is a first-time pick - not an override - and falls back to the current hardware recommendation."""
    from localm import hwdetect, setup_llama
...
def test_installed_backend_falls_back_to_vulkan_when_everything_fails(monkeypatch):
    """Both the marker read AND the hardware detection raise -> the universal default."""
    from localm import hwdetect, setup_llama
