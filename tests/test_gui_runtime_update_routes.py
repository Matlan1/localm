...
def runtime_dir(tmp_path, monkeypatch):
    """Redirect setup_llama's notion of 'the runtime lib dir' to a throwaway tmp dir, so the routes' own installed_backend()/check_runtime_update() calls never touch this box's real provisioned runtime."""
    d = tmp_path / "rt"
    d.mkdir()
...
def no_subprocess(monkeypatch):
    """Stub JobManager.start_cli so the update route dispatches a job without spawning a real setup-llama subprocess."""
    calls = []
...
def test_check_reports_a_newer_build_available(app, runtime_dir, monkeypatch):
    """The card offers the build setup-llama would install - which is the CONFIRMED pin, not upstream's newest."""
    monkeypatch.setattr(
        sl, "_latest_tag",
...
def test_check_reports_a_previous_build_when_one_is_on_record(app, runtime_dir):
    """`previous` is --rollback's own target (setup_llama.previous_tag), folded into the same read-only check so the runtime card can decide whether a rollback affordance has anything to offer without a second round trip."""
    sl._record_provisioned_backend(runtime_dir, "vulkan", build="b10300")
...
def test_update_dispatches_job_with_force_and_yes(app, runtime_dir, no_subprocess):
    """NEW-UPDATE-RUNTIME-CLASS-IS-A-NO-OP applies here too: without --force setup-llama's own already-provisioned guard would make this a no-op; --yes keeps the unattended job from ever hanging on a prompt."""
    sl._record_provisioned_backend(runtime_dir, "vulkan", build="b10300")
...
def test_update_with_nothing_installed_provisions_with_auto_detect(app, no_subprocess):
    """The uninstalled case used to 409 ('nothing to update - run setup first'), which made INITIAL PROVISIONING unreachable for a user who has only the GUI - and left a box whose runtime failed to provision with no route back, since the one surface still working refused the one action that would fix it."""
    with TestClient(app) as client:
        r = client.post("/api/runtime/update")
...
def test_update_switches_backend_when_one_is_named(app, runtime_dir, no_subprocess):
    """--backend, the switch a GUI user could not reach."""
    sl._record_provisioned_backend(runtime_dir, "vulkan", build="b10300")
...
def test_update_forwards_a_tag(app, runtime_dir, no_subprocess):
    """--tag installs AND PINS one exact build."""
    sl._record_provisioned_backend(runtime_dir, "vulkan", build="b10300")
...
def test_update_treats_blank_fields_as_absent(app, runtime_dir, no_subprocess):
    """An untouched select and an empty text box are the GUI's normal state, so they must mean 'no request', not a request for the empty string."""
    sl._record_provisioned_backend(runtime_dir, "vulkan", build="b10300")
...
def test_update_refuses_an_unknown_backend_before_dispatching(app, no_subprocess):
    """400, not a job that starts and fails: the caller gets the reason now, and the value set is setup_llama.BACKENDS - the same tuple the CLI's own click.Choice is built from, so the two surfaces cannot disagree."""
    with TestClient(app) as client:
...
def test_update_refuses_a_tag_the_cli_would_refuse(app, no_subprocess):
    """The route validates with setup_llama.is_safe_tag, the SAME predicate _validated_tag uses, so a tag is judged once."""
    with TestClient(app) as client:
...
def test_update_refuses_when_already_running_whatever_was_asked_for(
    ...
    """ONE job kind covers install, switch and update, so a switch cannot be started on top of an update already rewriting the same directory."""
...
def _seed_two_builds(runtime_dir, backend="vulkan"):
    """Two successive provisions, so there is exactly one build to roll back to: the marker (installed_build()) lands on the newer one and history holds both, matching what a real setup-llama run leaves behind."""
...
def test_rollback_honors_an_explicit_backend(app, runtime_dir, no_subprocess):
    """The installed backend is cuda, but a build is on record for vulkan (e.g. from before a switch) - an explicit backend must look up ITS history, the same as `setup-llama --backend vulkan --rollback` would."""
...
def test_rollback_refuses_when_nothing_is_recorded_to_roll_back_to(app, runtime_dir, no_subprocess):
    """Only ONE build recorded (the one installed now) - previous_tag has nothing that is not the current build, so this must 400 before dispatch rather than start a job the CLI would refuse anyway."""
    sl._record_provisioned_backend(runtime_dir, "vulkan", build="b10300")
    sl._record_runtime_history("vulkan", "b10300")
...
def test_rollback_treats_an_explicit_auto_backend_as_unnamed(app, runtime_dir, no_subprocess):
    """setup_llama._apply_version_request treats `--backend auto --rollback` as 'no backend named' (falls back to installed_backend()), because 'auto' is the CLI's hardware-detect default, not a real provisioned backend that could ever have history."""
    _seed_two_builds(runtime_dir)
...
def test_rollback_refuses_amd_rocm(app, runtime_dir, no_subprocess):
    """amd-rocm's build is fixed by the localm release, not chosen from upstream releases, so there is nothing for --rollback to move - same refusal setup_llama's own _apply_version_request gives on the CLI."""
