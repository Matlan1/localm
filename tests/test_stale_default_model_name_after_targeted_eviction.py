...
"""Sibling regression to test_stale_default_model_name_after_switch.py."""
...
class FakeEngine:
    """Stands in for the model backend only - see test_stale_default_model_name_after_switch.py."""
...
def two_model_server(monkeypatch):
    """Boots with model-a (the startup/default model), then switches to model-b before the eviction under test - identical premise to test_stale_default_model_name_after_switch.py's fixture of the same name."""
...
def test_get_engine_resolves_switched_model_after_unload_one_model(two_model_server):
    """unload_one_model(name) sibling of the unload_all_models fix: evicting the ACTIVE model by name alone (leaving the background model-a resident and untouched, since unload_one_model only touches its target) must still leave model-b resolvable by an unnamed request, exactly like unload_all_models alrea..."""
...
def test_get_engine_resolves_switched_model_after_idle_unload(two_model_server):
    """_idle_unload_once(ttl) sibling: when the idle-evicted model was the LAST one resident, its own partial-recovery fallback (_engines_lru[-1]) finds nothing and _active_model_name goes to None - must still be recoverable via _last_active_model_name, exactly like the other two eviction paths."""
    engines = two_model_server
