...
test("planModelFetch: net_mode=off refuses outright - off has no bypass", () => {
...
// these tests target.
function installFetchEnv(win, { netMode }) {
  win.fetch = async (url) => {
    const u = String(url);
    if (u.includes("/api/tts/config")) {
      return { ok: true, status: 200, json: async () => ({ net_mode: netMode }) };
    }
...
test("load(): net_mode=off refuses without ever offering the dialog", async () => {
  ...
  assert.equal(win.$("modal").style.display, "none",
    "off has no bypass - no confirmation dialog is offered");
  const errToast = calls.toasts.find((t) => t.isError);
  assert.ok(errToast, "a toast must explain the refusal");
  assert.match(errToast.msg, /net_mode=off/);
  assert.match(errToast.msg, /localm config net_mode ask/);
